...
#define ANDROID_LOG_NAME	"dnsmasq-W"
#endif /* ANDROID_LOG_NAME */
#define DEBUG_LEVLE_1 1
...
//		subnet_addr.s_addr ? inet_ntoa(subnet_addr) : (mess->giaddr.s_addr ? inet_ntoa(mess->giaddr) : iface_name));
  ...
  */
  ...
  switch (mess_type)
    ...
    //add by sw 2013-5-22 20:13:32 start
    {
        char pmac[24] = {'\0'};
        ...
        if(addr1) {
            ...
            print_mac(pmac, emac, emac_len);
            android_printf(DEBUG_LEVLE_1, "1.clientInfo[emac_len=%d,iface_name=%s,hostname=%s]", emac_len, iface_name, hostname);
            android_printf(DEBUG_LEVLE_1, "2.clientInfo[mac=%s,ip=%s]", pmac, addr1 ? inet_ntoa(a) : "none");
            // notify p2p framework client got its ip address.
            if(strstr(iface_name, "p2p")) {
                int sock = -1;
                asprintf(&networkInfo, " %s,%s,%s", iface_name, inet_ntoa(a), pmac);
                if ((sock = socket_local_client("android.net.wifi.p2p", ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_STREAM)) < 0) {
                    android_printf(DEBUG_LEVLE_1, "Error connecting (%s)\n", strerror(errno));
                } else {
                    if (write(sock, networkInfo, strlen(networkInfo)) < 0) {
                        android_printf(DEBUG_LEVLE_1, "write clientInfo error !!!");
                    } else {
                        usleep(1000*20);//20ms
                    }
                    android_printf(DEBUG_LEVLE_1, "[%s]", networkInfo);
                    close(sock);
                }
                free(networkInfo);
            } else {
                android_printf(DEBUG_LEVLE_1, "not p2p interface,NO need to notify p2p framework!!!");
            }
        }
    }
    //add by sw 2013-5-22 20:13:43 end
	  clear_packet(mess, end);
	  option_put(mess, end, OPTION_MESSAGE_TYPE, 1, DHCPACK);
	  option_put(mess, end, OPTION_SERVER_IDENTIFIER, INADDRSZ, ntohl(server_id(context, override, fallback).s_addr));
	  option_put(mess, end, OPTION_LEASE_TIME, 4, time);
