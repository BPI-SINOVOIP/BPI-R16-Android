        ...
        int uid)
{
    ...
    if (audio_is_linear_pcm(format)) {
        ...
        mFrameSizeAF = channelCount * audio_bytes_per_sample(format);//sizeof(int16_t);
    } else {
...
//        return BAD_VALUE;
//    }
     // limit outout sampling rate to 192k
    if (rate <= 0 || rate > 48000 * 2*2 ) return BAD_VALUE;
...
{
	struct timespec request = ClientProxy::kForever;
	if(mFlags & AUDIO_OUTPUT_FLAG_DIRECT)
	{
		request = ClientProxy::kNonBlocking;
	}
    ...
    while (userSize >= mFrameSize) {
        ...
        status_t err = obtainBuffer(&audioBuffer, &request);
        if (err < 0) {
            if (written > 0) {
                break;
            }
