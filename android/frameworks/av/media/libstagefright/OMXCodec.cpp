...
struct OMXCodecObserver : public BnOMXObserver {
    ...
    virtual void onMessage(const omx_message &msg) {
        ...
        if (codec.get() != NULL) {
			if(strcmp(codec->mComponentName, "OMX.allwinner.video.encoder.avc") == 0){
				usleep(0);
			}
...
void OMXCodec::setVideoInputFormat(
	...
	bool	success_recorder = meta->findInt32(kKeySrcWidth, &src_width);
	success_recorder = success_recorder && meta->findInt32(kKeySrcHeight, &src_height);

	if(success_recorder)
	{
		video_def->nFrameWidth = src_width;
    	video_def->nFrameHeight = src_height;
    	video_def->nStride = src_width;
	}
...
//not to force set the profile be baseline
#if 0
    // XXX
