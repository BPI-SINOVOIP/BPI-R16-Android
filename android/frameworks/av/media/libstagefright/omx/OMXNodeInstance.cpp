...
status_t OMXNodeInstance::createInputSurface(
    ...
    {	//* set use android graphic buffer.
    
    	OMX_STRING name = const_cast<OMX_STRING>("OMX.google.android.index.enableAndroidNativeBuffers");
    	OMX_INDEXTYPE index;
    	OMX_ERRORTYPE err = OMX_GetExtensionIndex(mHandle, name, &index);
    	...
    	if (err == OMX_ErrorNone) 
		{
	    	...
	    	ver.s.nStep = 0;
	    	EnableAndroidNativeBuffersParams params = {
	        	sizeof(EnableAndroidNativeBuffersParams), ver, portIndex, OMX_TRUE,
	    	};

	    	OMX_SetParameter(mHandle, index, &params);
    	}
    }
