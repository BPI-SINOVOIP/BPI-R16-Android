...
status_t MPEG4Extractor::parseChunk(off64_t *offset, int depth) {
    ...
    switch(chunk_type) {
		...
		case FOURCC('w', 'a', 'v', 'e'):
        {
			ALOGV("wave offset = %llx, chunksize = %llx", *offset, chunk_size);
            status_t err;
			*offset = data_offset;
			off64_t stop_offset = *offset + chunk_size - 8;
        ...
        case FOURCC('s', 'a', 'w', 'b'):
        {
			// for apple mov file, mp4a atom may be in another mp4a 
			if(chunk_data_size < 28)
			{
				ALOGV("offset = %llx, chunk_data_size = %llx", *offset, chunk_data_size);
				*offset += chunk_size;
				break;
			}
            uint8_t buffer[8 + 20];
			...
			uint32_t version = U16_AT(&buffer[8]); // for iso mov file format
			...
			*offset = data_offset + sizeof(buffer);
			...
			if(version == 1) {
				// samples_per_frame (4 bytes)
				// bytes_per_packet (4byts)
				// bytes_per_frame (4 bytes)
            ...
            if (*offset != stop_offset) {
				ALOGV("error, offset = %llx, stop_offset = %llx", *offset, stop_offset);
                return ERROR_MALFORMED;
            }
            break;
        }
