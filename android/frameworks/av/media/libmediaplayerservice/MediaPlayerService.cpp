...
{
	ALOGD("llh>>> mediaplayerservice:setDataSource(): url = %s", url);
    if (url == NULL)
    ...
    if (strncmp(url, "content://", 10) == 0) {
        ...
        return mStatus;
      	...
      	player_type playerType;
    		if(mKey == 0x777){
    				ALOGD("is audio/vnd.dlna.adts");	
    				playerType = AW_PLAYER;
    	  }
    		else{
    			ALOGD("is not audio/vnd.dlna.adts");
    			playerType = MediaPlayerFactory::getPlayerType(this, url);
				ALOGD("llh>>>playerType = %d",playerType);
    		}
...
status_t MediaPlayerService::Client::setNextPlayer(const sp<IMediaPlayer>& player) {
    ...
    if (c != NULL) {
        if (mAudioOutput != NULL) {
            ...
            if(mMsg == MEDIA_PLAYBACK_COMPLETE)
    		{
    			mMsg = MEDIA_NOP;
...
status_t MediaPlayerService::Client::setParameter(int key, const Parcel &request) {
    ALOGD("[%d] Client : setParameter(%d)", mConnId, key);
	  if((key == 0x776) || (key == 0x777)){
	      mKey = key;
	  }
...
status_t MediaPlayerService::Client::setInputDimensionValue(int type, int value)
{
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) 
        return UNKNOWN_ERROR;
    return p->setInputDimensionValue(type, value);
}
...
status_t MediaPlayerService::Client::setOutputDimensionValue(int type, int value)
{
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) 
        return UNKNOWN_ERROR;
    return p->setOutputDimensionValue(type, value);
}
	                                  ...
	                                  struct MediaPlayerInfo* mediaPlayerInfo) 
{
	...
	if ((mClients.size() > 0) && (mediaPlayerId >= 0) && (mediaPlayerId < mClients.size()))
	{
	    for (int i = 0, n = mClients.size(); i < n; ++i) {
	        sp<Client> c = mClients[i].promote();
	        if (c != 0) {
        ...
        const audio_offload_info_t *offloadInfo)
{
    ...
    if (((flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) != 0)||((flags & AUDIO_OUTPUT_FLAG_DIRECT) != 0)) {
...
MediaPlayerService::AudioCache::AudioCache(const sp<IMemoryHeap>& heap) :
    ...
    mError(NO_ERROR),  mCommandComplete(false), mMsecsPerFrame(0)
{
}
