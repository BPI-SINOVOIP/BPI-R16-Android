...
static const int DEFAULT_CACHE_START_PLAY_SIZE               = 128*1024;
...
typedef struct DemuxCompContext_t
{
	...
	//* YUNOS Info
	cdx_int64					tsSendTime;
	cdx_int64					tsFirstBtime;
...
static int base64Encode(const unsigned char *in,  unsigned long len,  unsigned char *out);
...
static int ParserCallbackProcess(void* pUserData, int eMessageId, void* param)
{
    ...
    switch(eMessageId)
    {
		...
		{
			demux->tsSendTime = *(cdx_int64*)param;
		...
		{
			demux->tsFirstBtime = *(cdx_int64*)param;
			break;
		}
                            ...
                            int                 nMaxBufferSize)
{
    ...
    int nStartPlaySizeWithOutVideo = DEFAULT_CACHE_START_PLAY_SIZE_WITHOUT_VIDEO;
    ...
    switch(eCachePolicy)
    {
        case CACHE_POLICY_QUICK:
        {
            ...
            if(PlayerHasVideo(demux->pPlayer) == 1)
            {
                StreamCacheSetSize(demux->pCache, DEFAULT_CACHE_START_PLAY_SIZE, DEFAULT_CACHE_BUFFER_SIZE);
            }
            else
            {
                StreamCacheSetSize(demux->pCache, nStartPlaySizeWithOutVideo, DEFAULT_CACHE_BUFFER_SIZE);
            }
            break;
        }
        ...
        case CACHE_POLICY_SMOOTH:
        {
            ...
            {
                StreamCacheSetSize(demux->pCache, nStartPlaySizeWithOutVideo, nMaxBufferSize);
            }
            break;
        }
...
static int DemuxCompSetCacheTime(DemuxCompContext* demux, int nCacheTimeMs)
{
    ...
    if(PlayerHasVideo(demux->pPlayer) == 0)
    {
    ...
    switch(eMessageId)
    {
		case 1: //YUNOS_TS_INFO
		{
			...
			if(demux->pParser && param != NULL)
			{
				...
				((cdx_int64*)info)[4] = demux->tsSendTime;
				((cdx_int64*)info)[5] = demux->tsFirstBtime;
