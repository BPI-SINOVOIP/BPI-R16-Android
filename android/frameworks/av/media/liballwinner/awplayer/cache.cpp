...
void StreamCacheDestroy(StreamCache* c)
{
    ...
    while(node != NULL)
    {
		...
		if(node->info)
		{
			cdx_atomic_t *ref = NULL;
			if(node->eMediaType == CDX_MEDIA_VIDEO)
			{
				ref = &((struct VideoInfo *)node->info)->ref;
			}
			else if(node->eMediaType == CDX_MEDIA_AUDIO)
			{
				ref = &((struct AudioInfo *)node->info)->ref;
			}
			else
			{
				ref = &((struct SubtitleInfo *)node->info)->ref;
			}
			CdxAtomicDec(ref);
			if(CdxAtomicRead(ref) == 0)
			{
				free(node->info);
			}
		}
...
int StreamCacheAddOneFrame(StreamCache* c, CacheNode* node)
{
    ...
    newNode->infoVersion    = node->infoVersion;
    newNode->pNext          = NULL;
...
static void StreamCacheFlushPassedList(StreamCache* c)
{
          ...
          c->nPassedDataSize > MAX_PASSED_DATA_SIZE_KEPT)
    {
        ...
        if(node != NULL)
        {
			...
			if(node->info)
			{
                logd("***free the node->info = %p",node->info);
				cdx_atomic_t *ref = NULL;
				if(node->eMediaType == CDX_MEDIA_VIDEO)
				{
					ref = &((struct VideoInfo *)node->info)->ref;
...
int64_t StreamCacheSeekTo(StreamCache* c, int64_t nSeekTimeUs)
{
    ...
    {        
        ...
        case CDX_PARSER_APE: //* must seek by parser
        {
            ret = -1;
            break;
        }
