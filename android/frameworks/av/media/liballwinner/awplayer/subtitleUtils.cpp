...
static int SubtitleUtilsConvertUnicode(char*               pOutBuf, 
                                       int                 nOutBufSize,
                                       const char*         pInBuf,
                                       ESubtitleTextFormat eInputTextFormat,
...
int SubtitleUtilsFillTextSubtitleToParcel(Parcel*       parcel,
...
#define MAX_OUTPUT_TEXT_SIZE (1024)
    ...
    parcel->writeInt32((int)(pSubItem->nPts/1000)); //* pts in unit of us, write out pts in unit of ms.

    parcel->writeInt32(KEY_STRUCT_TEXT);
    parcel->writeInt32(nTextLen);   //* write the size of the text sample
    parcel->writeInt32(nTextLen);   //* write the text sample as a byte array
    parcel->write((char*)strOutText, nTextLen);
    ...
    if(pSubItem->eAlignment != 0)
    {
        ...
        parcel->writeInt32(pSubItem->eAlignment);
        
        parcel->writeInt32(KEY_STRUCT_AWEXTEND_SCREENRECT);
        //*write text subtitle's whole screen rect.
        parcel->writeInt32(0);
        parcel->writeInt32(0);
        parcel->writeInt32(pSubItem->nReferenceVideoHeight);
        parcel->writeInt32(pSubItem->nReferenceVideoWidth);

        parcel->writeInt32(KEY_STRUCT_TEXT_POS);
        //*write text subtitle's position Rect.
        parcel->writeInt32(pSubItem->nStartY);
        parcel->writeInt32(pSubItem->nStartX);
        parcel->writeInt32(pSubItem->nEndY);
        parcel->writeInt32(pSubItem->nEndX);
    }
    ...
    if(pSubItem->nFontSize!=0 && pSubItem->nPrimaryColor!=0)
    {
        ...
        parcel->writeInt32(nFontStyleIdx);
        parcel->writeInt32(KEY_FONT_SIZE);
        parcel->writeInt32((int)pSubItem->nFontSize);
        parcel->writeInt32(KEY_TEXT_COLOR_RGBA);
        parcel->writeInt32((int)pSubItem->nPrimaryColor);
        parcel->writeInt32(KEY_STYLE_FLAGS);
        parcel->writeInt32(nSubStyleFlag);
    }
...
// The Parcel will be sent to MediaPlayer.java through event, and will be
// parsed in TimedText.java.
int SubtitleUtilsFillBitmapSubtitleToParcel(Parcel* parcel, SubtitleItem* pSubItem, int nSubtitleID)
{
    int nBufSize;
...
#if 0
    parcel->writeInt32(pSubItem->ePixelFormat); //* 0: ARGB, 1: YUV, defined in sdecoder.h
    ...
    parcel->writeInt32(pSubItem->nBitmapWidth);
    
    //* write the height of bmp subtitle
    parcel->writeInt32(KEY_STRUCT_AWEXTEND_PICHEIGHT);
    parcel->writeInt32(pSubItem->nBitmapHeight);

	//* write the reference video width of bmp subtitle
    parcel->writeInt32(KEY_STRUCT_AWEXTEND_REFERENCE_VIDEO_WIDTH);
    parcel->writeInt32(pSubItem->nReferenceVideoWidth);
    
    //* write the reference video height of bmp subtitle
    parcel->writeInt32(KEY_STRUCT_AWEXTEND_REFERENCE_VIDEO_HEIGHT);
    parcel->writeInt32(pSubItem->nReferenceVideoHeight);
    ...
    parcel->writeInt32(nBufSize);
    
    //* write the argb buffer as an int32_t array
    parcel->writeInt32(nBufSize/4);
    parcel->write(pSubItem->pBitmapData, nBufSize);
