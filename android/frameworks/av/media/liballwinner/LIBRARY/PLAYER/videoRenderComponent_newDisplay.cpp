        ...
        else if(msg.messageId == MESSAGE_ID_SET_WINDOW)
        {
            ...
            if(p->pLayerCtrl != NULL)
            {
				...
				while(1)
            	{
            		...
            		pPicture = LayerGetPicNode(p->pLayerCtrl);
            		if(pPicture == NULL)
            		{
            			break;
            		}
            ...
            if(p->bHadSetLayerInfoFlag == 0 && p->mFbmBufInfo.nBufNum != 0)
            {
			   ...
			   {
				   p->nGpuBufferNum = LayerSetBufferCount(p->pLayerCtrl, pFbmBufInfo->nBufNum + 2);
			   }
			   else
			   {
				   LayerSetExpectPixelFormat(p->pLayerCtrl,(enum EPIXELFORMAT)pFbmBufInfo->ePixelFormat);
				   p->nGpuBufferNum = LayerSetBufferCount(p->pLayerCtrl, pFbmBufInfo->nBufNum);
			   }
			   p->bHadSetLayerInfoFlag    = 1;
            }
...
static inline int SetGpuBufferToDecoder(VideoRenderCompContext*p)
{
	...
	for(int i = 0; i< p->nGpuBufferNum; i++)
	{
		...
		if(ret == 0)
		{
			if(i == (p->nGpuBufferNum - 2))
			{							
				p->pCancelPicture[0] = VideoDecCompSetVideoFbmBufAddress(p->pDecComp, pTmpVideoPicture, 1);
			}
			else if(i == (p->nGpuBufferNum  - 1))
			{
				p->pCancelPicture[1] = VideoDecCompSetVideoFbmBufAddress(p->pDecComp, pTmpVideoPicture, 1);
			}
			else
			{
	...
	LayerQueueBuffer(p->pLayerCtrl, p->pCancelPicture[0], 0);
	LayerQueueBuffer(p->pLayerCtrl, p->pCancelPicture[1], 0);
	return 0;
}
...
static inline int ResetBufToDecoder(VideoRenderCompContext*p)
{
	...
	if(p->bHadRequestReleasePicFlag == 0)
	{
		...
		{
			LayerReleaseBuffer(p->pLayerCtrl, pReleasePicture);
		}
	}
