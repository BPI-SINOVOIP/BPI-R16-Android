        ...
        if(msg.messageId == MESSAGE_ID_START)
        {
            ...
            else
            {
                ...
                if(p->pSoundCtrl)
                {
                    ...
                    SoundDeviceStart(p->pSoundCtrl,raw_data.raw_flag,raw_data.bitpersample);
#else
        ...
        else if(msg.messageId == MESSAGE_ID_RENDER)
        {
            ...
            if(bFirstFrameSend == 0 && (nSampleRate == 0 || nChannelNum == 0))
            {
                do
                {
                    ...
                    {
                        logd("init sound device.p:%p",p);
#ifdef CONFIG_ENABLE_DIRECT_OUT
                        p->pSoundCtrl = SoundDeviceInit(p->pAudioSink,p->pDecComp,AudioDecRawSendCmdToHalClbk);
#else
            ...
            {
            	/*modify by huangxin*/
                int timems = 40;//(nSampleRate*80/1000)*nFrameSize; //* pcm data of 40ms
                if(nSampleRate == 96000)
					timems = 80;
				if (nSampleRate == 192000) {
					nPcmDataLen = 131072;
				} else {
                // ALOGD("hx AudioRenderThread, l:%d,nPcmDataLen:%d, nFrameSize:%d\n", __LINE__, nPcmDataLen, nFrameSize);
				nPcmDataLen = (nSampleRate*timems/1000)*nFrameSize;
				}
			    pPcmData    = NULL;
				memset(&raw_data,0,sizeof(cedar_raw_data));
                ret = AudioDecCompRequestPcmData(p->pDecComp, &pPcmData, &nPcmDataLen, &nPts,&raw_data);	
				if(!raw_data.RawInitFlag)
				{
					raw_data.bitpersample = 16;
				}

				//loge("nPts:%lld,bitpersample:%d,nFrameSize:%d",nPts,raw_data.bitpersample,nFrameSize);
                //ALOGD("AudioRenderThread, l:%d,nPcmDataLen:%d, nFrameSize:%d bitpersample:%d\n", __LINE__, nPcmDataLen, nFrameSize, raw_data.bitpersample);
                if(ret < 0 || pPcmData == NULL || nPcmDataLen < nFrameSize)
                {
                    ...
                    ret = MessageQueueTryGetMessage(p->mq, &msg, 2); //* wait for 5ms if no message come.
                    if(ret == 0)    //* new message come, quit loop to process.
                    {
                        if(msg.messageId != MESSAGE_ID_PAUSE &&
                           msg.messageId != MESSAGE_ID_STOP  &&
            ...
            if(ret == 0)
            {
...
#ifdef CONFIG_ENABLE_DIRECT_OUT
            	if((nNewSampleRate != nSampleRate) || (nNewChannelNum != nChannelNum)||( ((unsigned int)raw_data.bitpersample != nBitsPerSample)&&(raw_data.bitpersample != 0)))
#else
...
#endif
            	{
            		...
            		if(1)//ret == 0)
            		{
						...
						nBitsPerSample = raw_data.bitpersample;
                        nFrameSize  = nChannelNum*nBitsPerSample/8;
						loge("Ok !  reset foramt nFrameSize:%d",nFrameSize);
            ...
            while(nPcmDataLen > 0)
            {
             //   loge("AudioRenderThread,l:%d write nPcmDataLen:%d\n", __LINE__, nPcmDataLen);

                nWritten = SoundDeviceWrite(p->pSoundCtrl, pPcmData, nPcmDataLen);
//usleep(100);
				//loge("write ret:%d",nWritten);
				if(nWritten > 0)
                {
                ...
                else
                {
                    ...
                    if(ret == 0)    //* new message come, quit loop to process.
                    {
                           ...
                           msg.messageId != MESSAGE_ID_QUIT)
                        {
                            //* post a render message to continue the rendering job after message processed.
                            PostRenderMessage(p->mq);
							if(msg.messageId != MESSAGE_ID_EOS && msg.messageId != MESSAGE_ID_RESET){
                            	AudioDecCompReleasePcmData(p->pDecComp, nPcmDataLen);
								}
                        }
                        goto process_message;
