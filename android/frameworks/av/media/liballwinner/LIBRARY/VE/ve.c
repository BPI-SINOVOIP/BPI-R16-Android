...
int VeChipId(void)
{
	...
	if (dev < 0) 
	{
		loge("cannot open /dev/sunxi_soc_info\n");
...
*/
	...
	#if (CONFIG_CHIP == OPTION_CHIP_1681)
    {
        ...
        read(fd, &ifm, 1);
        close(fd);
        if(ifm != V3_CHIP_ID && ifm != V3S_CHIP_ID_A && ifm != V3S_CHIP_ID_B)
        {
            abort();
        }
    }
...
#if (CONFIG_CHIP==OPTION_CHIP_1623 || CONFIG_CHIP==OPTION_CHIP_1625 || CONFIG_CHIP==OPTION_CHIP_1651)
