...
void IPTV_allbackProcess(IPTV_PLAYER_EVT_e evt, void *handler, unsigned long param1, unsigned long param2)
{
...
status_t CTC_Wrapper::prepareAsync()
{
    ...
    //* send a prepare.
    setMessage(&msg, AWPLAYER_COMMAND_PREPARE);
    AwMessageQueuePostMessage(mMessageQueue, &msg);
    return 0;
}
...
status_t CTC_Wrapper::resume()
{
    ...
    //* send a start message.
    setMessage(&msg, 
               AWPLAYER_COMMAND_RESUME,        //* message id.
               (unsigned int)&mSemStart,      //* params[0] = &mSemStart.
               (unsigned int)&mStartReply);   //* params[1] = &mStartReply.
    AwMessageQueuePostMessage(mMessageQueue, &msg);
    SemTimedWait(&mSemStart, -1);
...
status_t CTC_Wrapper::fast()
{
    ...
    //* send a stop message.
    setMessage(&msg, 
               AWPLAYER_COMMAND_FAST,        //* message id.
               (unsigned int)&mSemStop,      //* params[0] = &mSemStop.
               (unsigned int)&mStopReply);   //* params[1] = &mStopReply.
    AwMessageQueuePostMessage(mMessageQueue, &msg);
    SemTimedWait(&mSemStop, -1);
...
status_t CTC_Wrapper::stopFast()
{
    ...
    //* send a stop message.
    setMessage(&msg, 
               AWPLAYER_COMMAND_STOPFAST,        //* message id.
               (unsigned int)&mSemStop,      //* params[0] = &mSemStop.
               (unsigned int)&mStopReply);   //* params[1] = &mStopReply.
    AwMessageQueuePostMessage(mMessageQueue, &msg);
    SemTimedWait(&mSemStop, -1);
...
status_t CTC_Wrapper::seekTo(int nSeekTimeMs)
{
	...
	pthread_mutex_unlock(&mMutexStatus);		//* protect mSeeking and mSeekTime from being changed by the seek finish callback.
    //* send a start message.
    setMessage(&msg,
...
status_t CTC_Wrapper::getCurrentPosition(int* msec)
{
       ...
       mStatus == AWPLAYER_STATUS_COMPLETE)
    {
        ...
        if(mMediaInfo != NULL)
        {
			nPositionUs = mediaProcessor->GetCurrentPlayTime(); //* ts stream's pts is not started at 0.
...
static void PostReadMessage(AwMessageQueue* mq)
{
    ...
    {
        AwMessage msg;
        setMessage(&msg, AWPLAYER_COMMAND_READ);
        AwMessageQueuePostMessage(mq, &msg);
    }
//	logd("AwMessageQueueGetCount(%d)", AwMessageQueueGetCount(mq));
    return;
}
...
process_message:
        ...
        else if(msg.messageId == AWPLAYER_COMMAND_PREPARE)
        {
			...
			if(mediaInfo.nVideoStreamNum > 0)
			{
				...
				VideoPara.vFmt = videoCodecConvert_1((enum EVIDEOCODECFORMAT)mediaInfo.pVideoStreamInfo[0].eCodecFormat);
			...
			if(mediaInfo.nAudioStreamNum > 0)
			{
				...
				AudioPara.pExtraData = (unsigned char *)mediaInfo.pAudioStreamInfo[0].pCodecSpecificData;
...
//        	logd("AWPLAYER_COMMAND_READ1");
  ...
  //      	logd("AWPLAYER_COMMAND_READ2");
        	...
        	while(mediaProcessor->WriteData(pBuffer, ret) == -1)
        	{
				...
				ret1 = AwMessageQueueTryGetMessage(mMessageQueue, &msg, 10); // wait for 200ms if no message come.
