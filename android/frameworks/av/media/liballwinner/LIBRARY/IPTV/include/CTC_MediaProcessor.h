 ...
 * @param param2 		用户参数
 **/
typedef void (*IPTV_PLAYER_EVT_CB)(IPTV_PLAYER_EVT_e evt, void *handler, unsigned long param1, unsigned long param2);
...
public:
	 ...
	 * 			-1 - 失败
	 **/
	virtual int	GetWriteBuffer(IPTV_PLAYER_STREAMTYPE_e type, unsigned char** pBuffer, unsigned int *nSize) = 0;
	...
	virtual int WriteData(IPTV_PLAYER_STREAMTYPE_e type, unsigned char* pBuffer, unsigned int nSize, uint64_t timestamp) = 0;
#endif
	//暂停
	...
	//离开频道，在离开频道时调用该接口，如果不想在进入下一个频道再次Start，可以在这里记录下，然后在StartPlay接口中继续播放就好
