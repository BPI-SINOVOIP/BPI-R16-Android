...
static inline int64_t get_pts(const unsigned char* p)
{
...
static int push_es_data(unsigned char* data, int len, int new_frm, void* param)
{
	...
	while (len > 0)
	{
		...
		{
			logv("es->cur_ptr(%p), dataptr(%p), len(%d)", es->cur_ptr, dataptr, len);
		...
		{
			logv("es->cur_ptr(%p), dataptr(%p), es->free_size(%d)", es->cur_ptr, dataptr, es->free_size);
...
int ts_demux_handle_packets(void* handle, unsigned char* pktdata, int data_size)
{
    ...
    for(; pkt<=(buf_end - pkt_size); pkt += pkt_size)
    {
        ...
        if(result < 0 && result != -2)
            return result;
        else if(result == -2)	//* the player is switching status, decoder can not give buffer at this time.
        {
            result = 0;
            break;
        }
