...
static void String2Utf16(char *dest,char *src, int len) 
{
    int i = 0;
    if (len > 499) 
    {
...
//*********************************************************************************************************************//
//*********************************************************************************************************************//
static void Put32 (command_t *cmd, unsigned int value)
{
    cmd->buf[cmd->numBytes] = value % 256;        /* 取低8位 */
    value = value >> 8;
    cmd->buf[cmd->numBytes+1] = value % 256 ;
    value = value >> 8;
    cmd->buf[cmd->numBytes+2] = value % 256 ;
    value = value >> 8;
    cmd->buf[cmd->numBytes+3] = value % 256 ;
    cmd->numBytes += 4;
}
...
static int MmstSendCommand(int* seqNum,int s, int command, unsigned int switches, unsigned int extra, int length, unsigned char *data)
{
    command_t  cmd;
    int        len8;
    int        ret;
    ...
    Put32 (&cmd, len8 + 4);       /* Length until end of packet in 8 byte boundary lengths，Including own data field */
    ...
    Put32 (&cmd, 0x0);		/* (8bytes) double decision time stamp(used for net timing) */
    Put32 (&cmd, len8+2);    /* Length until end of packet in 8 byte boundary lengths. Including own data field */
...
//*********************************************************************************************************************//
//*********************************************************************************************************************//
static unsigned int Get32 (unsigned char *cmd, int offset)
{
    unsigned int ret;
...
static int MmstGetHeader (aw_mms_inf_t* mmsStreamInf, int s, unsigned char *header)
{
    ...
    while(1) 
    {
        ...
        if(preHeader[4] == 0x02)   //* (packet td type = 0x02)asf header
        {
            ...
            headerLen += packetLen;
            if((header[headerLen-1] == 1) && (header[headerLen-2]==1))   //asf header end with 0x01 0x01
            {   
	            return headerLen;
            }
        }
...
static int MmstInterpHeader (aw_mms_inf_t* mmsStreamInf, unsigned char *header, int header_len)
{
    ...
    while (i<header_len) 
    {
        ...
        length = (uint64_t)header[i] | ((uint64_t)header[i+1]<<8)
                |((uint64_t)header[i+2]<<16) | ((uint64_t)header[i+3]<<24)
                |((uint64_t)header[i+4]<<32) | ((uint64_t)header[i+5]<<40)
                |((uint64_t)header[i+6]<<48) | ((uint64_t)header[i+7]<<56);   //* Object size

        i += 8;

        if((guid_1 == 0x6cce6200aa00d9a6ULL) && (guid_2 == 0x11cf668e75b22630ULL) )  /* ASF_HEADER_Object*/
        {
            //mp_msg(MSGT_NETWORK,MSGL_INFO,MSGTR_MPDEMUX_MMST_HeaderObject);
        } 
        else if ((guid_1 == 0x6cce6200aa00d9a6ULL) && (guid_2 == 0x11cf668e75b22636ULL)) /*ASF_Data_Object*/
        {
            //mp_msg(MSGT_NETWORK,MSGL_INFO,MSGTR_MPDEMUX_MMST_DataObject);
        }
        else if((guid_1 == 0x6553200cc000e48eULL) && (guid_2 == 0x11cfa9478cabdca1ULL)) /*ASF_File_Properties_Object*/
        {
            packetLength = Get32(header, i+92-24);  //*  Minimum Data Packet Size in ASF_File_Properties_Object
            //mp_msg(MSGT_NETWORK,MSGL_INFO,MSGTR_MPDEMUX_MMST_FileObjectPacketLen,packetLength, Get32(header, i+96-24));
        } 
        else if((guid_1 == 0x6553200cc000e68eULL) && (guid_2 == 0x11cfa9b7b7dc0791ULL)) /*ASF_Stream_Properties_Object*/
        {
            streamId = header[i+48] | header[i+49] << 8;   // Flag(bit0-6 is stream num)
            //mp_msg(MSGT_NETWORK,MSGL_INFO,MSGTR_MPDEMUX_MMST_StreamObjectStreamID, streamId);
...
int MmstStreamingStart(char *uri, aw_mms_inf_t* mmsStreamInf)
{
 ...
 /*
   ...
   * This command is sent at the very start of protocol initiation. It sends local information to the serve cmd 1 0x01
   * ********************************************************************************************
   */

    /* prepare for the url encoding conversion */
#ifdef CONFIG_ICONV
    //* convert byte sequences from UTF-8 to UTF-16LE
    url_conv = iconv_open("UTF-16LE", "UTF-8");  //* allocate descriptor for character set conversion
#endif
    /* NSPlayer/version；<spcae>{128bit client GUID}; <space>Host:<space>server IP address*/
    snprintf(str, 1023, "\034\003NSPlayer/7.0.0.1956; {33715801-BAB3-9D85-24E9-03B90328270A}; Host: %s", url->hostname);

    String2Utf16 (data,str, strlen(str)); /* string to utf16 */
    //send_command(s, commandno ....)
    //* first packet: client send to server (tell server the player version)
    //* fs is server socket
    //* void MmstSendCommand(int* seqNum,int s, int command, unsigned int switches, unsigned int extra, int length, unsigned char *data)
    ret = MmstSendCommand(&mmsStreamInf->seqNum, fs, 1, 0, 0x0004000b, strlen(str)*2+2, (unsigned char*)data);
    if(ret < 0)
    {
    ...
    len = MmsGetNetworkData(mmsStreamInf,fs, data, MMST_BUF_SIZE);
    if(len <= 0)
    {
        closesocket(fs);
        return -1;
  ...
  ********************************************************************************************************************
  */
    String2Utf16(&data[8], "\002\000\\\\192.168.0.1\\TCP\\1037", 24);
    memset (data, 0, 8);
    /* send the second packet ( client transport protocol, IP, port)*/
    MmstSendCommand(&mmsStreamInf->seqNum, fs, 2, 0, 0, 24*2+10, (unsigned char*)data);
  ...
  *****************************************************************************************************************
  */
    String2Utf16(&data[8], path, strlen(path));
    memset(data, 0, 8);
    MmstSendCommand(&mmsStreamInf->seqNum,fs, 5, 0, 0, strlen(path)*2+10, (unsigned char*)data);
    free(path);
    if(MmstGetAnswer(mmsStreamInf, fs)== -1)  //if the command in the server send packet is 0x1b(unkown command), deal it
    {
        closesocket(fs);
        return -1;
    }
  ...
  */
    ...
    MmstSendCommand(&mmsStreamInf->seqNum,fs, 0x15, 1, 0, 40, (unsigned char*)data);
    ...
    mmsStreamInf->packetDataLen = packetLength;  //* Minimum Data Packet Size in ASF_File_Properties_Object
  ...
  */
    ...
    if(audio_id > 0)
    {
        ...
        data[3] = 0xFF;
        data[4] = audio_id;
        MmstSendCommand(s, 0x33, num_stream_ids, 0xFFFF | audio_id << 16, 8, data);
    }
    else
 #endif
    {
        for(i=1; i<mmsStreamInf->numStreamIds; i++)
        {
            data [ (i-1) * 6 + 2 ] = 0xFF;
            data [ (i-1) * 6 + 3 ] = 0xFF;
            data [ (i-1) * 6 + 4 ] = mmsStreamInf->streamIds[i];
            data [ (i-1) * 6 + 5 ] = 0x00;
        }
        MmstSendCommand(&mmsStreamInf->seqNum, fs, 0x33, mmsStreamInf->numStreamIds, 0xFFFF|mmsStreamInf->streamIds[0]<<16,
                         (mmsStreamInf->numStreamIds-1)*6+2, (unsigned char*)data);
    }
  ...
  */
    ...
    data[20] = 0x04;
    MmstSendCommand(&mmsStreamInf->seqNum,fs, 0x07, 1, 0xFFFF | mmsStreamInf->streamIds[0] << 16, 24, (unsigned char*)data);
    len = MmsGetNetworkData(mmsStreamInf, fs,data, MMST_BUF_SIZE);
    if(len <= 0)
    {
        closesocket(fs);
...
static int MmstStreamingRead(aw_mms_inf_t* mmsStreamInf)
{
    ...
    if(preHeader[4] == 0x04)  // Data packet
    {
        ...
        while(mmsStreamInf->exitFlag == 0)
        {
            ...
            if(mmsStreamInf->packetDataLen > (mmsStreamInf->stream_buf_size-mmsStreamInf->validDataSize))
            {
                usleep(40*1000);
                continue;
            }
        ...
        if(mmsStreamInf->bufWritePtr <= mmsStreamInf->bufReleasePtr)
        {
            ...
            dataSize = GetData(mmsStreamInf, mmsStreamInf->fd,(unsigned char*)mmsStreamInf->bufWritePtr, packetLen);
        ...
        else
        {
            ...
            else
            {
                 ...
                 memcpy(mmsStreamInf->buffer, data+remainSize, mmsStreamInf->packetDataLen-remainSize);
            }
        }
...
int AsfStreamStart(char *uri, aw_mms_inf_t* mmsStreamInf)
{
    ...
    {
        //fd = aw_asf_mmsu_streaming_start(mmsStreamInf );  //mmsu support is not implemented yet - using this code
		if(fd > -1)
        {
            return fd;
		}
...
void* CdxReadAsfStream(void* p_arg)
{
	...
	while(1)
	{
		...
		if( (mmsStreamInf->stream_buf_size - mmsStreamInf->validDataSize) < 3*1024*1024)  /* buffer 剩余空间不足*/
		{
			usleep(40*1000);
			continue;
		}
	    ...
	    {
	    	if((asfHttpCtrl->streamingType==ASF_TYPE_PLAINTEXT) || (asfHttpCtrl->streamingType==ASF_TYPE_REDIRECTOR))
	        {   
