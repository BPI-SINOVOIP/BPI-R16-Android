    ...
    if (impl->pHttpHeader)
    {
        for (i=0; i < impl->nHttpHeaderSize; i++)
        {
            ...
            if(strcasecmp("Range", impl->pHttpHeader[i].key) == 0)//Range: bytes=%lld-%lld
            {
				...
				cdx_int64 bytes = 0;
				sscanf(impl->pHttpHeader[i].val, "bytes=%lld-",&bytes);
				logd("bytes = %lld, pos = %lld",bytes, pos);
				if(pos != 0)
				{
					char s[256] = {0};	
					sprintf(s, "bytes=%lld-", pos);
					Pfree(impl->pool,(void*)impl->pHttpHeader[i].val);
					impl->pHttpHeader[i].val = Pstrdup(impl->pool,s);
				}
            }
...
void ClearExtraDataContainer(CdxHttpStreamImplT *impl)
{
...
static cdx_int32 __CdxHttpStreamClose(CdxStreamT *stream)
{
    ...
    CdxAtomicDec(&impl->ref);
/* 
    while((ref = CdxAtomicRead(&impl->ref)) != 0) // may not need
    {
        //CDX_LOGV("xxx (ref = CdxAtomicRead(&impl->ref)) != 0(%ld)", ref);
        usleep(10*1000);
    }
*/  
    if(impl->buffer)
