...
static cdx_int32 __VideoResizeStreamRead(CdxStreamT *stream, cdx_void *buf, cdx_uint32 len)
{
    struct CdxVideoResizeStreamImplS *impl;
	cdx_int32 len1 = 0;
	cdx_int32 len2 = 0;
    ...
    if((impl->pReadPtr+len) >= impl->bufferEnd)
    {
    	...
    	memcpy((void*)((cdx_int32)buf+len1), (void*)impl->buffer, len2);

    	if(lock(impl) < 0)
    	{
    		CDX_LOGE("lock the impl error\n");
    	}
    	impl->pReadPtr = (unsigned char*)((cdx_int32)impl->buffer+len2);
...
static cdx_int32 __VideoResizeStreamControl(CdxStreamT *stream, cdx_int32 cmd, cdx_void *param)
{
    ...
    switch (cmd)
    {
    	case STREAM_CMD_REQUEST_WRITE_INFO:
    	    ...
    	    else
    		{
    	    	...
    	    	if((unsigned int)(impl->pWritePtr+bufInfo->bufLen)>= (unsigned int)impl->bufferEnd)
    	    	{
...
static CdxStreamT *__VideoResizeStreamCreate(CdxDataSourceT *source)
{
	...
	sscanf(impl->filePath,"%[^=]=%d", buf1, &len);
    bufAddr = malloc(len);
	if(bufAddr == NULL)
	{
