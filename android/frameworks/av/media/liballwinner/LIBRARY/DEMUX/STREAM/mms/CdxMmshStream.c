...
int CdxMmshAddBasicAuthentication(CdxHttpHeaderT *httpHdr, const char *username, const char *password)
{
	char *auth = NULL;
	char *usrPass = NULL;
...
const char asf_stream_header_guid[16] = {0x91, 0x07, 0xdc, 0xb7,  	/*ASF_Stream_Properties_Object*/
  0xb7, 0xa9, 0xcf, 0x11, 0x8e, 0xe6, 0x00, 0xc0, 0x0c, 0x20, 0x53, 0x65};
const char asf_file_header_guid[16] = {0xa1, 0xdc, 0xab, 0x8c,     /*ASF_File_Properties_Object */
  0x47, 0xa9, 0xcf, 0x11, 0x8e, 0xe4, 0x00, 0xc0, 0x0c, 0x20, 0x53, 0x65};
const char asf_stream_group_guid[16] = {0xce, 0x75, 0xf8, 0x7b,   /*ASF_Stream_Bitrate_Properties_Object*/
  0x8d, 0x46, 0xd1, 0x11, 0x8d, 0x82, 0x00, 0x60, 0x97, 0xc9, 0xa2, 0xb2};

//*********************************************************************************************************************//
//*********************************************************************************************************************//

CdxHttpHeaderT* MmshRequest(aw_mms_inf_t* mmsStreamInf,	unsigned int offsetHi, unsigned int offsetLo, int64_t pos) 
{   
    char str[250];
	char *ptr = NULL;
	int i = 0;
	...
	sprintf(str,"Pragma: no-cache,rate=1.000000,stream-time=0,stream-offset=%u:%u,request-context=%d,max-duration=%d",
...
static int MmshStreamingType(char *contentType, char *features, CdxHttpHeaderT *httpHdr) 
{
	...
	if( !strcasecmp(contentType, "application/octet-stream") ||
		!strcasecmp(contentType, "application/vnd.ms.wms-hdr.asfv1") ||        // New in Corona, first request
...
{   
    ...
    //* asfHttpDataBuffer中的数据时在解析asf header object的时候 recv的，需要先确认其中的数据是否已经copy到mmsStreamInf->buffer中
...
{   
    ...
    {   
	    ...
	    if(chunkSize2read==0) 
        {
		    ...
		    chunkSize2read = AV_RL64(&asfh->objh.size);  /* header object size*/
	   }
    ...
    pos = MmshFindAsfGuid(buffer, asf_file_header_guid, start, size);   /*ASF_File_Properties_Object*/
    if(pos >= 0) 
    {
        ...
        mmsStreamInf->fileSize  = AV_RL64(&fileh->fileSize);
        mmsStreamInf->fileDuration = (AV_RL64(&fileh->playDuration)/10000);
        // before playing.
        // preroll: time in ms to bufferize before playing
       // mmsStreamInf->prebufferSize = (unsigned int)(((unsigned int)fileh->preroll/1000)*((unsigned int)fileh->maxBitrate/8));
       // mmsStreamInf->prebufferSize = (unsigned int)(((double)fileh->preroll/1000.0)*((double)fileh->maxBitrate/8.0));
    }
    pos = start;
    while((pos = MmshFindAsfGuid(buffer, asf_stream_header_guid, pos, size)) >= 0) /*ASF_Stream_Properties_Object GUID and object size*/
    {
        ...
        switch(ASF_LOAD_GUID_PREFIX(streamh->type)) 
        {
        	...
        	case 0xF8699E40 : 	// ASF_Audio_Media  audio stream
                ...
                {
                    asfCtrl->nAudio++;
	                asfCtrl->audioStreams = realloc(asfCtrl->audioStreams,asfCtrl->nAudio*sizeof(int));
	            }
           ...
           case 0xBC19EFC0 : // ASF_Vedio_Media   video stream
                ...
                {
	                asfCtrl->nVideo++;
	                asfCtrl->videoStreams = realloc(asfCtrl->videoStreams,asfCtrl->nVideo*sizeof(int));
	            }
     ...
     pos = MmshFindAsfGuid(buffer, asf_stream_group_guid, start, size); /*ASF_Stream_Bitrate_Properties_Object*/
     if (pos >= 0)
     {
...
int MmshStreamingStart(char *uri, aw_mms_inf_t* mmsStreamInf)
{
	...
	do
    {
        ...
        do
        {
            i = MmsGetNetworkData(mmsStreamInf, mmsStreamInf->sockFd, buffer,HTTP_BUFFER_SIZE); //recv 2K data every time
            if(i<=0)
            {
                CDX_LOGD("*****MmsGetNetworkData error!");
                goto err_out;
            }
		    CdxMmshResponseAppend(httpHdr, buffer, i); // copy the data from buffer to httpHdr->buffer
...
{   
    ...
    {    
        ...
        {
            CDX_LOGD("--- CDX_IO_STATE_CLEAR, buf_pos = %llx", (long long int)mmsStreamInf->buf_pos);
        ...
        {   
            ...
            {
                CDX_LOGD("**streamTYpe:%d, asfHttpCtrl->packetSize<%d> is smaller than the chunkSize<%d>, error.", ret, asfHttpCtrl->packetSize, chunkSize);
                return -1;
	        }
        ...
        {   
            ...
            {
                CDX_LOGD("******asfHttpCtrl->packetSize:%d chunkSize:%d > 32*1024.", asfHttpCtrl->packetSize, chunkSize);
                return -1;
            }
            ...
            else
            {
                ...
                memcpy(mmsStreamInf->buffer,mmsStreamInf->asfChunkDataBuffer+remainDataSize, wantSize-remainDataSize);
            }
    ...
    { 
        pos = MmshFindAsfGuid(headerBuffptr, asf_file_header_guid, 0, chunkSize);
        if(pos >= 0) 
        {
...
int MmshStreamingReadFunc(aw_mms_inf_t* mmsStreamInf, int wantSize)
{
    ...
    readLen = MmshStreamingRead( mmsStreamInf->sockFd,mmsStreamInf->bufWritePtr,remainDataSize,mmsStreamInf);
    if(readLen < 0)
    {   
        return STREAM_READ_END;
    }
...
int MmshNopStreamingReadFunc(aw_mms_inf_t* mmsStreamInf, int wantSize)
{
    ...
    readLen = NopStreamingRead(mmsStreamInf->fd, mmsStreamInf->bufWritePtr, remainDataSize, mmsStreamInf);
...
{   
    ...
    httpHdr = MmshRequest(mmsStreamInf,(mmsStreamInf->buf_pos>>32)&0xffffffff, mmsStreamInf->buf_pos&0xffffffff, 0);
    for(i=0; i < (int)httpHdr->bufferSize; ) 
    {   
        r = send(fd, httpHdr->buffer+i, httpHdr->bufferSize-i, DEFAULT_SEND_FLAGS);
		if(r <0) 
    ...
    {   
        ...
        if(chunk.type == ASF_STREAMING_HEADER)
        {  
            pos = MmshFindAsfGuid(bufPtr, asf_file_header_guid, 0, chunkSize);
            if(pos >= 0) 
            {
