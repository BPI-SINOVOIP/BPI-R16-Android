...
int AesStreamDecrypt(CdxAesStream *aesStream, cdx_uint8 *in, cdx_uint8 *out, cdx_uint32 size, int needRemove)
{
    CDX_CHECK(size);
    if((aesStream->downloadComplete || CdxStreamEos(aesStream->mediaFile)) && !size%16 && needRemove)
    {
        int size1 = size - 16;
        if(size1)
        {
...
static cdx_int32 AesStreamRead(CdxStreamT *stream, void *buf, cdx_uint32 size)
{
    ...
    while(aesReadLen >= 16)
    {
        ...
        //此时len2一定非0，否则len不会小于readLen
            ...
            //removeSize = AesStreamDecrypt(aesStream, aesStream->bigBuf + len1, aesStream->remaining + 16 - (readLen - len1), readLen - len1, 1);
            removeSize = AesStreamDecrypt(aesStream, aesStream->bigBuf + len1, aesStream->remaining, readLen - len1, 1);
...
static cdx_int32 AesStreamSeek(CdxStreamT *stream, cdx_int64 offset, cdx_int32 whence)
{
    ...
    if(remainder)
    {
        ...
        int removeSize = AesStreamDecrypt(aesStream, aesStream->bigBuf, aesStream->remaining, ret, 1);
...
static cdx_int32 AesStreamConnect(CdxStreamT *stream)
{
    ...
    memcpy(aesStream->iv, aesStream->originalIv, 16);
    AesStreamDecrypt(aesStream, (cdx_uint8 *)probeData->buf, (cdx_uint8 *)aesStream->probeData.buf, probeData->len, 0);
    memcpy(aesStream->iv, aesStream->originalIv, 16);
    aesStream->ioState = CDX_IO_STATE_OK;
	pthread_mutex_lock(&aesStream->lock);
    aesStream->status = STREAM_IDLE;
