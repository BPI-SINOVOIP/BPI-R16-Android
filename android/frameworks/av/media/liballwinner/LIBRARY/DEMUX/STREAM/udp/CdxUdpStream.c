...
int UdpSocketCreate(CdxUdpStream *udpStream, CdxUrlT *url)
{
    ...
    memcpy((cdx_uint8 *)&serverAddress.sin_addr.s_addr, host->h_addr_list[0], host->h_length);
    ...
    if((ntohl(serverAddress.sin_addr.s_addr) >> 28) == 0xe)
    {
        struct ip_mreq *multicast = &udpStream->multicast;
        multicast->imr_multiaddr.s_addr = serverAddress.sin_addr.s_addr;
    ...
    if(udpStream->isMulticast)
    {
        ...
        {
            CDX_LOGE("bind fail, errno = %d, Address = %x", errno, (int)serverAddress.sin_addr.s_addr);
            goto _err;
        }
    }
    else
    {
        ...
        {
            CDX_LOGE("connect fail, errno = %d, Address = %x", errno, (int)serverAddress.sin_addr.s_addr);
            if(sendto(socketFd, "hello", 5, 0, (struct sockaddr *)&serverAddress, sizeof(serverAddress)) == -1)
            {
                CDX_LOGE("sendto fail, errno = %d, Address = %x", errno, (int)serverAddress.sin_addr.s_addr);
                goto _err;
            }
        }
    }
...
void *UdpDownloadThread(void* parameter)
{
    ...
    while(1)
    {
        ...
        CDX_LOGD("2UdpDownloadData, downloadSize = %u", downloadSize);
