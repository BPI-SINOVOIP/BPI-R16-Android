...
#define AV_RB16(x)  ((((const CDX_U8*)(x))[0] << 8) | ((const CDX_U8*)(x))[1])
#define AV_WB16(p, d) do { \
                    ((CDX_U8*)(p))[1] = (d); \
                    ((CDX_U8*)(p))[0] = (d)>>8; } while(0)

#define AV_RL16(x)  ((((const unsigned char*)(x))[1] << 8) | \
                      ((const unsigned char*)(x))[0])
#define AV_WL16(p, d) do { \
                    ((CDX_U8*)(p))[0] = (d); \
                    ((CDX_U8*)(p))[1] = (d)>>8; } while(0)
...
//	CdxDataSourceT*     dataSource;
	...
	int 				fd;                   //* Connect to a server using a TCP connection
											  ...
											  // if success , return socket_server_fd(sockFd)
	...
	int64_t 			dmx_read_pos;   	  //* demuxer parses the stream to this file pos.the return value of Tell()

	int					stream_buf_size;      //*cache size, set in StreamOPen(),

	CdxUrlT* 			awUrl;         		  //* url

	char* 				buffer; 		      //* client buffer, store stream data from network and  demux read data from it
	...
	int 				seqNum;               //* client send packet to server need the sequence number

	int 				exitFlag;
	int                 closeFlag;

	/* the stream num of ASF_Stream_Properties_Object(the unique number of vedio or audio and so on)
	...
	int 				packetDataLen;        //* Minimum Data Packet Size in ASF_File_Properties_Object,actually is the packet size
	...
	int64_t 			fileSize;						//* entire size of the ASF file, equal to filesize in aw_asf_file_header_t(in ASF_File_Properties_Object)
	int64_t 			fileDuration;					//* entire play time of the ASF file( in ASF_File_Properties_Object)

	char*				firstChunk;						//* the first chunk data(chunk header and body),used for parse asf_header_object in mms_http_streaming_start
...
typedef struct
{
	...
	int					packetSize;				//* ASF every packet size, equal to the maxPacketSize in aw_asf_file_header_t
	int*				audioStreams;			//* every audio stream have diffreant audio number  (不同音轨对应ASF_Stream_Properties_Object中不同的stream num)
	int 				nAudio;					//* number of audio stream (音轨数)
	int*				videoStreams;
	int 				nVideo;
	int 				audioId;                //* chose the proper audio bumber according to bandwith
	int 				videoId;				//* chose the proper vedio bumber according to bandwith
} aw_mmsh_ctrl_t;
...
typedef struct
{
	...
	unsigned int 			flags;   			// Unknown, maybe flags ( usually contains 2 ) UINT32 4
	unsigned int 			minPacketSize; 		// Min size of the packet, in bytes UINT32 4
	unsigned int 			maxPacketSize; 		// Max size of the packet(equal to minPacketSize usually )  UINT32 4
	unsigned int 			maxBitrate; 		// Maximum bitrate of the media (sum of all the stream)
...
typedef struct
{
	...
	unsigned long long 		unk1; 				// Time offset ,Unknown, maybe reserved ( usually contains 0 ) UINT64 8
