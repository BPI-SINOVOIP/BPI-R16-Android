...
#define TABLE_INIT( count, tab )                \
    do {                                        \
        (count) = 0;                            \
        (tab) = NULL;                           \
    } while(0)

#define TABLE_APPEND_CAST( cast, count, tab, p )             \
      do {                                          \
        if( (count) > 0 )                           \
...
void RtspUrlParse (RtspUrlT *url, const char *str, unsigned char opt)
{
    url->pProtocol = NULL;
    url->pUsername = NULL;
    url->pPassword = NULL;
                        ...
                        unsigned int duration )
{
    ...
    if( tk->bQuicktime && tk->pEs == NULL )
    {
        ...
        unsigned char *sdAtom = (unsigned char*)&qtState.sdAtom[4];

        /* Get codec informations from the quicktime atoms :
         * http://developer.apple.com/quicktime/icefloe/dispatch026.html */
        if( tk->fmt.iCat == VIDEO_ES ) 
        {
            ...
            tk->fmt.iCodec = VLC_FOURCC(sdAtom[0],sdAtom[1],sdAtom[2],sdAtom[3]);
            tk->fmt.video.i_width  = (sdAtom[28] << 8) | sdAtom[29];
            tk->fmt.video.i_height = (sdAtom[30] << 8) | sdAtom[31];

            if( tk->fmt.iCodec == RTSP_FOURCC('a', 'v', 'c', '1') )
            {
    ...
    if( impl->mCurptk.pktData )
    {
        if( !tk->bMuxed && !tk->bAsf )
        {
            ...
            //p_block->i_dts = ( tk->fmt.iCodec == VLC_CODEC_MPGV ) ? VLC_TS_INVALID : (VLC_TS_0 + i_pts);
...
static int GetOneFrame(CdxRtspStreamImplT *impl)
{
    ...
    //task = pSys->scheduler->scheduleDelayedTask( 300000, CdxTaskInterruptData, impl );
        ...
        ( pSys->iNodatati > 120 ) )
    {
              ...
              pSys->bNoData && ( pSys->iNodatati > 34 ) )
    {
    ...
    if(pSys->rtsp)
    {
        ...
        if( !pSys->pTimeout && ( pSys->bGetparam /*"rtsp-wmserver"*/ ) )
        {
            ...
            if( pSys->pTimeout )
            {
                ...
                #if 0
                if( xxx_clone( &pSys->p_timeout->handle,  TimeoutPrevention,
                               pSys->p_timeout, VLC_THREAD_PRIORITY_LOW ) )
                {
...
static int __CdxRtspStreamClose(CdxStreamT *stream)
{
    ...
    if(pSys)
    {
        ...
        {
            //pthread_cancel(pSys->pTimeout->handle); 
