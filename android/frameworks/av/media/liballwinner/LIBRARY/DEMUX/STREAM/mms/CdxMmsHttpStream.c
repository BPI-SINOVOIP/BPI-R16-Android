...
extern const char asf_stream_header_guid[16];	/*ASF_Stream_Properties_Object*/
extern const char asf_file_header_guid[16];     /*ASF_File_Properties_Object */
extern const char asf_stream_group_guid[16];  /*ASF_Stream_Bitrate_Properties_Object*/
...
int aw_mms_http_streaming_start(CdxDataSourceT* datasource, aw_mms_inf_t* mmsStreamInf)
{
    ...
    int ret = CdxStreamOpen(datasource, NULL, &mmsStreamInf->exitFlag, (CdxStreamT**)&mmsStreamInf->httpCore, NULL);
    if(ret < 0)
    {
	...
	num = CdxStreamRead(mmsStreamInf->httpCore, mmsStreamInf->firstChunk+12, mmsStreamInf->firstChunkBodySize-12);
    if(num < 0)
    {
    ...
    pos = MmshFindAsfGuid(mmsStreamInf->firstChunk, asf_file_header_guid, start, mmsStreamInf->firstChunkBodySize); /*ASF_File_Properties_Object GUID and object size*/
    if(pos >= 0)
    {
    ...
    pos = start;
    while((pos = MmshFindAsfGuid(mmsStreamInf->firstChunk, asf_stream_header_guid, pos, mmsStreamInf->firstChunkBodySize)) >= 0) /*ASF_Stream_Properties_Object GUID and object size*/
    {
     ...
     pos = MmshFindAsfGuid(mmsStreamInf->firstChunk, asf_stream_group_guid, start, mmsStreamInf->firstChunkBodySize); /*ASF_Stream_Bitrate_Properties_Object*/
     if (pos >= 0)
     {
...
int aw_mms_http_streaming_read(char *buffer, int size, aw_mms_inf_t* mmsStreamInf)
{
    ...
    while(1)
    {
        ...
        if(chunk.type != ASF_STREAMING_HEADER && (!dropChunk))
        {
            ...
            {
                printf("***********asfHttpCtrl->packetSize is smaller than the chunkSize, error.\n");
                return -1;
	        }
	        wantSize = asfHttpCtrl->packetSize;  //the size of every data packet is the same (asfHttpCtrl->packetSize)
        }
        ...
        if(size < wantSize)
        {
            ...
            if(aw_asf_http_read_wrapper(mmsStreamInf->asfChunkDataBuffer, chunkSize, mmsStreamInf) <= 0)
            {
                return -1;
            }
            if(dropChunk == 1)
...
int aw_mms_http_streaming_seek(aw_mms_inf_t* mmsStreamInf)
{
	...
	struct HttpHeaderField hdr[7] = {{"Accept","*/*"},{"User-Agent","NSPlayer/4.1.0.3856"},{"Pragma","xClientGUID={c77e7400-738a-11d2-9add-0020af0a3278}"},
									{"Pragma","no-cache,rate=1.000000,stream-time=0,stream-offset=0:0,request-context=2,max-duration=0"},{"Pragma"," xPlayStrm=1"},
									{"Pragma","stream-switch-entry=ffff:1:0 ffff:2:0"},{"Pragma","stream-switch-count=2"}};
