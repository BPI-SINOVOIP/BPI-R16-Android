                        ...
                        cdx_long timeoutUs, cdx_int32 *pForceStop)
{
    ...
    if (CdxSockIsBlocking(sockfd))
    {
        while(1)
        {
            ...
            else if(ret == 0)//The TLS/SSL handshake was not successful but was shut down controlled and by the specifications of the TLS/SSL protocol. 
                        ...
                        cdx_long timeoutUs, cdx_int32 *pForceStop)
{
    ...
    if (CdxSockIsBlocking(sockfd))
    {
        while(1)
        {
            ...
            {
                CDX_LOGD("xxx recvSize(%ld),sockfd(%d), want to read(%lu), errno(%d), shut down by peer?",
    ...
    for (i = 0; i < loopTimes; i++)
    {
        ...
        while (1)
        {
            ...
            if (ret < 0)
            {
                ...
                {
                    CDX_LOGE("ret(%ld), read error, %s", ret, ERR_error_string(ERR_get_error(), NULL));
    ...
    if (CdxSockIsBlocking(sockfd))
    {
        while(1)
        {
            ...
            if (ret < 0)
            {
                ...
                {
                    CDX_LOGE("ret(%ld), write error, %s", ret, ERR_error_string(ERR_get_error(), NULL));
                    return -1;
                }
            }
            else if(ret == 0)
            {
                CDX_LOGD("xxx sendSize(%ld),sockfd(%d), want to read(%lu), errno(%d), shut down by peer?",
...
static cdx_int32 CdxSSLStreamForceStop(CdxStreamT *stream)
{
    ...
    pthread_mutex_unlock(&impl->stateLock);
    while(((ref = CdxAtomicRead(&impl->state)) != SSL_STREAM_IDLE) && ((ref = CdxAtomicRead(&impl->state)) != SSL_STREAM_CONNECTING))
    {
...
static void *StartSSLStreamThread(void *pArg)
{
    ...
    if(ret != 0)
    {
        CDX_LOGE("get host failed, host:%s, port:%s, err:%s", impl->hostname, tempPort, gai_strerror(ret));
        goto err_out;
    }
    ...
    do
    {
        ret = CdxSockAsynConnect(impl->sockFd, res->ai_addr, res->ai_addrlen, 0, &impl->forceStopFlag);
        if(ret == 0)
        {
            break;
        }
    ...
    impl->ctx = SSL_CTX_new(TLSv1_client_method());// downward compatibility? SSLv3_client_method()/SSLv23_client_method()
    if(impl->ctx == NULL)
    {
        CDX_LOGE("xxx %s", ERR_error_string(ERR_get_error(), NULL));
        //ERR_print_errors_fp(stderr);
    ...
    ret = CdxSSLConnect(impl->sockFd,impl->ssl, 0, &impl->forceStopFlag); // todo, non-blocking...
    if(ret < 0)
    {
