...
static cdx_void FreeMediaStreams(AwSstrParserImplT *impl)
{
    cdx_int32 i;
                           ...
                           const cdx_uint64 startTime)
{
    ...
    qual = aw_strtok_r(urlTemplate, "{", &saveptr);
    aw_strtok_r(NULL, "}", &saveptr);
    frag = aw_strtok_r(NULL, "{", &saveptr); //* frag=bitrate
    aw_strtok_r(NULL, "}", &saveptr);
    end = aw_strtok_r(NULL, "", &saveptr);
    char *url = NULL;
...
cdx_int32 SmsOpenSegment(AwSstrParserImplT *impl, SmsStreamT *sms)
{
    ...
    QualityLevelT *qlevel = GetQlevel(sms, sms->downloadQlvl); //* sms->downloadQlvl should change by bandwidth... todo
    if(!qlevel)
    {
    ...
    chunk->type = sms->type;
    char *url = SmsConstructUrl(sms->urlTemplate, impl->ismc->baseUrl, qlevel->Bitrate, chunk->startTime);
    if(!url)
    {
    ...
    CdxStreamT *stream;
    ret = CdxStreamOpen(impl->mediaStream[EsCatToIndex(impl->prefetchType)].datasource, &impl->statusMutex, 
                                &impl->exitFlag, &stream, NULL);
    if(ret < 0)
    {
    ...
    ret = CdxParserControl(impl->mediaStream[EsCatToIndex(impl->prefetchType)].parser, CDX_PSR_CMD_REPLACE_STREAM, (void *)stream);
...
cdx_int32 SmsOpenFirstSegment(AwSstrParserImplT *impl, cdx_int32 index)
{
    ...
    startTime = impl->ismc->download.lead[index];  //* 下载完片时 需要更新download.lead, 作为下一片的start time
...
//* h264, construct codec specific data. 000000016742C00D96640988DFF8C800C458800001F480005DC0078A15500000000168CE32C8 => 
//* [0]: 0x01 [1][2][3]: 0x00 00 00
//* [4]: 0x03 [5]: 0x01 [6]: spsLen high 8 bits [7]:spsLen low 8 bits [8, 7+spsLen]:sps data
//* [8+spsLen]:0x01 [9+spsLen]: ppsLen high 8 bits [10+spsLen]: ppsLen low 8 bits [11+spsLen, spsLen+ppsLen+10]: pps data
static cdx_int32 ConstructCodecSpecificDataForH264(AwSstrParserImplT *impl)
{
    ...
    for(i=0; (i+4)<=nAnalyseSize; i++)
    {
        ...
        else if(tmpBuf0[i] == 0x00 && tmpBuf0[i+1] == 0x00 && tmpBuf0[i+2] == 0x00 && tmpBuf0[i+3] == 0x01)
        {
            startCodeNum0 = 4;
            index0 = i;
            break;
...
static cdx_int32 SstrSetMediaInfo(AwSstrParserImplT *impl)
{
    ...
    if(impl->ismc->hasVideo)
    {
        ...
        CDX_LOGV("qlevel->id=%d, qlevel->CodecPrivateData=[%s]", qlevel->id, qlevel->CodecPrivateData);
    ...
    if(impl->ismc->hasAudio)
    {
        ...
        switch(fourCC)
        {
            case SSTR_FOURCC('A','A','C','L'):
            {
                ...
                CDX_LOGD("audio codec:[%d] AUDIO_CODEC_FORMAT_MPEG_AAC_LC", AUDIO_CODEC_FORMAT_MPEG_AAC_LC);
                break;
            }
            ...
            case SSTR_FOURCC('W','M','A','P'):
            {
                pMediaInfo->program[0].audio[0].eCodecFormat = AUDIO_CODEC_FORMAT_WMA_STANDARD; //*AUDIO_CODEC_FORMAT_WMA_STANDARD or AUDIO_CODEC_FORMAT_WMA_PRO ?
                CDX_LOGD("audio codec:[%d] AUDIO_CODEC_FORMAT_WMA_STANDARD", AUDIO_CODEC_FORMAT_WMA_STANDARD);
                break;
            }
            default:
            {
        ...
        {
            pMediaInfo->program[0].audio[0].nCodecSpecificDataLen = strlen(qlevel->CodecPrivateData) / 2;
            pMediaInfo->program[0].audio[0].pCodecSpecificData = (char *)impl->codecSpecDataA;
        }
        
        CDX_LOGV("audio nChannelNum =%u, nBitsPerSample=%d, nSampleRate=%d, codecdatalen=%d", qlevel->Channels, qlevel->BitsPerSample,
            qlevel->SamplingRate, pMediaInfo->program[0].audio[0].nCodecSpecificDataLen);
        CDX_LOGV("audio codecdata[0][1]-[last]:%x %x %x", impl->codecSpecDataA[0], impl->codecSpecDataA[1], impl->codecSpecDataA[strlen(qlevel->CodecPrivateData) / 2 -1]);    
    ...
    CDX_LOGD("SMS videoNum(%d), audioNum(%d), subtitleNum(%d)", pMediaInfo->program[0].videoNum, pMediaInfo->program[0].audioNum,pMediaInfo->program[0].subtitleNum);

    if(impl->ismc->vodDuration)
    {
        pMediaInfo->bSeekable = 1;
        pMediaInfo->program[0].duration = impl->ismc->vodDuration / impl->ismc->timeScale * 1000; //* ms
    }
    else
    {
        CDX_LOGV("not vod.");
...
static int __AwSstrParserInit(CdxParserT *parser)
{
    ...
    if(!impl->ismc->smsStreams || !impl->ismc->selectedSt || !impl->ismc->download.chunks || !impl->ismc->initChunks)
    {
...
static cdx_int32 __AwSstrParserPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    //CDX_LOGD("vpts:%lld, apts:%lld, spts:%lld, now prefetch type:%d", impl->streamPts[0], impl->streamPts[1], impl->streamPts[2], impl->prefetchType);
    ...
    if(ret < 0)
    {
        ...
        if(mErrno == PSR_EOS)
        {
            ...
            //CDX_LOGD("xxx impl->ismc->download.lead[%d]=%llu", index, impl->ismc->download.lead[index]);
...
NEXT_SEGMENT:
            ...
            if (CdxParserControl(impl->mediaStream[EsCatToIndex(impl->prefetchType)].parser, CDX_PSR_CMD_GET_CACHESTATE, &cacheState) < 0)
            {
            ...
            CDX_LOGD("newQlevelId=%u, newQlevel->Bitrate=%u, cacheState.nBandwidthKbps=%d kbps", newQlevelId, newQlevel->Bitrate, cacheState.nBandwidthKbps);
            QualityLevelT *qlevel = GetQlevel(sms, sms->downloadQlvl);
            if(!qlevel)
            {
            ...
            if(newQlevel->Bitrate != qlevel->Bitrate)
            {
                ...
                pkt->length = nExtraDataLen + 10; //* for h264, startcode may 3 bytes(00 00 01), while length 4 bytes.
                pkt->pts = -1;
...
#endif
                CDX_LOGD("cur estimate bitrate: %d, new qlevel->bitrate: %u, qlevel->Bitrate: %u", cacheState.nBandwidthKbps*1000, newQlevel->Bitrate, qlevel->Bitrate);
//                return 0;
            }
#endif
    ...
    //CDX_LOGD("xxx pts=%lld, index=%d, pkt->type=%d", pkt->pts, EsCatToIndex(impl->prefetchType), pkt->type);
...
static cdx_int32 __AwSstrParserRead(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    if(impl->bAddExtradata) 
    {
        ...
        if(impl->mediaInfo.program[0].video[0].eCodecFormat == VIDEO_CODEC_FORMAT_WMV3) //* for vc1, bandwidth switch, first packet should add extradata.
        {
            if(pkt->length <= pkt->buflen)
            {
                memcpy(pkt->buf, impl->mediaInfo.program[0].video[0].pCodecSpecificData, pkt->length);
            }
            else
            {
                memcpy(pkt->buf, impl->mediaInfo.program[0].video[0].pCodecSpecificData, pkt->buflen);
                memcpy(pkt->ringBuf, impl->mediaInfo.program[0].video[0].pCodecSpecificData+pkt->buflen, pkt->length - pkt->buflen);
            }
        }
        ...
        memcpy(tempBuf, impl->mediaInfo.program[0].video[0].pCodecSpecificData, impl->mediaInfo.program[0].video[0].nCodecSpecificDataLen);
...
#endif
    //CDX_LOGD("sstr read,type=%d, pkt buflen=%d, pkt->duration=%lld, pkt->length=%d, pkt->pts=%lld", pkt->type, pkt->buflen, pkt->duration, pkt->length, pkt->pts);
...
cdx_int32 SstrParserSeekTo(AwSstrParserImplT *impl, cdx_int64 timeMs, cdx_int32 index)
{
    ...
    //CDX_LOGD("want to seek to timeMs=%lld, seek to %lld ms, index=%d", timeMs, impl->streamPts[index] / 1000, index);

#if 1
	ret = CdxParserSeekTo(impl->mediaStream[index].parser, chunk->startTime / impl->ismc->timeScale * 1000000);
	if(ret < 0)
	{
...
static cdx_int32 __AwSstrParserSeekTo(CdxParserT *parser, cdx_int64 timeUs)
{
    cdx_int32 ret;
    AwSstrParserImplT *impl;
    ...
    if(impl->ismc->hasVideo)
    {
        ret = SstrParserSeekTo(impl, timeMs, 0);
        if(ret < 0)
        {
    ...
    if(impl->ismc->hasAudio)
    {
        ret = SstrParserSeekTo(impl, timeMs, 1);
        if(ret < 0)
        {
    ...
    if(impl->ismc->hasSubtitle)
    {
        ret = SstrParserSeekTo(impl, timeMs, 2);
        if(ret < 0)
        {
...
static CdxParserT *__AwSstrParserCreate(CdxStreamT *stream, cdx_uint32 flags)
{
    ...
    //* example: http://mediadl.microsoft.com/mediadl/iisnet/smoothmedia/Experience/BigBuckBunny_720p.ism/Manifest
    char *ismcUrl;
    char *pos;
...
//* UTF-16LE / UTF-16BE
static cdx_uint8 probeNeed[][42] = {{0x3C, 0x00, 0x53, 0x00, 0x6D, 0x00, 0x6F, 0x00, 0x6F, 0x00, 0x74, 0x00, 0x68, 0x00, 
...
static cdx_uint32 __AwSstrParserProbe(CdxStreamProbeDataT *probeData)
{
    ...
    memcpy(probe, probeData->buf, 512);
    probe[511] = probe[512] = '\0';
	...
	{
		if (memmem(probe, 512, probeNeed[i], 42))
        {
            ret =  CDX_TRUE;
