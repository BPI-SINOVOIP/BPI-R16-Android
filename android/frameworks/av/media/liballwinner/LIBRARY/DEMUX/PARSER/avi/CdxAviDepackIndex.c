...
cdx_int16 AviGetChunkHeader(AviFileInT *aviFile, enum AVI_CHUNK_TYPE ckType)
{
    ...
    if(chunk->fcc == listheaderLIST || chunk->fcc == ckidAVINEWINDEX)
    {
...
cdx_int16 AviReadByIndex(CdxAviParserImplT *p)
{
    ...
    else
    {
        ...
        switch(chunkTypeRet)
        {
            case CHUNK_TYPE_VIDEO:
            {
                aviIn->dataChunk.fcc = MAKEAVICKID(cktypeDIBbits, p->videoStreamIndex);
                break;
            }
            case CHUNK_TYPE_AUDIO:
            {
                aviIn->dataChunk.fcc = MAKEAVICKID(cktypeWAVEbytes, p->audioStreamIndex);
                break;
            }
            default:
            {
