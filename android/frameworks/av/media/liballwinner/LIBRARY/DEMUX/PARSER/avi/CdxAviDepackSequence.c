...
cdx_int16 AviBuildIdxForIdx1SequenceMode(CdxAviParserImplT *p)
{
    ...
    for(i = 0; i < aviIn->idx1Total; i++)
    {
        ...
        if(id == p->videoStreamIndex)
        {
            ...
            if(mIdx->dwFlags & AVIIF_KEYFRAME)
            {
        ...
        else if(mIdx->ckid == listtypeAVIRECORD)
        {
            CDX_LOGV("meet [rec ]");
            nIdx1RecNum++;
        }
...
check_next_chunk_head:
    ...
    if(chunk->fcc == ckidAVINEWINDEX && !aviFile->hasIndx)    //AVI1.0 idx1 
    {   
    ...
    else if(aviFile->hasIndx) //AVI2.0 
    {
        ...
        else if(chunk->fcc == ckidAVINEWINDEX || IsIndxChunkId(chunk->fcc))
        {
