...
{   
    ...
    while(curRedLen >= 4)
    {
       ...
       {   
          ...
          else if((startCode&0xf0) == VIDEO_ID)
          {   
              if((*fstSysPos) != 0)
              {
                  curCheckBuf +=4;
          ...
          else if((startCode==PADDING_STREAM)||(startCode==PRIVATE_STREAM_2)||(startCode==PRIVATE_STREAM_1) ||(startCode==PADDING_STREAM)
            ||(startCode==PRIVATE_STREAM_2)||((startCode>=0x01c0)&&(startCode<=0x01df)))
          {  
              bufOffset = (curCheckBuf[4]<<8) | curCheckBuf[5];
	          bufOffset += 6;
    ...
    while(curRedLen >= 4)
    {
        ...
        else if(startCode == SEQUENCE_HEADER_CODE)
        {
            curCheckBuf = newCheckBuf;
            curCheckBuf += 7;
...
void CorrectAudioStreamId(CdxMpgParserT *MpgParser, cdx_uint8 audIdx)
{
     ...
     {
       if(dvdIfo->audioIfo.audioErrorPackId[i]==PRIVATE_STREAM_1)
       {
...
{    
     ...
     for(i=0; i<dvdIfo->audioIfo.audioWrongNum; i++) 
     {
       if(dvdIfo->audioIfo.audioErrorPackId[i]==PRIVATE_STREAM_2)
       {
...
new_parse_data:
   ...
   do
   {
      ...
      else
      {
         ...
         if((nextByte == SYSTEM_HEADER_START_CODE)&&(*fstSysPos==0))
        {
        ...
        else if((nextByte==PRIVATE_STREAM_1)||((nextByte>=0x01c0)&&(nextByte<=0x01df)))
         {
   ...
   for(i=0; i<dvdIfo->audioIfo.audioNum; i++)
   {
      if(dvdIfo->audioIfo.audioCheckFlag[i] == 0)
      {   
          if(dvdIfo->audioIfo.audioPackId[i] == PRIVATE_STREAM_1)
          {
