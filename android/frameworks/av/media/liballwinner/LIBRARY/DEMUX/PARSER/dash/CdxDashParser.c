...
/*************************************************************************/
static int CdxUrlGetType(const char* pathName)
{
	char* begin;
	
	if(!pathName) return CDX_URL_TYPE_OTHER;
	...
	if (!strncasecmp(pathName, "file", 4)) return CDX_URL_TYPE_ABSOLUTE;
	return CDX_URL_TYPE_OTHER;
}

/*************************************************************************/
// merge the parentName and pathName to get the absolute url
/*************************************************************************/
char* CdxUrlConcatenate(const char* parentName, const char* pathName)
{
	int proto_type;
	int i, pathSepCount;
	char* outPath;
	...
	if(!parentName && !pathName) return NULL;
	if(!pathName) return strdup(parentName);
	if(!parentName) return strdup(pathName);

	if((strlen(parentName)>CDX_DASH_MAX_PATH) || (strlen(pathName)>CDX_DASH_MAX_PATH))
	{
	    CDX_LOGW(" the length of url longer than 200, not support");
		return NULL;
	}

	proto_type = CdxUrlGetType(pathName);
	if(proto_type != CDX_URL_TYPE_RELATIVE)
	{
		char* sep = NULL;
		//if the pathName is not relative url and parentName is absolute url, 
		//add the pathName to the end of parentName
		...
		if(sep)
		{
			...
			outPath = (char*)malloc(len+1+strlen(pathName));
	...
	if (pathName[0] == '.') 
	{
		...
		for (i = 0; i< (int)strlen(pathName) - 2; i++) 
		{
			/*current dir*/
			...
			/*parent dir*/
			if ( (pathName[i] == '.') && (pathName[i+1] == '.') && ( (pathName[i+2] == '\\') || (pathName[i+2] == '/') ))
			{
				pathSepCount ++;
				i+=2;
	...
	for (i = strlen(parentName); i > 0; i--) 
	{
		...
		if ((parentName[i-1] == '\\') || (parentName[i-1] == '/')) 
		{
			tmp[i-1] = 0;
	...
	for (i = 0; i<(int)strlen(outPath); i++) 
...
check_spaces:
	...
	while (outPath[i])
	{
		...
		if (!strncasecmp(outPath+i, "%3f", 3)) break;
		if (!strncasecmp(outPath+i, "%20", 3)) 
		{
			outPath[i]=' ';
...
int CdxDashGetSegmentDuration(awRepresentation *rep, awAdaptionSet *set, awPeriod *period, awMpd* mpd,
					 int *nb_segments, long long int *max_seg_duration)
{
	...
	*nb_segments = timescale = 0;
	...
	if(rep->segmentList || set->segmentList || period->segmentList)
	{
		...
		{
			if(period->segmentList->duration) duration = period->segmentList->duration;
			if(period->segmentList->timescale) timescale = period->segmentList->timescale;
			if(period->segmentList->segmentURL) segments = period->segmentList->segmentURL;
			if(period->segmentList->segmentTimeline) timeline = period->segmentList->segmentTimeline;
		}
		if(set->segmentList)
		{
			if(set->segmentList->duration) duration = set->segmentList->duration;
			if(set->segmentList->timescale) timescale = set->segmentList->timescale;
			if(set->segmentList->segmentURL) segments = set->segmentList->segmentURL;
			if(set->segmentList->segmentTimeline) timeline = set->segmentList->segmentTimeline;
		}
		if(rep->segmentList)
		{
			if(rep->segmentList->duration) duration = rep->segmentList->duration;
			if(rep->segmentList->timescale) timescale = rep->segmentList->timescale;
			if(rep->segmentList->segmentURL) segments = rep->segmentList->segmentURL;
			if(rep->segmentList->segmentTimeline) timeline = rep->segmentList->segmentTimeline;
		}
		...
		else
		{
			if(segments) 
			{
				*nb_segments = aw_list_count(segments);
			}
			if(max_seg_duration)
			{
				*max_seg_duration = (long long int)duration;
				*max_seg_duration = *max_seg_duration*1000 / timescale;
			}
		}
		return 0;
	}
	...
	{
		*max_seg_duration = mpd->mediaPresentationDuration;
		*nb_segments = 1;
		return 0;
	}
	...
	{
		single_segment = 0;
		if(period->segmentTemplate->duration) duration = period->segmentTemplate->duration;
		if(period->segmentTemplate->timescale) timescale = period->segmentTemplate->timescale;
		if(period->segmentTemplate->segmentTimeline) timeline = period->segmentTemplate->segmentTimeline;		
	}
	if(set->segmentTemplate)
	{
		single_segment = 0;
		if(set->segmentTemplate->duration) duration = set->segmentTemplate->duration;
		if(set->segmentTemplate->timescale) timescale = set->segmentTemplate->timescale;
		if(set->segmentTemplate->segmentTimeline) timeline = set->segmentTemplate->segmentTimeline;		
	}
	if(rep->segmentTemplate)
	{
		single_segment = 0;
		if(rep->segmentTemplate->duration) duration = rep->segmentTemplate->duration;
		if(rep->segmentTemplate->timescale) timescale = rep->segmentTemplate->timescale;
		if(rep->segmentTemplate->segmentTimeline) timeline = rep->segmentTemplate->segmentTimeline;		
	}
	if(!timescale) timescale = 1;
	...
	else
	{
		...
		{
			long long int nb_seg = (long long int)mediaDuration;
			nb_seg *= timescale;
			nb_seg /= (duration*1000);
			*nb_segments = (int)nb_seg;
...
/**************************************************************************************************/
int CdxDashGetSegmentIndex(int active_rep, awAdaptionSet *set, awPeriod *period, awMpd* mpd, cdx_int64 timeUs, long long int* segment_duration)
{
	int ret;
	int nb_segments;
	...
	ret = CdxDashGetSegmentDuration(rep, set, period, mpd, &nb_segments, &seg_duration);
	if(ret < 0)
	{
		return -1;
	}
...
*/
/*************************************************************************/
static void CdxDashResolveDuration(awRepresentation *rep, awAdaptionSet *set, awPeriod *period, long long int *out_duration, int *out_timescale, long long int *out_pts_offset, awSegmentTimeline **out_segment_timeline)
{
	int timescale = 0;
	long long int pts_offset = 0;
	awSegmentTimeline *segment_timeline;
	awMultipleSegmentBase *mbase_rep, *mbase_set, *mbase_period;
	...
	if (rep->segmentBase || set->segmentBase || period->segmentBase) 
	{
		*out_duration = period ? period->duration : 0;
		timescale = 0;
		if (rep->segmentBase && rep->segmentBase->presentationTimeOffset) pts_offset = rep->segmentBase->presentationTimeOffset;
		if (rep->segmentBase && rep->segmentBase->timescale) timescale = rep->segmentBase->timescale;
		if (!pts_offset && set->segmentBase && set->segmentBase->presentationTimeOffset) pts_offset = set->segmentBase->presentationTimeOffset;
		if (!timescale && set->segmentBase && set->segmentBase->timescale) timescale = set->segmentBase->timescale;
		if (!pts_offset && period->segmentBase && period->segmentBase->presentationTimeOffset) pts_offset = period->segmentBase->presentationTimeOffset;
		if (!timescale && period->segmentBase && period->segmentBase->timescale) timescale = period->segmentBase->timescale;
	...
	mbase_rep = rep->segmentList ? (awMultipleSegmentBase *) rep->segmentList : (awMultipleSegmentBase *) rep->segmentTemplate;
	mbase_set = set->segmentList ? (awMultipleSegmentBase *)set->segmentList : (awMultipleSegmentBase *)set->segmentTemplate;
	mbase_period = period->segmentList ? (awMultipleSegmentBase *)period->segmentList : (awMultipleSegmentBase *)period->segmentTemplate;

	segment_timeline = NULL;
	if (mbase_period) segment_timeline =  mbase_period->segmentTimeline;
	if (mbase_set && mbase_set->segmentTimeline) segment_timeline =  mbase_set->segmentTimeline;
	if (mbase_rep && mbase_rep->segmentTimeline) segment_timeline =  mbase_rep->segmentTimeline;
	...
	*out_timescale = timescale;
	...
	if (out_pts_offset) {
		pts_offset = mbase_rep ? mbase_rep->presentationTimeOffset : 0;
		if (!pts_offset && mbase_set && mbase_set->presentationTimeOffset) pts_offset = mbase_set->presentationTimeOffset;
		if (!pts_offset && mbase_period && mbase_period->presentationTimeOffset) pts_offset = mbase_period->presentationTimeOffset;
	...
	if (segment_timeline)
	{
		awSegmentTimelineEntry *ent = aw_list_get(segment_timeline->entries, 0);
		if (ent) *out_duration = ent->d;
	}
...
*	@segment_duration: the duration of the media segment, the property of SegmentTimeline
*/
/**************************************************************************************************************************/
static int CdxDashResolveUrl(CdxDashParser* dashStream, awPeriod* period,awAdaptionSet* set, awRepresentation* rep,
					CDX_DASHURLResolveType resolve_type, int item_index, char** out_url, long long int *segment_duration)
{
	char* url;
	awBaseURL* url_child;
	unsigned int start_number = 1;
	awSegmentTimeline* time_line = NULL;
	awMpd* mpd = dashStream->mpd;

	//int nb_segments = 0;
	//long long int max_seg_duration = 0;
	...
	url_child = aw_list_get(mpd->baseURLs, 0);
	if(url_child)
	{
		tmp_url = CdxUrlConcatenate(url, (char*)url_child->URL);
		free(url);
		url = tmp_url;
	}

	url_child = aw_list_get(period->baseURL, 0);
	if(url_child)
	{
		tmp_url = CdxUrlConcatenate(url, (char*)url_child->URL);
		free(url);
	...
	url_child = aw_list_get(set->baseURLs, 0);
	if(url_child)
	{
		tmp_url = CdxUrlConcatenate(url, (char*)url_child->URL);
		free(url);
	...
	url_child = aw_list_get(rep->baseURLs, 0);
	if(url_child)
	{
		// if the base url is started with http, is a whole url, do not need merge
		if(!strncasecmp("http://", (char*)url_child->URL, 7))
		{
			free(url);
			url = (char*)url_child->URL;
		}
		else
		{
			tmp_url = CdxUrlConcatenate(url, (char*)url_child->URL);
	...
	CdxDashResolveDuration(rep, set, period, segment_duration, &timescale, NULL, NULL);
	*segment_duration = (resolve_type==CDX_DASH_RESOLVE_URL_MEDIA) ? (int) ((double) (*segment_duration) * 1000.0 / timescale) : 0;
	...
	if(rep->segmentList || set->segmentList || period->segmentList)
	{
		awMpdURL* init_url = NULL;   //init segment url
		awMpdURL* index_url = NULL;
		awSegmentURL* segment_url;
		AW_List*     segments = NULL;
		int segment_count;
		...
		if(period->segmentList)
		{
			if(period->segmentList->initializationSegment)    init_url = period->segmentList->initializationSegment;
			if(period->segmentList->representationIndex)     index_url = period->segmentList->representationIndex;
			if(period->segmentList->segmentURL)               segments = period->segmentList->segmentURL;
			if(period->segmentList->startNumber != (unsigned int) -1) start_number = period->segmentList->startNumber;
			if(period->segmentList->segmentTimeline) time_line = period->segmentList->segmentTimeline;
		}

		if(set->segmentList)
		{
			if(set->segmentList->initializationSegment)    init_url = period->segmentList->initializationSegment;
			if(set->segmentList->representationIndex)     index_url = period->segmentList->representationIndex;
			if(set->segmentList->segmentURL)               segments = period->segmentList->segmentURL;
			if(set->segmentList->startNumber != (unsigned int) -1) start_number = period->segmentList->startNumber;
			if(set->segmentList->segmentTimeline) time_line = period->segmentList->segmentTimeline;
		}

		if(rep->segmentList)
		{
			if(rep->segmentList->initializationSegment)    init_url = period->segmentList->initializationSegment;
			if(rep->segmentList->representationIndex)     index_url = period->segmentList->representationIndex;
			if(rep->segmentList->segmentURL)               segments = period->segmentList->segmentURL;
			if(rep->segmentList->startNumber != (unsigned int) -1) start_number = period->segmentList->startNumber;
			if(rep->segmentList->segmentTimeline) time_line = period->segmentList->segmentTimeline;
		}

		segment_count = aw_list_count(segments);
		...
		case CDX_DASH_RESOLVE_URL_INIT:
			if(init_url)
			{
				...
				{
					*out_url = CdxUrlConcatenate(url, init_url->sourceURL);
					free(url);
				}
				else
				{
		...
		case CDX_DASH_RESOLVE_URL_MEDIA_TEMPLATE:
			...
			*out_url = url;
			segment_url = aw_list_get(segments, item_index);
			if(segment_url->media)
			{
				*out_url = CdxUrlConcatenate(url, segment_url->media);
				free(url);
			}
			return 0;
		...
		case CDX_DASH_RESOLVE_URL_INDEX:
			...
			*out_url = url;
			...
			{
				*out_url = CdxUrlConcatenate(url, segment_url->index);
				free(url);
			}
			return 0;
	...
	char* media_url = NULL;
	char* init_template = NULL;
	char* index_template = NULL;

	/*apply inheritance of attributes, lowest level having preceedence*/
	if(period->segmentTemplate)
	{
		if(period->segmentTemplate->initialization)    init_template = period->segmentTemplate->initialization;
		if(period->segmentTemplate->index)     index_template = period->segmentTemplate->index;
		if(period->segmentTemplate->media)     media_url= period->segmentTemplate->media;
		if(period->segmentTemplate->startNumber != (unsigned int) -1) start_number = period->segmentTemplate->startNumber;
		if(period->segmentTemplate->segmentTimeline) time_line = period->segmentTemplate->segmentTimeline;
	}

	if(set->segmentTemplate)
	{
		if(set->segmentTemplate->initialization)    init_template = set->segmentTemplate->initialization;
		if(set->segmentTemplate->index)     index_template = set->segmentTemplate->index;
		if(set->segmentTemplate->media)     media_url= set->segmentTemplate->media;
		if(set->segmentTemplate->startNumber != (unsigned int) -1) start_number = set->segmentTemplate->startNumber;
		if(set->segmentTemplate->segmentTimeline) time_line = set->segmentTemplate->segmentTimeline;
	}

	if(rep->segmentTemplate)
	{
		if(rep->segmentTemplate->initialization)    init_template = rep->segmentTemplate->initialization;
		if(rep->segmentTemplate->index)     index_template = rep->segmentTemplate->index;
		if(rep->segmentTemplate->media)     media_url= rep->segmentTemplate->media;
		if(rep->segmentTemplate->startNumber != (unsigned int) -1) start_number = rep->segmentTemplate->startNumber;
		if(rep->segmentTemplate->segmentTimeline) time_line = rep->segmentTemplate->segmentTimeline;
	}
	...
	char* solved_template = (char*)malloc( strlen(url_to_solve)+(rep->id? strlen(rep->id) : 0)*2 );
	solved_template[0] = 0;
	strcpy(solved_template, url_to_solve);
	char* first_sep = strchr(solved_template, '$');
	...
	while(first_sep)
	{
		...
		if(format_tag)
		{
			...
			format_tag[0] = 0;
			if(!strchr(format_tag+1, 'd') && !strchr(format_tag+1, 'i') && !strchr(format_tag, 'u'))
			{
		...
		{
			if(resolve_type == CDX_DASH_RESOLVE_URL_MEDIA_TEMPLATE)
			{
				strcat(solved_template, "$Number$");
			}
			else
			{
				// replace the '$Number$' with ( start_number + item_index )
		...
		else if(!strcmp(first_sep+1, "Time"))
		{
			...
			// use segment timeline
				int k, nb_seg, cur_idx, nb_repeat;
				long long int time, start_time;
				nb_seg = aw_list_count(time_line->entries);
				cur_idx = 0;
				start_time=0;
				for (k=0; k<nb_seg; k++) {
					awSegmentTimelineEntry *ent = aw_list_get(time_line->entries, k);
					if (item_index > cur_idx+(int)ent->r) {
						cur_idx += 1 + ent->r;
					...
					*segment_duration = ent->d;
					*segment_duration = (int) ((double) (*segment_duration) * 1000.0 / timescale);
					nb_repeat = item_index - cur_idx;
					time = ent->t ? ent->t : start_time;
					time += nb_repeat * ent->d;
	...
	*out_url = CdxUrlConcatenate(url, solved_template);
	free(url);
	free(solved_template);
	return 0;	
}
...
int CdxDashInitSegmentUrl(CdxDashParser* dashStream, int type)
{
	int ret;
	char* init_url = NULL;
	long long int segment_duration;
	if(!dashStream) return -1;
	awMpd* mpd = dashStream->mpd;
	awPeriod* period = aw_list_get(mpd->periods, dashStream->activePeriodIndex);
	awAdaptionSet* v_set = aw_list_get(period->adaptationSets, dashStream->mediaStream[type].activeAdapSetIndex);
	if(!v_set)
	{
		return -1;
	}
	
	awRepresentation* v_rep = aw_list_get(v_set->representation, dashStream->mediaStream[type].activeRepSetIndex);
	if(!v_rep )
	{
		return -1;
	}
	...
	//if(v_rep)
	{
		ret = CdxDashResolveUrl(dashStream, period, v_set, v_rep, CDX_DASH_RESOLVE_URL_INIT, 0, &init_url, &segment_duration);
		if(ret < 0)
		{
		...
		//CDX_LOGD("--dash-- init url = %s", init_url);
		...
		if(!strstr(init_url, "://") || !strncasecmp(init_url, "file://", 7) || 
			!strncasecmp(init_url, "gmem://", 7) || !strncasecmp(init_url, "views://", 8))
		{
			
		}

		dashStream->mediaStream[type].datasource->uri = init_url;
...
/******************************************************************************************/
int CdxDashGetSegmentUrl(CdxDashParser* dashStream, awPeriod* period, awAdaptionSet* set, awRepresentation* rep, int activeSegmentIndex)
{
	int ret;
	long long int segment_duration;
	char* segment_url = NULL;

	ret = CdxDashResolveUrl(dashStream, period, set, rep, CDX_DASH_RESOLVE_URL_MEDIA, activeSegmentIndex, &segment_url, &segment_duration);
	if(ret)
	{
		CDX_LOGE("resolve init segment url error!\n");
		if(segment_url)
	...
	CDX_LOGD("**** segment url = %s", segment_url);
	dashStream->tmpUrl = segment_url;
...
/******************************************************************************************/
//****<  used by seekto
//****< NOTE: if the active segment index not changed, we need not open a new parser,just use the old one
/******************************************************************************************/
int CdxDashSeekToTime(CdxDashParser* impl, cdx_int64 timeMs, int type)
{
	...
	awAdaptionSet* set = aw_list_get(period->adaptationSets, impl->mediaStream[type].activeAdapSetIndex);
	...
	ret = CdxDashGetSegmentIndex(impl->mediaStream[type].activeRepSetIndex, set, period, mpd, timeMs, &segment_duration);
	if(ret < 0)
	{
	...
	if(ret != impl->mediaStream[type].activeRepSetIndex)
	{
		...
		time = ret*segment_duration;

		awRepresentation* rep = aw_list_get(set->representation, impl->mediaStream[type].activeRepSetIndex);
	
		//get the right segment url
		ret = CdxDashGetSegmentUrl(impl, period, set, rep, impl->mediaStream[type].activeSegmentIndex);	
		if(ret < 0) 
		{
			return ret;
		}
		...
		//impl->mediaStream[type].parser = CdxParserPrepare(impl->mediaStream[type].datasource, 0, &impl->exitFlag);
        ret = CdxParserControl(impl->mediaStream[type].parser, CDX_PSR_CMD_REPLACE_STREAM, (void*) impl->mediaStream[type].stream);
        if(ret < 0)
        {
    ...
    CDX_LOGD("---- baseTime = %lld, activeSegmentIndex = %d", time, impl->mediaStream[type].activeSegmentIndex);
	ret = CdxParserSeekTo(impl->mediaStream[type].parser, (timeMs-time)*1000);
	if(ret < 0)
	{
		return -1;
	}
...
static cdx_int32 __CdxDashParserClose(CdxParserT *parser)
{
    ...
    if(DashPsr->stream)
    {
        CdxStreamForceStop(DashPsr->stream);
    }
    for(i=0; i<3; i++)
    {
        if(DashPsr->mediaStream[i].parser)
            CdxParserForceStop(DashPsr->mediaStream[i].parser);
    }
...
static cdx_int32 __CdxDashParserPrefetch(CdxParserT *parser, CdxPacketT * pkt)
{
	...
	else if(status == PSR_EOS)
	{
		...
		if(impl->mediaStream[impl->prefetchType].eou == 1)
		{
			...
			impl->streamPts[impl->prefetchType] = 0xfffffffffffffff; // maybe the pts is small in live play
			impl->mStatus = CDX_DASH_PREFETCHING;
			return CdxParserPrefetch(parser, pkt);
		}
		
		period = aw_list_get(mpd->periods, impl->activePeriodIndex);
		set = aw_list_get(period->adaptationSets, impl->mediaStream[impl->prefetchType].activeAdapSetIndex);
		rep = aw_list_get(set->representation, impl->mediaStream[impl->prefetchType].activeRepSetIndex);
		if(!rep)
		{
			CDX_LOGE("-- get representation failed, repIdx(%d)", impl->mediaStream[impl->prefetchType].activeRepSetIndex);
			return -1;
		}
...
// resolution change
		...
		if(impl->prefetchType == 0)
		{
			...
			else if(ret == 1)
			{
				...
				if(impl->hasVideo)
				{
					...
					CDX_LOGD("-- extradataSize = %d, size=%d", mediaInfo.program[0].video[0].nCodecSpecificDataLen, size);
		...
		//***< get next segment url			
		ret = CdxDashGetSegmentUrl(impl, period, set, rep, impl->mediaStream[impl->prefetchType].activeSegmentIndex);
		if(ret < 0) 
		{
		    impl->mStatus = CDX_DASH_IDLE;
        ...
        ret = CdxStreamOpen(impl->mediaStream[impl->prefetchType].datasource, &impl->mutex, &impl->exitFlag, 
        					&impl->mediaStream[impl->prefetchType].stream, NULL);
        if(ret < 0)
        {
            //***< the url is not exist, so the end of url
        ...
        ret = CdxParserControl(impl->mediaStream[impl->prefetchType].parser, CDX_PSR_CMD_REPLACE_STREAM, (void*)impl->mediaStream[impl->prefetchType].stream);
        if(ret < 0)
        {
            CDX_LOGW("*** dash replace stream error!");
            impl->mStatus = CDX_DASH_IDLE;
...
static cdx_int32 __CdxDashParserGetMediaInfo(CdxParserT *parser, CdxMediaInfoT * pMediaInfo)
{
	...
	if(impl->hasVideo)
	{
		...
		{
			memcpy(&pMediaInfo->program[0].video[i], &pVideoMediaInfo->program[0].video[i], sizeof(VideoStreamInfo));
		}
	}
	if(impl->hasAudio)
	{	
		...
		for(i=0; i<pMediaInfo->program[0].audioNum; i++)
		{
            ...
            memcpy(&pMediaInfo->program[0].audio[i], &pAudioMediaInfo->program[0].audio[i], sizeof(AudioStreamInfo));
		}
	}
	if(impl->hasSubtitle)
	{	
		...
		for(i=0; i<pMediaInfo->program[0].subtitleNum; i++)
		{
            ...
            memcpy(&pMediaInfo->program[0].subtitle[i], &pSubtitleMediaInfo->program[0].subtitle[i], sizeof(SubtitleStreamInfo));
		}
	}
	
    CDX_LOGD("--- videoNum = %d, audioNum = %d", pMediaInfo->program[0].videoNum, pMediaInfo->program[0].audioNum);	
...
cdx_int32 __CdxDashParserSeekTo(CdxParserT *parser, cdx_int64  timeUs )
{
	int ret;
	CdxDashParser          *impl;
    impl = (CdxDashParser *)parser;
	...
	if(impl->hasVideo)
	{	
		ret = CdxDashSeekToTime(impl, timeMs, 0);
		if(ret < 0)
		{
		    impl->mStatus = CDX_DASH_IDLE;
			return -1;
	...
	if(impl->hasAudio)
	{
		ret = CdxDashSeekToTime(impl, timeMs, 1);
		if(ret < 0)
		{
		    impl->mStatus = CDX_DASH_IDLE;
			return -1;
	...
	if(impl->hasSubtitle)
	{
		ret = CdxDashSeekToTime(impl, timeMs, 2);
		if(ret < 0)
		{
		    impl->mStatus = CDX_DASH_IDLE;
			return -1;
