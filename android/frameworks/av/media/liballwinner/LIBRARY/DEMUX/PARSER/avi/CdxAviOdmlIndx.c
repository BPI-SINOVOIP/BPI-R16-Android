...
cdx_int32 InitialPsrIndxTableReader(ODML_SUPERINDEX_READER *pReader, AviFileInT *aviIn, cdx_int32 streamIndex)
{
    ...
    if(AVI_INDEX_OF_INDEXES == sInfo->indx->bIndexType)
    {
    ...
    else if(AVI_INDEX_OF_CHUNKS == sInfo->indx->bIndexType)
    { //we build super index for ourselves.
        CDX_LOGW("super index type is AVI_INDEX_OF_CHUNKS, we don't support now!\n");
        return AVI_ERR_FAIL;
        #if 0
    ...
    {
        CDX_LOGW("we don't know how to process ODML case: AVI_INDEX_IS_DATA.");
        return AVI_ERR_FAIL;
    }
    return ret;
_error2:
