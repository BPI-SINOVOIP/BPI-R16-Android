...
cdx_int16 FlvGetData(CdxStreamT *stream, cdx_int32 *data)
{
	...
	if(len != 1 || type != AMF_DATA_TYPE_NUMBER)
	{
		CDX_LOGE("not AMF_DATA_TYPE_NUMBER");
		return -1;				
	}
...
cdx_int16 FlvBuildKeyFrameIndex(struct VdecFlvIn *s)
{
	...
	if(len != 1 || type != AMF_DATA_TYPE_MIXEDARRAY)
	{
	...
	while((arrynum-- > 0) && (FlvGetString(s->fp, buffer) > 0))
	{
		...
		switch(headertag)
		{
			...
			{
				len = CdxStreamRead(s->fp, &type, 1);
				if(len != 1 || type != AMF_DATA_TYPE_STRING)
				{
...
//				}
			...
			case Keyframes:
          	{
				len = CdxStreamRead(s->fp, &type, 1);
				if(len != 1 || type != AMF_DATA_TYPE_OBJECT)
				{
				...
				for(keyframes_counter = 0; keyframes_counter < 2; keyframes_counter++)
				{
					...
					len = CdxStreamRead(s->fp, &type, 1);
					if(len != 1 || type != AMF_DATA_TYPE_ARRAY)
					{
					...
					//LOGV("AMF_DATA_TYPE_ARRAY,length,%x",length);
					length = EndianConvert(length, 4);
					CDX_LOGV("AMF_DATA_TYPE_ARRAY,length,%x",length);
					...
					if(strcmp(buffer, "times"))//case position index
					{
						...
						{
							if(CdxStreamRead(s->fp, &type, 1) != 1 || type != AMF_DATA_TYPE_NUMBER)
							{
			...
			default:
			{
				...
				switch(type)
				{
					case AMF_DATA_TYPE_NUMBER:
					{
						...
						{
						    CDX_LOGE("AMF_DATA_TYPE_NUMBER seek failed.");
							return -1;
						}
						break;
					}
					case AMF_DATA_TYPE_BOOL:
					{
						...
						{
						    CDX_LOGE("AMF_DATA_TYPE_BOOL seek failed.");
							return -1;
						}
						break;
					}
					case AMF_DATA_TYPE_STRING:
					{
						if(FlvGetString(s->fp, buffer) <= 0)
						{
						    CDX_LOGE("AMF_DATA_TYPE_STRING FlvGetString failed.");
							return -1;
						}
						break;
					}
					...
					case AMF_DATA_TYPE_DATE:
					{
						...
						{
						    CDX_LOGE("AMF_DATA_TYPE_DATE seek failed.");
							return -1;
						}
						break;
                    }
					case AMF_DATA_TYPE_ARRAY:
					{
						if(CdxStreamRead(s->fp, &length, 4) != 4)
						{
						    CDX_LOGE("AMF_DATA_TYPE_ARRAY Read failed.");
							return -1;
						}
						if(length != 0)
							return -1;
