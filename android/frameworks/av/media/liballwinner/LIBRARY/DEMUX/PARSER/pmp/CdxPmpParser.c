...
static const cdx_uint32 sampRateTab[NUM_SAMPLE_RATES] = {
...
{
    cdx_uint8 *tmpBuf = (cdx_uint8 *)CdxMalloc(pmpParser->auStrmNum * pmpParser->maxAuPerFrame * 4 + 13);
    if (!tmpBuf)
    {
    ...
    cdx_uint32 *tmpAuFrameSize = (cdx_uint32 *)CdxMalloc(pmpParser->auStrmNum * pmpParser->maxAuPerFrame * 4);
    if (!tmpAuFrameSize)
    {
        free(tmpBuf);
        free(auSize);
    ...
    if(pmpParser->auCodecID == 1)
	{
	    ...
	    cdx_uint16 channelConfig_3bits           = 0;
	    ...
	    dataFrame->aac_hdr[3] = channelConfig_3bits<<6;
...
static int BuildKeyFrameIndexTable(CdxPmpParser *pmpParser)
{
    ...
    cdx_uint32 frmCnt, keyFrmNum = 0;
    for(frmCnt = 0; frmCnt < pmpParser->vdFrmNum; frmCnt++)
    {
    ...
    cdx_uint32 keyFrmCnt = 0;
    for(frmCnt = 0; keyFrmCnt < keyFrmNum; frmCnt++)
    {
...
static int SetMediaInfo(CdxPmpParser *pmpParser)
{
    ...
    video->nFrameRate = pmpParser->vdRate * 1000 / pmpParser->vdScale;/*视频帧率，表示每1000秒有多少帧画面被播放，例如25000、29970、30000等*/
    video->nFrameDuration = pmpParser->vdFrmInterval;/*两帧画面之间的间隔时间，是帧率的倒数，单位为us*/
...
_newDataFrame:    
    if(pmpParser->curStream == 0)
    {
        ...
        cdx_uint8 *tmpBuf = dataFrame->tmpBuf;
        ret = CdxStreamRead(pmpParser->file, tmpBuf, 13);
        if(ret != 13)
        {
        ...
        dataFrame->audFrmNum = tmpBuf[0];
        dataFrame->firstDelay = tmpBuf[1] | tmpBuf[2]<<8 | tmpBuf[3]<<16 | tmpBuf[4]<<24;
        //dataFrame->lastDelay = tmpBuf[5] | tmpBuf[6]<<8 | tmpBuf[7]<<16 | tmpBuf[8]<<24;
        dataFrame->vdFrmSize = tmpBuf[9] | tmpBuf[10]<<8 | tmpBuf[11]<<16 | tmpBuf[12]<<24;
        ...
        {
            CDX_LOGE("audFrmNum(%u) > maxAuPerFrame(%u)", dataFrame->audFrmNum, pmpParser->maxAuPerFrame);
            ret = -1;
            pmpParser->mErrno = PSR_UNKNOWN_ERR;
            goto _exit;
        }
        len = dataFrame->audFrmNum * 4 * pmpParser->auStrmNum;
        ret = CdxStreamRead(pmpParser->file, tmpBuf, len);
        if(ret != len)
        {
            CDX_LOGE("CdxStreamRead fail, request(%d), ret(%d)", len, ret);
            ret = -1;
        ...
        cdx_uint32 *audioFrameSize = (cdx_uint32 *)tmpBuf;
        for (i = 0; i < pmpParser->auStrmNum; i++)
        {
...
static int MakeSpecificData(CdxPmpParser *pmpParser)
{
    int ret = 0;
    CdxPacketT *pkt;
    unsigned int vCodecFormat = pmpParser->vdCodecID == 1 ? VIDEO_CODEC_FORMAT_H264 : VIDEO_CODEC_FORMAT_XVID;
    while(1)
    {
        pkt = NULL;
        ...
        {
            if(pkt->length + pmpParser->vProbeBuf.probeDataSize > pmpParser->vProbeBuf.probeBufSize)
            {
            ...
            {
                cdx_uint8 *data = pmpParser->vProbeBuf.probeBuf + pmpParser->vProbeBuf.probeDataSize;
            ...
            ret = ProbeVideoSpecificData(&pmpParser->tempVideoInfo, 
                pmpParser->vProbeBuf.probeBuf, pmpParser->vProbeBuf.probeDataSize, vCodecFormat, CDX_PARSER_PMP);
...
static cdx_int32 PmpParserPrefetch(CdxParserT *parser, CdxPacketT *cdxPkt)
{
    ...
    {
        CDX_LOGE("status != CDX_PSR_IDLE && status != CDX_PSR_PREFETCHED, PmpParserPrefetch invaild");
        pmpParser->mErrno = PSR_INVALID_OPERATION;
        return -1;
    }
...
cdx_int32 PmpParserSeekTo(CdxParserT *parser, cdx_int64  timeUs)
{
    ...
    else if((cdx_uint64)timeUs >= pmpParser->durationUs)
    {
        CDX_LOGI("PSR_EOS");
        pmpParser->mErrno = PSR_EOS;
        return 0;
