...
cdx_int32 __RemuxPsrPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    {
        if(impl->preVideopts >= ptkHead.pts)
        {
            pkt->pts = impl->preVideopts + 1;
            impl->preVideopts = pkt->pts;
        }
        else
        {
            pkt->pts = ptkHead.pts;
    ...
    logv("==========prefetch pkt->type(%d),pkt->length(%d), pkt->pts(%lld), pkt->flags(%d)", pkt->type, pkt->length, pkt->pts, pkt->flags);
...
cdx_int32 __RemuxPsrRead(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    else 
    {
        ...
        logv("============ pkt->buflen(%d), pkt->ringBufLen(%d), pkt->length(%d)", pkt->buflen, pkt->ringBufLen, pkt->length);
...
cdx_int32 __RemuxPsrGetMediaInfo(CdxParserT *parser, CdxMediaInfoT *pMediaInfo)
{
    ...
    if(pMediaInfo->program[0].video[0].pCodecSpecificData && pMediaInfo->program[0].video[0].eCodecFormat == VIDEO_CODEC_FORMAT_XVID)
    {
        int width, height;
    ...
    {
        int width, height;
        //char a[32]={00, 0x00, 0x80, 0x02, 0x08, 0x04, 0x26, 0x20, 0x20, 0x20, 0x21, 0xFF, 0xFF, 0x31, 0x01, 0x01, 0x01, 0x0F, 0xFF, 0xF9, 0x88, 0x08, 0x08, 0x08, 0x7F, 0xFF, 0xCC, 0x40, 0x40, 0x40, 0x43, 0xFF};
...
static int __RemuxPsrInit(CdxParserT *parser)
{
	...
	if(CdxStreamGetMetaData(impl->stream, "extra-data", (void **)&extraDataContainer) == 0 && extraDataContainer)
	{
	    ...
	    if(extraDataContainer->extraDataType == EXTRA_DATA_RTSP)
	    {
            ...
            memcpy(impl->extraDataContainer.extraData, extraDataContainer->extraData, sizeof(CdxMediaInfoT));
	    }
