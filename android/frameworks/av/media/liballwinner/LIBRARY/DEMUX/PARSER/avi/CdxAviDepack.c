...
cdx_int16 GetNextChunkHead(CdxStreamT *pf, AviChunkT *chunk, cdx_uint32 *length)
{
    ...
    else if(chunk->fcc == listheaderLIST)
    {
...
AviStreamInfoT *GetNextStreamInfo(AviFileInT *s)
{
    ...
    } while((chunk->fcc != listheaderLIST) || strncmp((char *)chunk->buffer, "strl", 4) != 0);
    ...
    while((fpOffset = CdxStreamTell(pf)) < listEnd - 1)  
    {
        ...
        switch(chunk->fcc)
        {
            case ckidSTREAMHEADER://strh
            {
                ...
                else if(streamHeader->fccType == streamtypeAUDIO)
                {
                    streamInfo->mediaType = 'a';
                }
                else if(streamHeader->fccType == streamtypeTEXT)
                {
                    streamInfo->mediaType = 't';
                }
                break;
            }
            case ckidSTREAMFORMAT://strf
            {
                ...
                if(streamInfo->mediaType == 'v') //may not really video
                {
                    ...
                    if(bmpInfoHdr->biCompression == mmioFOURCC('D', 'X', 'S', 'B') ||
                       bmpInfoHdr->biCompression == mmioFOURCC('D', 'X', 'S', 'A'))
                    {
                        streamInfo->mediaType = 't';
            ...
            case ckidODMLINDX://indx 
            {
                ...
                if(streamInfo->indx->bIndexSubType
                    || streamInfo->indx->bIndexType != AVI_INDEX_OF_INDEXES
                    || streamInfo->indx->wLongsPerEntry != 4) //-- bIndexSubType ?   0 | AVI_INDEX_2FIELD
                {
...
//                    break;
                ...
                {
                    if(streamInfo->indx->bIndexType == AVI_INDEX_OF_INDEXES)
                    {
...
cdx_int32 GetIdx1Info(AviFileInT *aviIn)
{
    ...
    while(mFcc != ckidAVINEWINDEX && (i++ < 1000));

    if(mFcc == ckidAVINEWINDEX)
    {
        ...
        if((CdxStreamTell(aviIn->fp) - aviIn->idx1Start) == (cdx_int64)mLength)
        {
            ...
            while(1)
            {
                ...
                if(mIdx.ckid != listtypeAVIRECORD)
                {
                    break;
                }
                else
            ...
            if((tmpTag == mIdx.ckid && tmpSize == mIdx.dwChunkLength)
                || (tmpTag == mIdx.ckid && tmpTag == listheaderLIST))
            {
                indexType = 1;
            }
            else
...
cdx_int16 AVIReaderOpenFile(CdxAviParserImplT *impl)//(AviFileInT *aviFile)// CedarXDataSourceDesc *datasrc_desc)
{
    ...
    if((ret < 0) || (chunk->fcc != formheaderRIFF) || (strncmp((char *)chunk->buffer, "AVI ", 4) != 0))
    {
    ...
    } while(chunk->fcc != listheaderLIST || strncmp((char*)chunk->buffer, "hdrl", 4) != 0);

    // read the 'avih' chunk
    ret = GetNextChunk(aviFile->fp, &aviFile->dataChunk); 
    if ((ret < 0) || (chunk->fcc != ckidAVIMAINHDR))
    {
        CdxStreamClose(aviFile->fp);
        aviFile->fp = NULL;
                ...
                switch (bmpInfoHdr->biCompression)
                {
                    ...
                    default:
                    {
                        CDX_LOGV("video bitstream type(%x, %x): UNKNOWNVBS, try it with xvid!\n",  \
                                                strmHdr->fccHandler, bmpInfoHdr->biCompression);
                        if(strmHdr->fccHandler == mmioFOURCC('M','S','V','C'))
                        {
                            p->aviFormat.vFormat.eCodecFormat = VIDEO_CODEC_FORMAT_UNKNOWN;
                        }
                        else
...
//#elif(EPDK_ARCH == EPDK_ARCH_SUNII)
            ...
            //case mmioFOURCC('I','V','5','0'): // Intel/Ligos Indeo Interactive 5.0 (IV50)
            //{
            //    CDX_LOGW("codec not support IV50 yet.");
            //}

            //Cinepack, not support
            case mmioFOURCC('d','i','v','c'):
            case mmioFOURCC('D','I','V','C'):
//            case mmioFOURCC('M','P','4','2'):   //MSMPEGV2
            case mmioFOURCC('m','p','4','2'):
            case mmioFOURCC('f','f','d','s'):
            default:
            {
...
cdx_int32 SetFileParserSubInfo(CdxAviParserImplT *p)
{
    ...
    if(p->hasSubTitle)
    {
        for(i = 0; i < p->hasSubTitle; i++)
        {
            ...
            if(strmHdr->fccType == streamtypeVIDEO) //bitmap form of subtitles.
            {
                switch(pStrmInfo->aviSubTag)
                {
                    case mmioFOURCC('D','X','S','A'):
                    {
            ...
            else if(strmHdr->fccType == streamtypeTEXT) //text form of subtitles.
            {
                CDX_LOGW("not yet ...");
            }
            else 
...
static cdx_int32 CreateStreamName(cdx_uint8 *pStreamName, cdx_int32 nNameSize, cdx_int32 streamIdx)
{
    ...
    strStreamId[0] = ToHex(((streamIdx) & 0xf0)>>4);
    strStreamId[1] = ToHex((streamIdx) & 0x0f);
...
{ //The ‘strh’ chunk of a subtitle stream should be as it  is normally defined, with fccType ‘txts’ for text form 
  //of subtitles or ‘vids’ for bitmap form of subtitles. 
    ...
    if(psbsHdr->fccType == streamtypeVIDEO) //bitmap form of subtitles.
    {
    ...
    else if(psbsHdr->fccType == streamtypeTEXT) //text form of subtitles.
    {
        CDX_LOGW("not yet...");
    }
    else
...
cdx_int16 AviOpen(CdxAviParserImplT *impl)//(struct FILE_PARSER *p, CedarXDataSourceDesc *datasrc_desc)
{
    ...
    do
    {
        ...
        if(chunk->fcc == listheaderLIST && strncmp((char *)chunk->buffer, "odml", 4) == 0)
        {
    ...
    }while(chunk->fcc != listheaderLIST || strncmp((char *)chunk->buffer, "movi", 4) != 0);
              ...
              || impl->aviFormat.vFormat.eCodecFormat == VIDEO_CODEC_FORMAT_WMV2) && impl->aviFormat.vFormat.nCodecSpecificDataLen == 0))
    {
        ...
        do
        {
            ...
            if(((chunk->fcc >> 16) == cktypeDIBbits) || ((chunk->fcc >> 16) == cktypeDIBcompressed))
            {
                cdx_int32       i;
...
cdx_int16 AVIBuildIdx(CdxAviParserImplT *p)
{
    ...
    else
    {
        ...
        avih = (MainAVIHeaderT*)aviIn->avih->buffer;
        if((avih->dwFlags & AVIF_ISINTERLEAVED) || p->hasSubTitle)  
        {
            aviIn->readmode = READ_CHUNK_SEQUENCE;
