...
cdx_uint8 CheckFirstPack(CdxMpgParserT *MpgParser, cdx_uint8 *buf)
{
    ...
    else
    {
        ...
        if(MpgOpenShowDword(buf) == SYSTEM_HEADER_START_CODE)
	    { 
		   nSize = (buf[4]<<8) | buf[5];
		   buf += nSize + 6;
        ...
        if(((nNext32bits & 0xf0)!=VIDEO_ID) && ((nNext32bits & 0xe0)!=AUDIO_ID) && 
            (nNext32bits !=PRIVATE_STREAM_1))
	    {  
		   return 0;
	    }
        else
	...
	while (nEsMapLength >= 4)
	{
		...
		switch(nType)
		{
           ...
           case STREAM_TYPE_VIDEO_MPEG2:
			    MpgParser->mVideoFormatT.eCodecFormat = STREAM_TYPE_VIDEO_MPEG2;
				break;
		   case STREAM_TYPE_VIDEO_MPEG4:
			   	MpgParser->mVideoFormatT.eCodecFormat = VIDEO_CODEC_FORMAT_MPEG4;
				break;
		   case STREAM_TYPE_VIDEO_CODEC_FORMAT_H264:
...
cdx_uint8 CheckAudioType(CdxMpgParserT *MpgParser, cdx_uint8 *buf, cdx_uint32 offset)
{
	...
	if(code>=0x80 && code <=0x87)
	{
		...
		//mMpgParserCxt->nAudioId = PRIVATE_STREAM_1;
        ...
        if(i == MpgParser->nhasAudioNum)
		{
            ...
            mMpgParserCxt->nAudioIdArray[MpgParser->nhasAudioNum] = PRIVATE_STREAM_1;
...
{   
    ...
    if(startCode == SYSTEM_HEADER_START_CODE)           // 0x000001BB
    {  
        if(packInf[0] == 1)
        {
            packInf[2] = packInf[3];
    ...
    else if(startCode == PACK_START_CODE)               // 0x000001BA
    { 
       if(packInf[0] == 1)
       {
    ...
    else if(startCode == PROGRAM_STREAM_MAP)            // 0x000001BC
    {
    ...
    else if((startCode == PADDING_STREAM) || (startCode == PRIVATE_STREAM_2))       // 0x000001BE 0x000001BF
    {
       bufOffset = (checkBufPtr[4]<<8) | checkBufPtr[5];
	   checkBufPtr += (bufOffset + 6);
    }
    else        // video pack, audio pack, subtitle pack
    {
       ...
       isVideoPack   = ((startCode&0xf0) == VIDEO_ID);
       ...
       else if(startCode == SEQUENCE_HEADER_CODE)                                // sequence header
       {
          calPackNum[vidSeq]++;
       ...
       bufOffset = 4;
       if((isMpegAudPack==1)||(isVideoPack==1)||(startCode==PRIVATE_STREAM_1))
       {
...
cdx_uint8 *MpgOpenJudgePacketType(CdxMpgParserT *MpgParser, cdx_uint8 *cur_ptr, cdx_int16 *nRet)
{
	...
	*nRet = 0;
	...
	if(MpgOpenShowDword(cur_ptr) == SYSTEM_HEADER_START_CODE)
	{
...
__read_new_data:
 ...
 __cal_parse_result:
    ...
    else 
    {
        ...
        {
            if((MpgParser->bIsVOB==1) && (packInf[2]!=0))
            {    
