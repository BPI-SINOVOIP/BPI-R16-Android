...
static cdx_int32 IsmcFragmentElementExtract(xmlNodePtr node, SmsStreamT *sms, cdx_int32 loopCount, cdx_bool *bWeird,
                                        cdx_int64 *computedStartTime, cdx_int64 *computedDuration)
{
...
static cdx_int32 IsmcQualityLevelExtract(xmlNodePtr node, SmsStreamT *sms, cdx_uint32 qid)
{
    ...
    while(attrPtr != NULL)
    {
        ...
        else if(!strcmp((char *)attrPtr->name, "WaveFormatEx"))
        {
            ...
            //cdx_uint16 dataLen = ((cdx_uint16 *)waveFormatEx)[8]; //* cbSize: Codec Specific Data Size
...
*******************************/    
            ...
            //CDX_LOGD("wftag=0x%x, (char *)attrPtr->children->content=[%s]", wfTag, (char *)attrPtr->children->content);
        ...
        else if(!strcmp((char *)attrPtr->name, "MaxWidth") || !strcmp((char *)attrPtr->name, "Width"))
            ql->MaxWidth = strtoul((char *)attrPtr->children->content, NULL, 10);
        else if(!strcmp((char *)attrPtr->name, "MaxHeight") || !strcmp((char *)attrPtr->name, "Height"))
...
static cdx_int32 IsmcStreamIndexExtract(xmlNodePtr node, AwIsmcT *ismc, cdx_uint32 trackId)
{
    ...
    while(node != NULL)
    {
        ...
        {
            if(IsmcFragmentElementExtract(node, sms, loopCount, &bWeird, &computedStartTime, &computedDuration) < 0)
            {            
    ...
    if(sms->qlevelNb == 0)
    {
        sms->qlevelNb = SmsArrayCount(sms->qlevels);//sms->qlevels->iCount; //* iCount will increase while append... vlc_array_append(sms->qlevels, ql)
    }
