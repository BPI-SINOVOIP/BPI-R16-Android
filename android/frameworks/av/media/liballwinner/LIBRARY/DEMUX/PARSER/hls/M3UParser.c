...
/* what可以是'"'，这一点优于原来的函数. */
//***********************************************************//
static ssize_t FindNextUnquoted(const AString *line, char what, cdx_uint32 offset) 
{
...
status_t MakeURL(const char *baseURL, const char *url, AString **out)
{
	*out = NULL;
	...
	if (url[0] == '/') 
	{
		...
		if (pathStart != NULL) 
		{
			memSize = pathStart - baseURL + urlLen + 1;
			...
			memcpy(temp + (pathStart - baseURL), url, urlLen + 1);/*url是以'\0'结尾的*/
		} 
		else 
		{
...
{
    char *end;
    long long lval = strtoll(s, &end, 10);
								...
								uint64_t *length, uint64_t *offset) 
{
    ...
    {
        char *end;
        *length = strtoull(colonPos + 1, &end, 10);
        if (!strcmp(end, colonPos + 1) || (*end != '\0' && !isspace(*end))) 
            return ERROR_MALFORMED;
        *offset = curOffset;    
    }
    else
    {
        char *end;
        *length = strtoull(colonPos + 1, &end, 10);
        if (!strcmp(end, colonPos + 1) || (*end != '@' && *end != '\0' && !isspace(*end))) 
            return ERROR_MALFORMED;
        *offset = strtoull(atPos + 1, &end, 10);
        if (!strcmp(end,colonPos + 1) || (*end != '\0' && !isspace(*end))) 
            return ERROR_MALFORMED;        
    }
    return OK;
		...
		*equalPos = '\0';
		...
		else if (!strcasecmp("group-id", key)) 
		{
            ...
            if (len < 2
                    ...
                    || val[len - 1] != '"') 
			{
                CDX_LOGE("Expected quoted string for GROUP-ID, got '%s' instead.", val);
				err = ERROR_MALFORMED;
				goto _err;
            }
		...
		else if (!strcasecmp("language", key)) 
		{
            ...
            if (len < 2
                    ...
                    || val[len - 1] != '"') 
			{
                CDX_LOGE("Expected quoted string for LANGUAGE, got '%s' instead.", val);
				err = ERROR_MALFORMED;
				goto _err;
            }
	...
	*P = NULL;
    ...
    {   
        ...
        if (!startsWith(line.mData,"#")) /*不是空行，不是标签，不是注释，是URL*/
        {
			...
			if (itemMeta.mNumAtom == 0 || 
				(!playList->mIsVariantPlaylist && !findInt64(&itemMeta,"durationUs", &durationUs))
				|| (playList->mIsVariantPlaylist && !findInt64(&itemMeta,"bandwidth", &bandwidth)))
				/*一行<uri>（不是URI=）只会出现在streaminf 或EXTINF之后*/
			{
				CDX_LOGE("ERROR_MALFORMED");
				err = ERROR_MALFORMED;
				goto _err;
