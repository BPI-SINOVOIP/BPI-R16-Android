...
static int AwtsNewTrack(AwtsParser *awtsParser) 
{
	...
	if (track->nCodecSpecificDataLen > 0) 
    {
        ...
        ret = CdxStreamRead(awtsParser->file, track->pCodecSpecificData, track->nCodecSpecificDataLen);
...
static int AwtsReadStreamInfo(AwtsParser *awtsParser)
{
    ...
    CDX_LOGD("Length = 0x%x %x %x %x %x %x %x", tmp[0], tmp[1], tmp[2], tmp[3], tmp[4], tmp[5], tmp[6]);
