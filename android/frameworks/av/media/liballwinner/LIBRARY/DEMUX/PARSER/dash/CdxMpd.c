...
static int64_t MpdParseDate(char *attr)
{
    //attr = NULL;
#if 0
	unsigned int year, month, day, h, m, ms;
	int oh, om;
	float s;
	int64_t res;
	int ok = 0;
...
static void commonAttributeTraverse(xmlNodePtr node, MPD_CommonAttributes* com)
{
	...
	while (atrrPtr != NULL) {
		if(!strcmp((const char*)atrrPtr->name, "profiles")) {
			...
			if (!strcmp((char*)atrrPtr->children->content, "progressive")) com->scan_type = AW_MPD_SCANTYPE_PROGRESSIVE;
			else if (!strcmp((char*)atrrPtr->children->content, "interlaced")) com->scan_type = AW_MPD_SCANTYPE_INTERLACED;
	...
	if(!strcmp((const char*)node->name, "MPD"))
	{
		...
		{	
			if(!strcmp((const char*)atrrPtr->name, "type")) {
				...
				mpd->mediaPresentationDuration = MpdParseDuration((char*)atrrPtr->children->content);
				...
				mpd->suggestedPresentationDelay = MpdParseDuration((char*)atrrPtr->children->content);
...
*/
awMpd* ParseMpdFile(char* filename, int fd, char* buffer, int size, const char *encoding, int options)
{
