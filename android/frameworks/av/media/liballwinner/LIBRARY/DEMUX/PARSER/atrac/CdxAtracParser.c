...
int ff_id3v2_match(const cdx_int8 *buf, const char *magic)
{
...
int ff_id3v2_tag_len(const cdx_int8 *buf)
{
...
static int CdxAtracParserSeekTo(CdxParserT *parser, cdx_int64 timeUs)
{
    ATRACParserImpl *atrac;
    int ret = 0;
    int nSeekBlock = 0;
...
CdxParserT *CdxAtracParserOpen(CdxStreamT *stream, cdx_uint32 flags)
{
    ...
    //ret = pthread_create(&AtracParserImple->openTid, NULL, AtracOpenThread, (void*)AtracParserImple);
    //CDX_FORCE_CHECK(!ret || !AtracParserImple->openTid);
...
{
    unsigned mTaglen = 0;
    if (ff_id3v2_match(p->buf, ID3v2_EA3_MAGIC)) 
        mTaglen = ff_id3v2_tag_len(p->buf);
