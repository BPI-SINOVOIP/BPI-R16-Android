...
static cdx_int32 M3u9ParserPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    {
        CDX_LOGW("status != CDX_PSR_IDLE && status != CDX_PSR_PREFETCHED, M3u9ParserPrefetch invaild");
        m3u9Parser->mErrno = PSR_INVALID_OPERATION;
        return -1;
    }
    ...
    if(ret < 0)
    {
        ...
        if(mErrno == PSR_EOS)
        {
			...
			int ret = CdxParserPrepare(&m3u9Parser->cdxDataSource, NO_NEED_DURATION, &m3u9Parser->statusLock, &m3u9Parser->forceStop,
    ...
    //CDX_LOGD("CdxParserPrefetch pkt->pts=%lld, pkt->type=%d, pkt->length=%d", pkt->pts, pkt->type, pkt->length);
    return 0;
_exit:
	pthread_mutex_lock(&m3u9Parser->statusLock);
...
cdx_int32 M3u9ParserSeekTo(CdxParserT *parser, cdx_int64 timeUs)
{
    ...
    if(item->seqNum != m3u9Parser->curSeqNum)
    {
		...
		ret = CdxParserPrepare(&m3u9Parser->cdxDataSource, NO_NEED_DURATION, &m3u9Parser->statusLock, &m3u9Parser->forceStop,
			&m3u9Parser->child, &m3u9Parser->childStream, NULL, NULL);
		if(ret < 0)
		{
            m3u9Parser->curSeqNum = -1;
...
#if 1
    ret = CdxParserSeekTo(m3u9Parser->child, timeUs - mDurationUs);
#endif
_exit:
