...
bool PlaylistProbe(const char *data, cdx_uint32 size)
{
	...
	return startsWith(data + offset, "http://")||startsWith(data + offset, "https://")||startsWith(data + offset, "file://");
}
...
status_t PlaylistParse(const void *_data, cdx_uint32 size, Playlist **P, const char *baseURI) 
{
    ...
    {   
		...
		else
        {
            ...
            memcpy(line, &data[offset], offsetData - offset);    /*从data[offset]读到data[offsetData-1]构成一行*/
