...
enum CodecID codec_get_id(const AVCodecTag *tags, cdx_uint32 tag)
{
    cdx_int32 i;
...
cdx_int32 ebml_read_header (MatroskaDemuxContext *matroska, cdx_int32 *version)
{
    ...
    if (!id || level_up != 0 || id != EBML_ID_HEADER) 
    {
        return AVERROR_INVALIDDATA;
    }
        ...
        for (i = 0; i < matroska->num_tracks; i++) 
        {
            ...
            // This is the MS compatibility mode which stores a WAVEFORMATEX in the CodecPrivate.
				...
				*************************************/
				...
				audiotrack->tag         = tag;
				audiotrack->channels    = AV_RL16(track->codec_priv+2);
				audiotrack->samplerate  = AV_RL32(track->codec_priv+4);
				audiotrack->bitrate     = (AV_RL32(track->codec_priv+8))*8; // it is bytes per seconds, so we should x 8
				audiotrack->block_align = AV_RL16(track->codec_priv+12);
				audiotrack->bitdepth    = AV_RL16(track->codec_priv+14);
                int cbSize = AV_RL16(track->codec_priv+16);
...
cdx_int32 matroska_parse_block_header(MatroskaDemuxContext *matroska)
{
    ...
    block_time = AV_RB16(matroska->data_parse);
...
cdx_int32 matroska_parse_blockgroup (MatroskaDemuxContext *matroska)
{
    ...
    while (res == 0) 
    {
        ...
        switch (id) 
        {
            ...
            case MATROSKA_ID_BLOCKREFERENCE: // We've found a reference, so not even the first frame in the lace is a key frame.
...
...X.X..X..........X.....X.......X....X...X......X.X...X...........X.........X....X.X......X....
*/
cdx_int32 RASL_InterleaveTable[RASL_NFRAMES * RASL_NBLOCKS] = {
                ...
                cdx_int32 bitOffsetFrom, cdx_int32 numBits)
{
    ...
    cdx_int32 bofMod8, botMod8, nbMod8, eightMinusBotMod8, eightMinusBofMod8, i, iMax;
    cdx_uint8 rightInword, leftInword, *byteOffsetFrom, *byteOffsetTo,
                  alignWord, endWord;
    cdx_uint8 lmask[9] = {0, 0x80, 0xc0, 0xe0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff}; // Flawfinder: ignore
    cdx_uint8 rmask[9] = {0, 0x01, 0x03, 0x07, 0x0f, 0x1f, 0x3f, 0x7f, 0xff}; // Flawfinder: ignore

    cdx_int32 nibbleAlignFrom, nibbleAlignTo, alignCase=30; // special case variables
    ...
    if (numBits>>3 == 0) // quick and easy if we have fewer than 8 bits to align
    {
    	...
    	if (nbMod8 >= eightMinusBotMod8)  // have more extra input bits than free space in current output byte
        {
    ...
    if (bitOffsetFrom%4 == 0 && bitOffsetTo%4 == 0)	// byte-packing done here is optimized for the common case of nibble-alignment
    {
        ...
        if (nibbleAlignFrom == nibbleAlignTo) // either src and dest both byte-aligned or both half byte-aligned
            alignCase = nibbleAlignFrom == 0 ? 0 : 3;

        if (nibbleAlignFrom != nibbleAlignTo)
            alignCase = nibbleAlignFrom == 0 ? 1 : 2; // src aligned, dest half aligned
    ...
    rightInword = *(byteOffsetFrom);
    alignWord = (leftInword >> bofMod8) + (rightInword << (eightMinusBofMod8));
    *(byteOffsetTo) = (*(byteOffsetTo) & rmask[botMod8]) + (alignWord << (botMod8));
...
{
	cdx_uint8 temp[RASL_MAXCODEBYTES];     // space for swapping
    cdx_int32 nCodeBytes, nCodeBits=0;
    cdx_int32 nCodeBits_tab[4] = {RA65_NCODEBITS, RA85_NCODEBITS, RA50_NCODEBITS, RA160_NCODEBITS};
    cdx_int32 fi, fo;                    // frame in/out

    if(type < 4)
    {
        nCodeBits = nCodeBits_tab[type];
    }
    ...
    if(nCodeBits%8 == 0)
    {
        ...
        for (fi = 0; fi < RASL_NFRAMES * RASL_NBLOCKS; fi++)
        {
            ...
            //Note that when (fo == fi), the frame doesn't move, and if (fo < fi), we have swapped it already.
    ...
    else
    {
        for (fi = 0; fi < RASL_NFRAMES * RASL_NBLOCKS; fi++)
        {
            ...
            if (fo > fi) // do the swap if needed
            {
                ra_bitcopy(temp, RASL_MAXCODEBYTES, buf,  0,            fo*nCodeBits, nCodeBits);
                ra_bitcopy(buf,  ulBufSize,         buf,  fo*nCodeBits, fi*nCodeBits, nCodeBits);
                ra_bitcopy(buf,  ulBufSize,         temp, fi*nCodeBits, 0,            nCodeBits);
		    }
	    }
    }
}
...
int CdxMatroskaOpen(struct CdxMkvParser *p)
{
    ...
    if(p->hasVideo)
    {
        ...
        p->vFormat.nFrameRate 		= (cdx_uint16)videotrack->frame_rate/1000;
        //p->vFormat.nMicSecPerFrame 	= track->default_duration/1000;
        if(!p->vFormat.nFrameRate)
        {
        	if( track->default_duration)
        		p->vFormat.nFrameRate = 1000000/(track->default_duration/1000);
        }
