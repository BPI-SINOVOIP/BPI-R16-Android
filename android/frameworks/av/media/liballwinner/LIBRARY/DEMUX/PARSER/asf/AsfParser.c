...
#define DO_2BITS(bits, var, defval) \
        switch (bits & 3) \
        { \
        case 3: var = CdxStreamGetLE32(ioStream); rsize += 4; break; \
        case 2: var = CdxStreamGetLE16(ioStream); rsize += 2; break; \
        case 1: var = CdxStreamGetU8(ioStream); rsize++; break; \
...
static int32_t AsfGetValue(CdxStreamT *ioStream, int32_t type)
{
    switch (type)
    {
    case 2: return CdxStreamGetLE32(ioStream);
    case 3: return CdxStreamGetLE32(ioStream);
    case 4: return (int32_t)CdxStreamGetLE64(ioStream);
    case 5: return CdxStreamGetLE16(ioStream);
    default: return -1;
    }
}

static int32_t PsrDataAudioStream(cdx_uint8 *data, int dataSize, AsfMediaStreamT *avStream)
{
    int32_t id;
    int dataPos = 0;
    ...
    {
        codec->extradata = malloc(codec->extradata_size + FF_INPUT_BUFFER_PADDING_SIZE);
...
static int32_t PsrDataVideoStream(cdx_uint8 *data, int32_t dataLen, AsfMediaStreamT *avStream)
{
    CDX_UNUSE(dataLen);
...
/* 
        ...
        {
            uint8_t *tmpExtraData = malloc(avStream->codec.extradata_size - 10);
...
static int32_t PsrObjStream(AsfContextT *ctx, int64_t objSize)
{
...
int32_t AsfReadHeader(AsfContextT *ctx)
{
    AsfGuid guid;
    CdxStreamT *ioStream = ctx->ioStream;
    int32_t i;
    int64_t gsize;
    uint32_t bitrate[128] = {0};
    ...
    for (;;) 
    {
        ...
        if (gsize < 24)
        {
            CDX_LOGE("invalid file (%lld, %lld), gsize(%lld)", CdxStreamTell(ioStream), ctx->fileSize, gsize);            
            //goto fail; //*rtsp
        }
        ...
        {
            PsrObjStream(ctx, gsize);
        }
        else if (!memcmp(&guid, &comment_header, sizeof(AsfGuid)))
        {
            int32_t len1, len2, len3, len4, len5;
        ...
        {
            int32_t stream_count = CdxStreamGetLE16(ioStream);
            int32_t j;

            for (j = 0; j < stream_count; j++) 
            {
                int32_t flags, bitrate, stream_id;
        ...
        {
            int32_t desc_count, i;

            desc_count = CdxStreamGetLE16(ioStream);
            for (i = 0; i < desc_count; i++)
            {
                int32_t name_len,value_type,value_len;
        ...
        else if (!memcmp(&guid, &ext_stream_header, sizeof(AsfGuid))) 
        {
            ...
            stream_num = CdxStreamGetLE16(ioStream); // stream-num
        ...
        {
            //CdxStreamSeek(ioStream, gsize - 24, SEEK_CUR);
            CdxStreamSkip(ioStream, gsize - 24); //*rtsp
        }

        if (CdxStreamTell(ioStream) >= (int64_t)(ctx->fileSize - 1)) 
        {
    ...
    for (i=0; i<128; i++)
    {
        int32_t stream_num = ctx->asfid2avid[i];
        if (stream_num >= 0)
        {
            struct AsfCodecS *codec = &ctx->streams[stream_num]->codec;
            if (!codec->bit_rate)
            {
...
static int32_t AsfGetPacket(AsfContextT *ctx)
{
    CdxStreamT *ioStream = ctx->ioStream;
    uint32_t packet_length, padsize, dummyVal;
    int32_t rsize = 8;
    int32_t c, d, e, off;
    ...
    DO_2BITS(ctx->packet_flags >> 5, packet_length, ctx->packet_size);
    ...
    {
        CDX_LOGE("invalid packet_length %d at:%lld\n", packet_length, CdxStreamTell(ioStream));
        return -1;
    }
    ...
    {//multiple payloads
        ctx->packet_segsizetype = CdxStreamGetU8(ioStream); rsize++;
    ...
    ctx->packet_size_left = packet_length - padsize - rsize;
    if (packet_length < ctx->hdr.min_pktsize)
    {
        padsize += ctx->hdr.min_pktsize - packet_length;
    }
...
*/
static int32_t AsfReadFrameHeader(AsfContextT *ctx)
{
    CdxStreamT *ioStream = ctx->ioStream;
    int32_t rsize = 1;
    uint32_t num = CdxStreamGetU8(ioStream);
    ...
    if (ctx->packet_replic_size >= 8) 
    {
        ...
        rsize += ctx->packet_replic_size; // FIXME - check validity
    } 
    ...
    if (ctx->packet_flags & 0x01) 
    {
        ...
        if((int32_t)ctx->packet_frag_size > ctx->packet_size_left - rsize)
        {
    ...
    {
        ctx->packet_frag_size = ctx->packet_size_left - rsize;
    }
...
//    CDX_LOGD("packet_frag_size(0x%x), pos(%lld)", ctx->packet_frag_size, CdxStreamTell(ioStream));
    ...
    ctx->packet_size_left -= rsize;

    return 0;
}

int32_t AsfNewPacket(AsfPacketT *pkt, int32_t size)
{
    pkt->size = size;
    return 0;
}

int32_t AsfReadPacket(AsfContextT *ctx)
{
    ...
    for (;;) 
    {
        ...
        {
            int32_t ret = ctx->packet_size_left + ctx->packet_padsize;
            if (ret < 0)
            {
            ...
            if (ctx->data_object_size != (uint64_t)-1 &&
                (ctx->packet_pos - ctx->data_object_offset >= ctx->data_object_size))
            {
        ...
        if (ctx->packet_time_start == 0)
        {
            ...
            if (ctx->stream_index == (int8_t)0xff)
            {
        ...
        if (ctx->packet_replic_size == 1)
        {
            ...
            {
                ctx->packet_time_start = 0;
                CdxStreamSkip(ioStream, ctx->packet_multi_size);
...
static int32_t AsfReadClose(AsfContextT *ctx)
{
    int32_t i;
...
static void AsfBuildSimpleIndex(AsfContextT *ctx)
{
    ...
    CdxStreamSeek(ctx->ioStream, ctx->data_object_offset + ctx->data_object_size, SEEK_SET);
    ...
    for (j = 0; j < 4; j++)
    {
        ...
        {
            gsize = CdxStreamGetLE64(ctx->ioStream);
        ...
        {
            gsize = CdxStreamGetLE64(ctx->ioStream);
            CdxStreamSkip(ctx->ioStream, gsize - 24);
        }
...
static int32_t AsfSeekToTime(AsfContextT *ctx, int32_t seconds)
{
    int32_t times;
    int64_t pos;
    ...
    if (ctx->index_ptr)
    {
        int32_t *PktIdxPtr = (int32_t*)((uintptr_t)ctx->index_ptr);
        int32_t packetnum;
		int32_t seek_temp_time;
		int32_t tmp_index_number = 0;
        ...
        pos = ctx->data_offset + ctx->packet_size*(int64_t)packetnum;
        CdxStreamSeek(ctx->ioStream, pos, SEEK_SET);

		AsfResetHeader(ctx);
		AsfGetPacket(ctx);
	    AsfReadFrameHeader(ctx);
		seek_temp_time = (int32_t)(ctx->packet_frag_timestamp/1000);
		CdxStreamSeek(ctx->ioStream, pos, SEEK_SET);
    ...
    {    //we must enhance it
        ...
        startpkt = (int32_t)(ctx->nb_packets * seconds / ctx->durationMs);//from packet

        while (1)
        {
            pos = ctx->data_offset + ctx->packet_size * (int64_t)startpkt;
            if (CdxStreamSeek(ctx->ioStream, pos, SEEK_SET) < 0)
            {
                CDX_LOGW("CdxStreamSeek error! pos[%x,%x]", (uint32_t)(pos>>32), (uint32_t)pos);
                return -1;
            }
            AsfResetHeader(ctx);
            AsfGetPacket(ctx);                  
...
int AsfPsrCoreIoctrl(AsfContextT *ctx, uint32_t uCmd, uint32_t uParam)
{
    switch(uCmd)
    {
    case CDX_MEDIA_STATUS_PLAY:
    ...
    {
        int64_t seekPosUs;
        seekPosUs = *(int64_t *)(uintptr_t)uParam;
