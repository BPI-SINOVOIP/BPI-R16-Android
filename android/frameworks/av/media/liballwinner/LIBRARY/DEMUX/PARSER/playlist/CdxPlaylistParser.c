...
static int DownloadParsePlaylist(CdxPlaylistParser *playlistParser)
{
	...
	while(1)
	{
    	...
    	ret = CdxStreamRead(playlistParser->cdxStream, playlistParser->playlistBuf + readSize, 1024);
    	if(ret < 0)
    	{
	...
	status_t err;
    if ((err = PlaylistParse(playlistParser->playlistBuf, readSize, &playlist, playlistParser->playlistUrl)) != OK)
    {
...
static cdx_int32 PlaylistParserPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    {
        CDX_LOGW("status != CDX_PSR_IDLE && status != CDX_PSR_PREFETCHED, PlaylistParserPrefetch invaild");
        playlistParser->mErrno = PSR_INVALID_OPERATION;
        return -1;
    }
    ...
    if(ret < 0)
    {
        ...
        if(mErrno == PSR_EOS)
        {
			...
			ret = CdxParserPrepare(&playlistParser->cdxDataSource, NO_NEED_DURATION, &playlistParser->statusLock, &playlistParser->forceStop,
    ...
    CDX_LOGV("CdxParserPrefetch pkt->pts=%lld, pkt->type=%d, pkt->length=%d", pkt->pts, pkt->type, pkt->length);
    return 0;
_exit:
    pthread_mutex_lock(&playlistParser->statusLock);
...
CdxParserT *PlaylistParserOpen(CdxStreamT *stream, cdx_uint32 flags)
{
    ...
    //memset(playlistParser->playlistBuf, 0, playlistParser->playlistBufSize);//由每次下载m3u9文件时重置
