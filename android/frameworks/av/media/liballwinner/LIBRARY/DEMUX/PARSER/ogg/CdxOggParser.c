...
static void avcodec_get_context_defaults2(AVCodecContext *s, enum AVMediaType codec_type){
    //int flags=0;
...
static inline AVCodecContext *avcodec_alloc_context2(enum AVMediaType codec_type){
...
static int ogg_new_buf(CdxOggParser *ogg, int idx)
{
    struct ogg_stream *os = ogg->streams + idx;
...
_nextPage:
    ...
    if (idx < 0) {
        if (data_packets_seen(ogg))
            idx = ogg_replace_stream(ogg, serial, nsegs);
        else
                      ...
                      int64_t *fpos)
{
    ...
    do {
        ...
        CDX_LOGV("ogg_packet: idx=%d pstart=%d psize=%d segp=%d nsegs=%d",
        ...
        if (!os->codec) {
            if (os->header < 0) {
                ...
                if (!os->codec) {
                    CDX_LOGW("Codec not found, idx=%d", idx);
                    os->header = 0;
                    os->invalid = 1;
                    return 0;
                }
    ...
    if (os->header) {
        os->header = os->codec->header(ogg, idx);
        if (!os->header) {
            os->segp  = segp;
            os->psize = psize;
...
ogg_gptopts (CdxOggParser *ogg, int i, uint64_t gp, int64_t *dts)
{
    struct ogg_stream *os = ogg->streams + i;
    uint64_t pts = AV_NOPTS_VALUE;

    if(os->codec && os->codec->gptopts){
        pts = os->codec->gptopts(ogg, i, gp, dts);
    } else {
        pts = gp;
        if (dts)
            *dts = pts;
...
static int ogg_read_header(CdxOggParser *ogg)
{
    ...
    for (i = 0; i < ogg->nstreams; i++) {
        ...
        if (os->header < 0) {
            CDX_LOGE("Header parsing failed for stream %d", i);
...
static int64_t ogg_calc_pts(CdxOggParser *ogg, int idx, int64_t *dts)
{
    struct ogg_stream *os = ogg->streams + idx;
...
static void ogg_validate_keyframe(CdxOggParser *ogg, int idx, int pstart, int psize)
{
    struct ogg_stream *os = ogg->streams + idx;
    if (psize && ogg->streams[idx].stream->codec->codec_id == CODEC_ID_THEORA) {
...
retry:
    ...
    if (os->keyframe_seek && !(os->pflags & KEY_FRAME))
        goto retry;
    os->keyframe_seek = 0;
...
static cdx_int32 OggParserClose(CdxParserT *parser)
{
    ...
    for(i=0; i<(int)oggParser->nb_chapters; i++)
    {
    	if(oggParser->chapters[i])
    	{
            ...
            if(oggParser->chapters[i]->title)
            {
                free(oggParser->chapters[i]->title);
            }
#endif
