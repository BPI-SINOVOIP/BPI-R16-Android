...
static cdx_uint8 aviHeaders[][8] = {{ 'R', 'I', 'F', 'F', 'A', 'V', 'I', ' ' }, 
                                    { 'R', 'I', 'F', 'F', 'A', 'V', 'I', 'X' }, 
                                    { 'R', 'I', 'F', 'F', 'A', 'V', 'I', 0x19 }, 
                                    { 'O', 'N', '2', ' ', 'O', 'N', '2', 'f' },
                                    { 'R', 'I', 'F', 'F', 'A', 'M', 'V', ' ' }, 
...
_read_one_chunk:
    ...
    switch(chunk->fcc >> 16)
    {
        ...
        case cktypeDIBcompressed:
        {
			if(impl->hasVideo == 2)
            {
                if(StreamFromFOURCC(chunk->fcc) == impl->videoStreamIndexArray[0]
                    || StreamFromFOURCC(chunk->fcc) == impl->videoStreamIndexArray[1]) 
                {
                    pkt->type = CDX_MEDIA_VIDEO;
                    if(StreamFromFOURCC(chunk->fcc) == impl->videoStreamIndexArray[1])// minor_stream
                    {
                        pkt->flags |= MINOR_STREAM;
                    }
                    CDX_LOGV("xxxxxxxxx video num[%d]", StreamFromFOURCC(chunk->fcc));
                }
                else
                {
                    pkt->type = CDX_MEDIA_UNKNOWN;
			...
			else
			{
                ...
                if(StreamFromFOURCC(chunk->fcc) == impl->videoStreamIndex)
                {
                    pkt->type = CDX_MEDIA_VIDEO;
...
/*            cdx_int32   streamId; // for support switch audio stream in CdxAviParserControl, may not use...
            //check if the stream number is wanted
            streamId = StreamFromFOURCC(chunk->fcc);
        ...
        case cktypeSUBtext: //TODO
        {
            //check if the stream number is wanted
            if(StreamFromFOURCC(chunk->fcc) == impl->subTitleStreamIndex)
            {
 ...
 //                       chunk->fcc, tmpAviPsr->CurChunkInfo.chk_type, cdx_pkt->pkt_length);
    ...
    if(pkt->type == CDX_MEDIA_VIDEO) //TODO: PTS
    {
		...
		if(impl->hasVideo == 2)
		{
			...
			{
				if(impl->videoStreamIndexArray[(cdx_int32)vSteamIndex] == StreamFromFOURCC(chunk->fcc))
				{
