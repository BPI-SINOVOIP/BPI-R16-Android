...
void ClearExtraDataContainer(CdxHlsParser *hlsParser)
{
...
void GetExtraDataContainer(CdxHlsParser *hlsParser, CdxStreamT *stream, CdxParserT *parser)
{
	...
	ClearExtraDataContainer(hlsParser);
        ...
        if(cipherReference != hlsParser->u.media.cipherReference)
        {
            ...
            if(!strcasecmp(cipherMethod->mData, "AES-128"))
            {
                ...
                if(findString(&cipherReference->itemMeta, "cipher-iv", &cipherIv))
                {
                    ...
                    for (i = 0; i < 16; ++i)
                    {
                        char c1 = tolower(cipherIv->mData[2 + 2 * i]);
                        char c2 = tolower(cipherIv->mData[3 + 2 * i]);
                        ...
                        hlsParser->aesCipherInf.iv[i] = nibble1 << 4 | nibble2;
                    }
                }
                else
                {
...
int DownloadParseM3u8(CdxHlsParser *hlsParser)
{
	...
	while(1)
	{
    	...
    	ret = CdxStreamRead(hlsParser->cdxStream, hlsParser->m3u8Buf + m3u8ReadSize, 1024);
    	if(ret < 0)
    	{
	...
	int64_t playlistFetchTimeUs = CdxGetNowUs();
...
#if 0            
			if(hlsParser->u.media.refreshState != THIRD_UNCHANGED_RELOAD_ATTEMPT)
			{
...
int TimeToRefreshPlaylist(CdxHlsParser *hlsParser)
{
	...
	if(!findInt32(&playlist->mMeta,"target-duration", &targetDurationSecs))
	{
	...
	int64_t minPlaylistAgeUs = targetDurationUs / 2;
	switch (hlsParser->u.media.refreshState)
	{
		case INITIAL_MINIMUM_RELOAD_DELAY:
		{
            ...
            minPlaylistAgeUs = item->u.mediaItemAttribute.durationUs;
            break;   
		}
	...
	return hlsParser->u.media.mLastPlaylistFetchTimeUs + minPlaylistAgeUs <= CdxGetNowUs();
}
...
CdxParserT *HlsParserOpen(CdxStreamT *stream, cdx_uint32 flags)
{
	...
	hlsParser->m3u8BufSize = 1024*1024;
