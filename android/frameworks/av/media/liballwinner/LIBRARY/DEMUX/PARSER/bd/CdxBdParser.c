...
static int ExtOpen(CdxStreamT *stream, char *fileUrl)
{
    ...
    CDX_LOGI("open file success, fd(%d)", (int)param.outHdr);
    return (int)(intptr_t)param.outHdr;
}
...
static int OpenFile(CdxBdParser *bdParser, char *fileUrl)
{
    ...
    bdParser->fd = fd;
    CDX_LOGI("dup a new fd(%d)", bdParser->fd);
    return 0;
}
...
static int ReadFile(CdxBdParser *bdParser)
{
    ...
    {
        CDX_LOGW("read file fail, fd(%d) fileLen(%d), BufSize(%d)", bdParser->fd, fileLen, bdParser->fileBufSize);
        close(bdParser->fd);
        bdParser->fd = -1;
        return -1;
    }
...
static cdx_int32 BdParserPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
	...
	if((bdParser->streamPts[2] < bdParser->streamPts[1])&&(bdParser->streamPts[2] < bdParser->streamPts[0]))
	{
    ...
    if(ret < 0)
    {
        ...
        if(mErrno == PSR_EOS)
        {
            ...
            else if(bdParser->prefetchType == 0)
            {
				...
				ret = CdxStreamOpen(&bdParser->cdxDataSource, &bdParser->statusLock, &bdParser->forceStop, &cdxStream, NULL);
                close(fd);
                if(ret < 0)
                {
                    if(!bdParser->forceStop)
                ...
                pthread_mutex_lock(&bdParser->statusLock);
                ret = CdxParserControl(bdParser->child[bdParser->prefetchType], CDX_PSR_CMD_REPLACE_STREAM, (void *)cdxStream);
                pthread_mutex_unlock(&bdParser->statusLock);
                if(ret < 0)
                {
                    bdParser->mErrno = PSR_UNKNOWN_ERR;
            ...
            else
            {
                ...
                if(bdParser->curSubPlayItemIndex >= bdParser->curPlaylist->subPathE->number_of_SubPlayItems)
                {
                    m++;
                    bdParser->prefetchType = (bdParser->prefetchType + 1)%3;
                    goto next_child;
                ...
                subPlayItem = bdParser->curPlaylist->subPathE->subPlayItems + bdParser->curSubPlayItemIndex;
...
//    CDX_LOGD("bdParser->prefetchType = %d", bdParser->prefetchType);
    ...
    if(bdParser->child[1] && bdParser->streamPts[0] + 2000000LL < bdParser->streamPts[1])
    {
        CDX_LOGW("maybe BD_BASE pts jump");
        bdParser->streamPts[1] = 0;
    }
    //CDX_LOGD("streamPts[0] = %llu, streamPts[1] = %llu, streamPts[2] = %llu, bdParser->streamPts[3]=%llu", bdParser->streamPts[0], bdParser->streamPts[1], bdParser->streamPts[2], bdParser->streamPts[3]);
...
cdx_int32 BdParserSeekTo(CdxParserT *parser, cdx_int64  timeUs)
{
    ...
    if(i != bdParser->curPlayItemIndex)
    {
        ...
        pthread_mutex_lock(&bdParser->statusLock);
        ret = CdxParserControl(bdParser->child[0], CDX_PSR_CMD_REPLACE_STREAM, (void *)cdxStream);
        pthread_mutex_unlock(&bdParser->statusLock);
        if(ret < 0)
        {
        ...
        if(bdParser->child[1])
        {
            ...
            pthread_mutex_lock(&bdParser->statusLock);
            ret = CdxParserControl(bdParser->child[1], CDX_PSR_CMD_REPLACE_STREAM, (void *)cdxStream);
            pthread_mutex_unlock(&bdParser->statusLock);
            if(ret < 0)
            {
                CDX_LOGE("CDX_PSR_CMD_REPLACE_STREAM fail");
...
//    bdParser->streamCtl.param = &bdParser->streamSeekPos;
    //CDX_LOGD("bdParser->streamSeekPos=%lld, epfine->SPN_EP_fine=%u", bdParser->streamSeekPos, epfine->SPN_EP_fine);
    ret = CdxParserControl(bdParser->child[0], CDX_PSR_CMD_STREAM_SEEK, &bdParser->streamSeekPos);
    if(ret < 0)
    {
        bdParser->mErrno = PSR_UNKNOWN_ERR;
        goto _exit;
    ...
    if(bdParser->child[1])
    {
        ...
        {
            stream = clip->ps->streams + i;
            CDX_LOGD("clip=%p, clip->ps=%p, clip->ps->streams=%p, stream = %p", clip, clip->ps, clip->ps->streams, stream);
            if(stream->mediaType == TYPE_VIDEO)
            {
                break;
            }
...
//        bdParser->streamCtl.param = &bdParser->streamSeekPos;
        ...
        //CDX_LOGD("bdParser->streamSeekPos=%lld, epfine->SPN_EP_fine=%u", bdParser->streamSeekPos, epfine->SPN_EP_fine);
        ret = CdxParserControl(bdParser->child[1], CDX_PSR_CMD_STREAM_SEEK, &bdParser->streamSeekPos);
        if(ret < 0)
        {
            bdParser->mErrno = PSR_UNKNOWN_ERR;
            goto _exit;
...
cdx_int64 parsePlayItem(PlayItem *playItem, cdx_uint8 *data)
{
    ...
    CDX_LOGV("ClipName = %s, ref_to_STC_id = %d, IN_time = 0x%x, OUT_time = 0x%x, durationUs = %lld", playItem->clip.ClipName, playItem->ref_to_STC_id, playItem->IN_time, playItem->OUT_time, playItem->durationUs);
...
cdx_int64 parseMplsRoughly(CdxBdParser *bdParser, Playlist *playlist, cdx_uint8 *data)
{
    cdx_uint8 *cur = data;
    ...
    playlist->playItems = (PlayItem *)CdxMalloc(playlist->numer_of_PlayItems * sizeof(PlayItem));
...
int parseCPI(Clip *clip, cdx_uint8 *data)
{
    ...
    ClearCPI(clip);
    clip->epMapForOneStream = CdxMalloc(clip->number_of_stream_PID_entries * sizeof(EPMapForOneStream));
    ...
    for(i = 0; i < clip->number_of_stream_PID_entries; i++)
    {
        ...
        epMapForOneStream->coarse = CdxMalloc(epMapForOneStream->number_of_EP_coarse_entries * sizeof(EPMapCoarse));
        epMapForOneStream->fine = CdxMalloc(epMapForOneStream->number_of_EP_fine_entries * sizeof(EPMapFine));
        ...
        for(j = 0; j < epMapForOneStream->number_of_EP_fine_entries; j++)
        {
            ...
            epMapFine->PTS_EP_fine = (epMapCoarse->PTS_EP_coarse << 18) | (epMapFine->PTS_EP_fine << 8);
            epMapFine->SPN_EP_fine |= epMapCoarse->SPN_EP_coarse & 0xfffe0000;

            CDX_LOGV("epMapFine->PTS_EP_fine(%u), epMapFine->SPN_EP_fine(%u)", epMapFine->PTS_EP_fine, epMapFine->SPN_EP_fine);
        }
    ...
    for(i = 0; i < clip->number_of_program_sequences; i++)
    {
        ...
        for(j = 0; j < ps->number_of_streams_in_ps; j++)
        {
               ...
               stream->coding_type == 0x20)//从流
            {
                ...
                {
                    CDX_LOGD("MVC Dependent view video(%d)", ps->number_of_streams_in_ps);
                    break;//只取了一个从流
                }
            }
        ...
        if(ID1 == 2 && ID2 == 2)
        {
            ...
            for(j = 0; j < number_of_SubPath_extensions; j++)
            {
                ...
                if(SubPath_type == 8)
                {
                    ...
                    playlist->subPathE->subPlayItems = (SubPlayItem *)CdxMalloc(playlist->subPathE->number_of_SubPlayItems * sizeof(SubPlayItem));
                    memset(playlist->subPathE->subPlayItems, 0x00, playlist->subPathE->number_of_SubPlayItems * sizeof(SubPlayItem));
                    int k;
                    SubPlayItem *subPlayItem;
...
int parseMpls(CdxBdParser *bdParser, Playlist *playlist, cdx_uint8 *data)
{
    ...
    for(i = 0; i < playlist->numer_of_SubPaths; i++)
    {
        ...
        else
        {
            ...
            playlist->subPath->subPlayItems = (SubPlayItem *)CdxMalloc(playlist->subPath->number_of_SubPlayItems * sizeof(SubPlayItem));
            
            memset(playlist->subPath->subPlayItems, 0x00, playlist->subPath->number_of_SubPlayItems * sizeof(SubPlayItem));
            int j;
            SubPlayItem *subPlayItem;
...
int processMpls(CdxBdParser *bdParser, Playlist *playlist)
{
    ...
    if(bdParser->playlistIsAppointed)
    {
        ...
        while(1)
        {
            ret = CdxStreamRead(bdParser->cdxStream, bdParser->fileBuf + readSize, (cdx_int32)size - readSize);
            if(ret < 0)
            {
...
//    char *tmp = strstr(bdParser->originalUrl, "BDMV/");
    ...
    {
        bdParser->curPlaylist = CdxListEntry(bdParser->playlists.head, typeof(*bdParser->curPlaylist), node);
    }
    ...
    {
        playItem = playlist->playItems + i;
        if(prePlayItem && !memcmp(prePlayItem->clip.ClipName, playItem->clip.ClipName, 5))
        {
    ...
    if(playlist->subPathE)
    {
        ...
        {
            subPlayItem = playlist->subPathE->subPlayItems + i;
            if(preSubPlayItem && !memcmp(preSubPlayItem->clip.ClipName, subPlayItem->clip.ClipName, 5))
            {
...
//    bdParser->cdxDataSource.extraDataType = EXTRA_DATA_APPOINTED_TS;
	...
	ret = CdxParserPrepare(&bdParser->cdxDataSource, flags, &bdParser->statusLock, &bdParser->forceStop,
    ...
    for(i = 0; i < clip->ps->number_of_streams_in_ps; i++)
    {
		...
		else if(stream->mediaType == TYPE_AUDIO && k < AUDIO_STREAM_LIMIT)
		{
			if(stream->metadata)
			{
				...
				{
					unsigned sampling_frequency = ((AudioMetaData *)stream->metadata)->samplingFrequency;
    ...
    if(bdParser->child[1])
    {
        ...
        //cdxProgram->videoNum += cdxProgram1->videoNum;//主从流认为是一个，否则demux 的setMediaInfo会有问题
        CDX_CHECK(cdxProgram1->videoNum == 1);
        /*
        for(i = 0; i < cdxProgram1->videoNum && cdxProgram->videoNum < ; i++)
        {
            memcpy(&cdxProgram->video[cdxProgram->videoNum], &cdxProgram1->video[i], sizeof(VideoStreamInfo));
            cdxProgram->videoNum++;
        }
        */
    }
    ...
    {
        CDX_LOGW("cdxProgram->audioNum(%d), playlist->audioCount(%d)", cdxProgram->audioNum, playlist->audioCount);
    }
    if(cdxProgram->videoNum != playlist->videoCount)
    {
        CDX_LOGW("cdxProgram->videoNum(%d), playlist->videoCount(%d)", cdxProgram->videoNum, playlist->videoCount);
    }
    if(cdxProgram->subtitleNum != playlist->subsCount)
    {
        CDX_LOGW("cdxProgram->subtitleNum(%d), playlist->subsCount(%d)", cdxProgram->subtitleNum, playlist->subsCount);
    }
...
CdxParserT *BdParserOpen(CdxStreamT *stream, cdx_uint32 flags)
{
    ...
    bdParser->flags = flags & (DISABLE_VIDEO | DISABLE_AUDIO | DISABLE_SUBTITLE | MUTIL_AUDIO);
