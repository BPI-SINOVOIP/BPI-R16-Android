#define LOG_TAG "RawStreamParser"
...
static cdx_int32 AwRawStreamParserGetMediaInfo(CdxParserT *parser, CdxMediaInfoT *pMediaInfo)
{
    ...
    pMediaInfo->program[0].video[0].pCodecSpecificData = rawStreamParser->tempVideoInfo.pCodecSpecificData;
    pMediaInfo->program[0].video[0].nCodecSpecificDataLen = rawStreamParser->tempVideoInfo.nCodecSpecificDataLen;
    return 0;
}
...
static cdx_int32 AwRawStreamParserPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
	RawStreamParser *rawStreamParser = (RawStreamParser*)parser;
	int nLen, ret;
    if(rawStreamParser->status != CDX_PSR_IDLE && rawStreamParser->status != CDX_PSR_PREFETCHED)
    {
...
{ 
    ...
    else
    {
        ...
        memcpy(pkt->ringBuf, rawStreamParser->pRawStreamBuf + pkt->buflen, pkt->length - pkt->buflen);
    }
...
static int AwRawStreamProbeH265(cdx_char *p)
{
	...
	if(nNuhLayerId != 0)
	{
		logd(" Maybe this is not h265 stream. nNuhLayerId() != 0");
		return 0;
	}
