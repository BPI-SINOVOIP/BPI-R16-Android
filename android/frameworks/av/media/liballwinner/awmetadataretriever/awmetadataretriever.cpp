...
static void ConvertMb32420ToNv21Y(char* pSrc,char* pDst,int nWidth, int nHeight)
{
	int nMbWidth = 0;
	int nMbHeight = 0;
	int i = 0;
...
static void ConvertMb32420ToYv12C(char* pSrc,char* pDstU, char*pDstV,int nPicWidth, int nPicHeight)
{
	int nMbWidth = 0;
	int nMbHeight = 0;
	int i = 0;
...
static void map32x32_to_yuv_Y(unsigned char* srcY, unsigned char* tarY,unsigned int coded_width,unsigned int coded_height)
{
    ...
    int nWidthMatchFlag;
    unsigned int nLeftValidLine;  //in the bottom macroblock(32*32), the valid line is < 32.
	ptr = srcY;
...
//#if (defined(__CHIP_VERSION_F23) || defined(__CHIP_VERSION_F51) || defined(__CHIP_VERSION_F50) || defined(__CHIP_VERSION_F67)) //a31 temp change to this, because of vertical screen
//    int dst_stride = mode==0 ? (coded_width + 15) & (~15) : coded_width;
//#elif (defined(__CHIP_VERSION_F33))
//  #if (CEDARX_ANDROID_VERSION<=7)
//    int dst_stride = mode==0 ? (coded_width + 15) & (~15) : coded_width;
//  #else
//    int dst_stride = mode==0 ? (coded_width + 7) & (~7) : coded_width;
//  #endif
//#else
...
static int transformPicture(VideoPicture* pPicture, VideoFrame* pVideoFrame)
{
    ...
    unsigned char*   pClip;
    int convertflag = 0;
    void* pPictureOut = NULL;
	unsigned char* out_y = NULL;
	unsigned char* out_u = NULL;
	unsigned char* out_v = NULL;
...
//			unsigned char out_v = NULL;
			...
			nHeight16Align = (pPicture->nHeight+15) &~ 15;
			nMemSizeY = pPicture->nLineStride*nHeight16Align;
			nMemSizeC = nMemSizeY>>2;

			pPictureOut = (void*)malloc(nMemSizeY * 3/2);
			out_y = (unsigned char*)pPictureOut;
			out_u = out_y + nMemSizeY;
			out_v = out_u + nMemSizeY/4;


			map32x32_to_yuv_Y((unsigned char*)pPicture->pData0, (unsigned char*)out_y, pPicture->nWidth, pPicture->nHeight);
			map32x32_to_yuv_C(0,(unsigned char*)pPicture->pData1, (unsigned char*)out_u, (unsigned char*)out_v, pPicture->nWidth / 2, pPicture->nHeight / 2);
    ...
    if(convertflag == 1)
    {
		MemAdapterFlushCache(out_y, pPicture->nLineStride*pPicture->nHeight);
		MemAdapterFlushCache(out_u, pPicture->nLineStride*pPicture->nHeight/4);
		MemAdapterFlushCache(out_v, pPicture->nLineStride*pPicture->nHeight/4);
    }else
    {
    ...
    if(convertflag == 1)
    	pSrcY = (unsigned char*)out_y + pPicture->nTopOffset * pPicture->nLineStride + pPicture->nLeftOffset;
    else
    	pSrcY = (unsigned char*)pPicture->pData0 + pPicture->nTopOffset * pPicture->nLineStride + pPicture->nLeftOffset;

    if(convertflag == 1) //default YV12.
    {
    	pSrcU = (unsigned char*)out_u + (pPicture->nTopOffset/2) * (pPicture->nLineStride/2) + pPicture->nLeftOffset/2;
    	pSrcV = (unsigned char*)out_v + (pPicture->nTopOffset/2) * (pPicture->nLineStride/2) + pPicture->nLeftOffset/2;
    }else
    {
        if(pPicture->ePixelFormat== PIXEL_FORMAT_YV12)
        {
...
static int transfromId3Info(String8* mStringId3, cdx_uint8* pData, cdx_int32 nSize, cdx_int32 nEncodeTpye,cdx_int32 flag)
{
	...
	for(idx = 0; idx < kNumMapEntries; idx++)
	{
		if(flag == kMap[idx].from)
		{
			...
			{
				detector->addTag(kMap[idx].name, (const char*)mStringId3->string());
			}
		}
	}
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (cdx_int32 j = 0; j < kNumMapEntries; ++j) {
                if (kMap[j].name && !strcmp(kMap[j].name, name)) {
					...
					mStringId3->setTo("");
					mStringId3->setTo(value,strlen(value));
                }
            }
        }
    }   
    ...
    if(0x00 == nEncodeTpye)
    {
    	...
    	logd("ucnv_convert, nEncodeTpye 0x%02x, using GBK", nEncodeTpye);
	    ...
	    logd("convert gbk to utf-8 src = %s, dst = %s\n", gbkSrc, utf8Dst);
