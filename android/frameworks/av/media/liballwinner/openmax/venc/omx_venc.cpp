...
//for 4k recorder @30fps
	...
	m_sInPortDef.nBufferCountMin 				 = 4;
...
//for 4k recorder @30fps	
#if ((CONFIG_CHIP == OPTION_CHIP_1639)&&(CONFIG_PRODUCT == OPTION_PRODUCT_TVBOX))
    m_sOutPortDef.nBufferCountMin 			= 6;
    m_sOutPortDef.nBufferCountActual 		= 6;
#else
	    ...
	    default:
	    {
			...
			switch((VIDENC_CUSTOM_INDEX)paramIndex)
	    	{
				...
				case VideoEncodeCustomParamEnableAndroidNativeBuffers:
				{
					...
					OMX_U32  index = ((EnableAndroidNativeBuffersParams*)paramData)->nPortIndex;
...
int openVencDriver(omx_venc* pVenc)
{
	...
	if(pVenc->m_vencCodecType == VENC_CODEC_H264)
	{
		...
		logd("profile-venc=%d, profile-omx=%d, frame_rate:%d, bit_rate:%d, eColorFormat:%08x\n",pVenc->m_vencH264Param.sProfileLevel.nProfile,\
			pVenc->m_sH264Type.eProfile,pVenc->m_vencH264Param.nFramerate,pVenc->m_vencH264Param.nBitrate,pVenc->m_sInPortFormat.eColorFormat);
	...
	{
		if(pVenc->m_sInPortFormat.eColorFormat == OMX_COLOR_FormatAndroidOpaque && pVenc->mIsFromCts)
		{
	...
	logd("omx_venc base_config info: src_wxh:%dx%d, dis_wxh:%dx%d, stride:%d\n", (int)pVenc->m_sInPortDef.format.video.nFrameWidth,\
		(int)pVenc->m_sInPortDef.format.video.nFrameHeight,(int)pVenc->m_sOutPortDef.format.video.nFrameWidth,\
		(int)pVenc->m_sOutPortDef.format.video.nFrameHeight,pVenc->m_sInPortDef.format.video.nStride);
...
#if (GPU_TYPE_MALI == 0)
	if(pVenc->m_sInPortFormat.eColorFormat == OMX_COLOR_FormatAndroidOpaque)
