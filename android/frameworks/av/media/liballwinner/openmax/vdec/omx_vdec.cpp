    ...
    switch(paramIndex)
    {
	    case OMX_IndexParamPortDefinition:
	    {
            ...
            if (((OMX_PARAM_PORTDEFINITIONTYPE *)(paramData))->nPortIndex == m_sInPortDef.nPortIndex)
            {
				...
				//* TODO for any new IC
				OMX_PARAM_PORTDEFINITIONTYPE * para = (OMX_PARAM_PORTDEFINITIONTYPE *)paramData;
				...
				{
					logw("ChipId: %d, H265 decode do not support over 1080p, but this video is %lux%lu", 
						VeChipId(), para->format.video.nFrameWidth, para->format.video.nFrameHeight);
					return OMX_ErrorUnsupportedSetting;
				}
				
				//* H3s/H2 do not support 4k decode
				if ((VeChipId() == CHIP_H3s || VeChipId() == CHIP_H2)
					&& ((para->format.video.nFrameWidth > 1920)
						|| (para->format.video.nFrameHeight > 1088)))
				{
                ...
                // the folllowing values are not right, we should get offset in first time getVideoPicture
				mVideoRect.nTop    = 0;
				mVideoRect.nLeft   = 0;
				mVideoRect.nWidth  = m_sInPortDef.format.video.nFrameWidth;  
				mVideoRect.nHeight = m_sInPortDef.format.video.nFrameHeight; 
            }
	        else if (((OMX_PARAM_PORTDEFINITIONTYPE *)(paramData))->nPortIndex == m_sOutPortDef.nPortIndex)
	        {
    ...
    while (1)
    {
        	...
        	(pSelf->mResolutionChangeFlag == OMX_FALSE))
        {
        	...
        	if(ValidPictureNum(pSelf->m_decoder,0))
        	{
            	...
            	if((OMX_U32)width_align != pSelf->m_sOutPortDef.format.video.nFrameWidth 
					|| (OMX_U32)height_align  != pSelf->m_sOutPortDef.format.video.nFrameHeight
            	...
            	{
            		logw(" port setting changed -- old info : widht = %d, height = %d, mVideoRect: %d, %d, %d, %d ",
					...
					logw(" port setting changed -- new info : widht = %d, height = %d, mVideoRect: %d, %d, %d, %d ",
            	...
            	MemAdapterFlushCache((void*)picture->pData0, picture->nLineStride * picture->nHeight*3/2);
            	...
            	if(pSelf->m_useAndroidBuffer == OMX_FALSE)
            	{
					...
					mOutPutBufferInfo.nWidth  = pSelf->m_sOutPortDef.format.video.nFrameWidth;
					mOutPutBufferInfo.nHeight = pSelf->m_sOutPortDef.format.video.nFrameHeight;
