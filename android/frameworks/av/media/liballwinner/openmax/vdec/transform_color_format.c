...
typedef struct ScalerParameter
{
	...
	void *addr_y_in;
	void *addr_c_in;
	uintptr_t addr_y_out;
	uintptr_t addr_u_out;
	uintptr_t addr_v_out;
}ScalerParameter;
...
void TransformMB32ToYV12(VideoPicture* pict, OutPutBufferInfo* pOutPutBufferInfo)
{
	...
	cdx_scaler_para.addr_y_out = (uintptr_t)pOutPutBufferInfo->pAddr;
...
#if(CONFIG_CHIP == OPTION_CHIP_1673)
    nDstBufWidth = (pOutPutBufferInfo->nWidth + 31)&~31;   //on chip-1673, gpu is 32pixel align
#else
    nDstBufWidth = (pOutPutBufferInfo->nWidth + 15)&~15;   //other chip, gpu is 16pixel align
#endif
...
void TransformYV12ToYV12Soft(VideoPicture* pict, OutPutBufferInfo* pOutPutBufferInfo)
{
    ...
    copyWidth  = pict->nRightOffset - pict->nLeftOffset;
#if(CONFIG_CHIP == OPTION_CHIP_1673)
    dstWidth   = (pOutPutBufferInfo->nWidth + 31)&~31;   //on chip-1673, gpu is 32pixel align
#else
    dstWidth   = (pOutPutBufferInfo->nWidth + 15)&~15;   //other chip, gpu is 16pixel align
#endif
    dstHeight  = pOutPutBufferInfo->nHeight;
