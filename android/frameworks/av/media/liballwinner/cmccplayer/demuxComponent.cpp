...
static const int DEFAULT_CACHE_START_PLAY_SIZE = 128*1024; //512*1024
        ...
        case PARSER_NOTIFY_VIDEO_STREAM_CHANGE:
		{
            ...
            if(demux->cacheThreadId != 0)
            {
                ...
                setMessage(&msg, 
                          ...
                          (unsigned int)&demux->semStreamChange,//* sem
                           0);                                  //* replyVuale
...

{
	...
	setMessage(&msg, 
               ...
               (unsigned int)pHTTPServer);                  //* params[5] = pHTTPServer;
...
int DemuxCompSetStreamSource(DemuxComp* d, const char* pUri)
{
    ...
    setMessage(&msg, 
               ...
               (unsigned int)pUri);                 		//* params[3] = uri of IStreamSource.
    AwMessageQueuePostMessage(demux->mq, &msg);
    SemTimedWait(&demux->semSetDataSource, -1);
...
/*
			...
			*/
            ...
            {   
                ...
                if(demux->mLivemode == 1 || demux->mLivemode == 2)
                    sprintf(cmccLog, "[info][%s %s %d]playmode: live, duration: %lldms", LOG_TAG, __FUNCTION__, __LINE__, demux->mediaInfo.nDurationMs);
                else
                    sprintf(cmccLog, "[info][%s %s %d]playmode: vod, duration: %lldms", LOG_TAG, __FUNCTION__, __LINE__, demux->mediaInfo.nDurationMs);
        ...
        else if(msg.messageId == DEMUX_COMMAND_RESET_URL)
        {
            ...
            //if(demux->cacheThreadId != 0)
            {
                ...
                setMessage(&newMsg, DEMUX_COMMAND_RESET_URL, 
                           (unsigned int)&demux->semCache, 
                           (unsigned int)&demux->nCacheReply,
                           (unsigned int)nSeekTimeMs);
                AwMessageQueuePostMessage(demux->mqCache, &newMsg);
...
reopen:
			...
			do {
            	...
            	if(ret < 0) 
            	{
                    ...
                    unsigned int statusCode = demux->statusCode;    
...
static int PlayerBufferOverflow(Player* p)
{
    ...
    if(PlayerHasVideo(p))
    {
        ...
        logv("picNum = %d, frameDuration = %d, dataSize = %d, bitrate = %d, bVideoOverflow = %d",
