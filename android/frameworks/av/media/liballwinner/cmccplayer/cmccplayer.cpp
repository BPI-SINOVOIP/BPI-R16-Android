...
#endif
{
	...
	setMessage(&msg, 
			   ...
			   (unsigned int)(mHTTPService.get()));     //* params[5] = mHTTPService.get();
...
status_t CmccPlayer::setDataSource(int fd, int64_t offset, int64_t length)
{
    ...
    setMessage(&msg, 
               ...
               fd,                                                  //* params[3] = fd.
               (unsigned int)(offset>>32),              //* params[4] = high 32 bits of offset.
               (unsigned int)(offset & 0xffffffff),     //* params[5] = low 32 bits of offset.
               (unsigned int)(length>>32),              //* params[6] = high 32 bits of length.
               (unsigned int)(length & 0xffffffff));    //* params[7] = low 32 bits of length.
    AwMessageQueuePostMessage(mMessageQueue, &msg);
    SemTimedWait(&mSemSetDataSource, -1);
...
status_t CmccPlayer::setDataSource(const sp<IStreamSource> &source)
{
	...
	char str[128];
	sprintf(str, "customer://%u%s", (unsigned int)stream, suffix);
    //* send a set data source message.
    setMessage(&msg, 
               AWPLAYER_COMMAND_SET_SOURCE,         //* message id.
               (unsigned int)&mSemSetDataSource,    //* params[0] = &mSemSetDataSource.
               ...
               (unsigned int)str);         			//* params[3] = uri of IStreamSource.
    AwMessageQueuePostMessage(mMessageQueue, &msg);
    SemTimedWait(&mSemSetDataSource, -1);
    return (status_t)mSetDataSourceReply;
}
...
status_t CmccPlayer::setVideoSurfaceTexture(const sp<IGraphicBufferProducer>& bufferProducer)
{
    ...
    setMessage(&msg, 
               ...
               (unsigned int)&mSemSetSurface,       //* params[0] = &mSemSetSurface.
               (unsigned int)&mSetSurfaceReply,     //* params[1] = &mSetSurfaceReply.
               (unsigned int)bufferProducer.get()); //* params[2] = pointer to ISurfaceTexture.
    AwMessageQueuePostMessage(mMessageQueue, &msg);
...
status_t CmccPlayer::setVideoSurfaceTexture(const sp<ISurfaceTexture> &surfaceTexture)
{
    ...
    setMessage(&msg, 
               ...
               (unsigned int)&mSemSetSurface,       //* params[0] = &mSemSetSurface.
               (unsigned int)&mSetSurfaceReply,     //* params[1] = &mSetSurfaceReply.
               (unsigned int)surfaceTexture.get()); //* params[2] = pointer to ISurfaceTexture.
    AwMessageQueuePostMessage(mMessageQueue, &msg);
    SemTimedWait(&mSemSetSurface, -1);
...
void CmccPlayer::setAudioSink(const sp<AudioSink> &audioSink)
{
    ...
    setMessage(&msg, 
               ...
               (unsigned int)&mSemSetAudioSink,     //* params[0] = &mSemSetAudioSink.
               (unsigned int)&mSetAudioSinkReply,   //* params[1] = &mSetAudioSinkReply.
               (unsigned int)audioSink.get());      //* params[2] = pointer to AudioSink.
    AwMessageQueuePostMessage(mMessageQueue, &msg);
...
status_t CmccPlayer::prepareAsync()
{
	...
	if(OK == mPrepareReply && mLogRecorder != NULL)
	{
		...
		char cmccLog[4096] = "";
		sprintf(cmccLog, "[info][%s %s %d]Playback is ready, spend time: %lldms", LOG_TAG, __FUNCTION__, __LINE__, (endTimeMs - startTimeMs));
		logi("AwLogRecorderRecord %s.", cmccLog);
		AwLogRecorderRecord(mLogRecorder, cmccLog);
	}
    ...
    while(1)
    {
        ...
        if(msg.messageId == AWPLAYER_COMMAND_SET_SOURCE)
        {
            ...
            else
            {
				...
				int handle, ret;
				ret = sscanf(uri, "customer://%d", &handle);
				if (ret != 1)
				{
    ...
    switch(messageId)
    {
        ...
        case AWPLAYER_MESSAGE_DEMUX_BUFFER_END:
        {
            ...
            if(mLogRecorder != NULL)
            {
                ...
                char cmccLog[4096] = "";
                sprintf(cmccLog, "[info][%s %s %d]buffering end, lasting time: %lldms", LOG_TAG, __FUNCTION__, __LINE__, (bufferEndTimeMs - mBufferTimeMs));
                logi("AwLogRecorderRecord %s.", cmccLog);
                AwLogRecorderRecord(mLogRecorder, cmccLog);
            }
        ...
        case AWPLAYER_MESSAGE_PLAYER_FIRST_PICTURE:
        {
            ...
            if(mLogRecorder != NULL)
            {
                ...
                char cmccLog[4096] = "";
                sprintf(cmccLog, "[info][%s %s %d]show the first video frame, spend time: %lldms", LOG_TAG, __FUNCTION__, __LINE__, (renderTimeMs - mPlayTimeMs));
                logi("AwLogRecorderRecord %s.", cmccLog);
                AwLogRecorderRecord(mLogRecorder, cmccLog);
            }
            break;
        ...
        {
            Parcel parcel;
            unsigned int  nSubtitleId   = (unsigned int)((unsigned int*)param)[0];
            SubtitleItem* pSubtitleItem = (SubtitleItem*)((unsigned int*)param)[1];
