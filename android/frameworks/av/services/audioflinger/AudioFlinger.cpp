...
{
    ALOGW("AudioFlinger::AudioFlinger() Enter...");
...
/* used to access input devices internally from input router */
AudioFlinger::AudioHwDevice* AudioFlinger::findSuitableHwDev_byName(
        ...
        audio_devices_t devices)
{
    for (size_t i = 0; i < mAudioHwDevs.size(); i++) {
        AudioHwDevice *audioHwDevice = mAudioHwDevs.valueAt(i);
        audio_hw_device_t *dev = audioHwDevice->hwDevice();
        ALOGV("findSuitableHwDev_byName : module name %s", dev->common.module->name);
        if (strcmp(dev->common.module->name, module_name) == 0)
            return audioHwDevice;
    }
    return NULL;
}
        ...
        status_t *status)
{
    ...
    if (audio_is_linear_pcm(format) && ((format != AUDIO_FORMAT_PCM_16_BIT)&&(format != AUDIO_FORMAT_PCM_DIR_16_BIT)&&(format != AUDIO_FORMAT_PCM_24BIT)&&(format != AUDIO_FORMAT_RAW_AC3)&&(format != AUDIO_FORMAT_RAW_DTS))) {
...
status_t AudioFlinger::setParameters(audio_io_handle_t ioHandle, const String8& keyValuePairs)
{
    ...
    if (ioHandle == 0) {
        AudioFlinger::RoutingThread::checkSuspendFlag(this, keyValuePairs);

        RoutingThread *rt_thread = checkRoutingThread_l(ioHandle);
        if (rt_thread != 0)
        {
            ALOGV("set params for routingthread");
            if (rt_thread->setParameters(keyValuePairs) == NO_ERROR)
            {
                ALOGV("setParameters(): rt return no error");
                return NO_ERROR;
            }
        }
    ...
    ALOGV("setParameters(): return BAD_VALUE");
    return BAD_VALUE;
}
...
// achieve minimal end to end latency

// AudioTrack::createTrack_l() OUTPUT equivalent

audio_io_handle_t AudioFlinger::openInputRouter(audio_devices_t device,
        audio_io_handle_t output)
{
    audio_io_handle_t rt_io_handle = 0;
    status_t status;
    AudioHwDevice *inHwDev;
    struct audio_config config;
    audio_stream_in_t *inStream = NULL;

    // only a2dp input allowed
    if (!(device & (AUDIO_DEVICE_IN_BLUETOOTH_A2DP|AUDIO_DEVICE_BIT_IN)))
        return 0;

    memset(&config, 0, sizeof(struct audio_config));

    // fixme -- create better retrieval of a2dp hal interface
    inHwDev = findSuitableHwDev_byName("A2DP Audio HW HAL", device);
    if (inHwDev == NULL)
      return 0;
    ...
    status = inHwHal->open_input_stream(inHwHal, id, device, &config,
                                      &inStream);

    ALOGD("openInputRouter() opened input %p, samplingRate %d, format %d, channels %x, "
          "status %d",
          inStream,
          config.sample_rate,
          config.format,
    ...
    ALOGD("%s route onto output %d",__FUNCTION__, output);

    mRoutingThread =
        AudioFlinger::RoutingThread::Initialize(this, (audio_stream_t*)inStream, output,
                                                &rt_io_handle);

    ALOGD("%s:: calling Routing Thread Initializer complete ret: %d",
            __FUNCTION__, rt_io_handle);

    return rt_io_handle;
}

void AudioFlinger::closeInputRouter(void)
{
    audio_devices_t devices = AUDIO_DEVICE_IN_BLUETOOTH_A2DP|AUDIO_DEVICE_BIT_IN;
    AudioHwDevice *inHwDev;
    audio_stream_in_t *inStream = NULL;

    inHwDev = findSuitableHwDev_byName("A2DP Audio HW HAL", devices);
    if (inHwDev == NULL)
    {
        ALOGE("closeInputRouter failed to find interface");
        return;
    }

    audio_hw_device_t *inHwHal = inHwDev->hwDevice();

    inHwHal->close_input_stream(inHwHal, (struct audio_stream_in *)mRoutingThread->mInputStream);

    ALOGD("closeInputRouter complete");
}
...
{
    // stop any open routing tracks
    if (mRoutingThread && (input == (audio_io_handle_t)INPUT_ROUTER_HANDLE))
    {
        if (mRoutingThread->mOutputThread != NULL)
        {
           ALOGD("%s: Remove Output track from PlaybackThread %d",
                  __FUNCTION__, mRoutingThread->mOutputTrack->name());
           Mutex::Autolock _l(mRoutingThread->mOutputThread->mLock);
           mRoutingThread->mOutputThread->destroyTrack_l(mRoutingThread->mOutputTrack);
        }

        ALOGD("%s: exit routingthread loop", __FUNCTION__);
        mRoutingThread->threadLoopExit();
        ALOGD("%s: exit routingthread loop done", __FUNCTION__);
    ...
    delete in;
    ALOGD("closeInput() %d  mRoutingThread %p DONE", input, mRoutingThread);
    return NO_ERROR;
}
...
// checkRoutingThread_l() must be called with AudioFlinger::mLock held
// only one routing thread supported
// fixme -- need better control mechanism (dedicated cond outside of routingthread class ?)
AudioFlinger::RoutingThread *AudioFlinger::checkRoutingThread_l(audio_io_handle_t handle) const
{
    int wait_ms = 5;
    int attempts = 3;

    if (mRoutingThread == NULL) {
        return NULL;
    }

    ALOGD("checkRoutingThread_l handle %d %p", handle, mRoutingThread->mInputStream);

    // check handle (if not broadcast handle or dedicated input router handle)
    if ((handle != 0) && (handle != (int)INPUT_ROUTER_HANDLE))
        return NULL;

    // wait until routing thread is ready
    while ((mRoutingThread->isReady()==false) && attempts--) {
        // if routing thread is created but not yet running wait 5 ms
        ALOGD("checkRoutingThread_l :: wait %d ms", wait_ms);
        usleep(wait_ms*1000);
    }
    return mRoutingThread;
}
