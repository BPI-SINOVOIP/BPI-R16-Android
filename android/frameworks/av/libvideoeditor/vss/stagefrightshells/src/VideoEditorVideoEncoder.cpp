...
//            goto cleanUp;
            switch( pEncoderContext->mCodecParams->FrameRate )
            {
              case M4ENCODER_k5_FPS:
            	  Cts = pEncoderContext->mLastCTS + 500/5;
            	  break;
              case M4ENCODER_k7_5_FPS:
            	  Cts = pEncoderContext->mLastCTS + 500/8;
            	  break;
              case M4ENCODER_k10_FPS:
            	  Cts = pEncoderContext->mLastCTS + 500/10;
            	  break;
              case M4ENCODER_k12_5_FPS:
            	  Cts = pEncoderContext->mLastCTS + 500/13;
            	  break;
              case M4ENCODER_k15_FPS:
            	  Cts = pEncoderContext->mLastCTS + 500/15;
            	  break;
              case M4ENCODER_k20_FPS:
            	  Cts = pEncoderContext->mLastCTS + 500/20;
            	  break;
              case M4ENCODER_k25_FPS:
            	  Cts = pEncoderContext->mLastCTS + 500/25;
            	  break;
              case M4ENCODER_k30_FPS:
            	  Cts = pEncoderContext->mLastCTS + 500/30;
            	  break;
              case M4ENCODER_kVARIABLE_FPS:
                  Cts = pEncoderContext->mLastCTS + 500/30;
                  break;
...
//                pEncoderContext->mPuller->getBufferBlocking();
        	MediaBuffer *outputBuffer =pEncoderContext->mPuller->getBufferNonBlocking();      // star modify
