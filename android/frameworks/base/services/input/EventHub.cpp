...
status_t EventHub::openDeviceLocked(const char *devicePath) {
    ...
    /* Special case of HID mice (sony VGP-BMS80)with special keys. Should not consider
       this as a Keyboard input device*/
    if((identifier.vendor==0x054c) && (identifier.product==0x03fa)&&
       (device->classes & (INPUT_DEVICE_CLASS_CURSOR|INPUT_DEVICE_CLASS_KEYBOARD)))
    {
       ALOGV("%s Case of HID mice with special keys. Remove Keyboard mask.", __func__);
       device->classes &= ~(INPUT_DEVICE_CLASS_KEYBOARD);
    }
