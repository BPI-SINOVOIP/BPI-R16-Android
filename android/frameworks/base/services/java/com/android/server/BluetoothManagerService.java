...
// BRCM
    // Radio management specific declarations and definitions
    private int mRadioType;
    private final RemoteCallbackList<IBluetoothManagerCallback> mCallbacks;
    private final RemoteCallbackList<IBluetoothStateChangeCallback> mStateChangeCallbacks;
    private final RemoteCallbackList<IBluetoothRadioMgrCallback> mRadioMgrCallbacks;
    private static final int BLUETOOTH_RADIO_TYPE_ALL = 0x0FF;
...
// /BRCM
    ...
    BluetoothManagerService(Context context) {
        ...
        mRadioMgrCallbacks = new RemoteCallbackList <IBluetoothRadioMgrCallback>();
...
// BDROID/AFBT-430-20
    public boolean enableRadio(int radioType) {
        if (DBG) {
            Log.d(TAG,"enableRadio():  mBluetooth =" +
    ...
    public boolean disable(boolean persist) {
        ...
        public boolean disableRadio(int radioType) {
            ...
            if (DBG) {
                Log.d(TAG,"disableRadio(): mBluetooth = " +
                    (mBluetooth==null?"null":mBluetooth) +
                    " mBinding = " + mBinding);}
        ...
        public boolean registerRadioMgrCallback(IBluetoothRadioMgrCallback callback) {
            synchronized(mConnection) {
                boolean added = mRadioMgrCallbacks.register(callback);
                return added;
            }
        }


        public boolean unregisterRadioMgrCallback(IBluetoothRadioMgrCallback callback) {
            synchronized (mConnection) {
                return( mRadioMgrCallbacks.unregister(callback));
            }
        }

        public boolean enableGatt() {
            if (DBG) Log.d(TAG,"enableGatt()");
            if ((mBluetoothGatt != null) ||(mGattBinding == true))
    {
                if (DBG)    Log.d(TAG,"enableGatt(): Gatt service already enabled...!"  +
                                 "mBluetoothGatt = " + "binding =" + mGattBinding);
                return true;
            }
    ...
    /**
     * Inform Radio Mgr instances that Gatt Service is up/down
     */
    private void sendGattServiceStatusCallback(boolean up) {
        if (gattState == up) {
              if (DBG) Log.d(TAG,"GATT status is already sent ");
              return;
        }
        gattState = up;
        if (DBG) Log.d(TAG,"Calling sendGattServiceStatusCallback callbacks up = " + up);
        int n = mRadioMgrCallbacks.beginBroadcast();
        if (DBG) Log.d(TAG,"Broadcasting GattServiceStatusCallback() to " + n + "receivers");
        for (int i=0; i <n;i++) {
            try {
                mRadioMgrCallbacks.getBroadcastItem(i).onGattServiceStateChange(up,mBluetoothGatt);
            }  catch (RemoteException e) {
                Log.e(TAG, "Unable to call Gattservice callback fn on callback #" + i, e);
            }
        }
        mRadioMgrCallbacks.finishBroadcast();
    }

    /**
     * Inform Radio Mgr instances that BT Radio is up/down
     */
    private void sendRadioSatusCallback(boolean up) {

        if (radioState == up) {
              if (DBG) Log.d(TAG,"Radio status is already sent ");
              return;
        }
        radioState = up;
        if (DBG) Log.d(TAG,"Calling sendRadioSatusCallback callbacks up = " + up);
        int n = mRadioMgrCallbacks.beginBroadcast();
        Log.d(TAG,"Broadcasting Radio() to " + n + " Radio Mgr receivers.");
        for (int i=0; i <n;i++) {
            try {
                mRadioMgrCallbacks.getBroadcastItem(i).onBTRadioStateChange(up);
            }  catch (RemoteException e) {
                Log.e(TAG, "Unable to call Gattservice callback fn on callback #" + i, e);
            }
        }
    ...
    /**
     * Inform Radio Mgr instances that Airplane mode is on/off
     */
    private void sendAirplaneModeChangeCallback(boolean on) {
        if (DBG) Log.d(TAG,"Calling sendAirplaneModeChangeCallback callbacks up = " + on);
        int n = mRadioMgrCallbacks.beginBroadcast();
        Log.d(TAG,"Broadcasting onBAirplane mode chnage to " + n + " Radio Mgr receivers.");
        for (int i=0; i <n;i++) {
            try {
                mRadioMgrCallbacks.getBroadcastItem(i).onAirplaneModeChange(on);
            }  catch (RemoteException e) {
    ...
    private void onUserSwitched() {
        if (DBG) Log.d(TAG,"Calling onUserSwitched callbacks");
        int n = mRadioMgrCallbacks.beginBroadcast();
        Log.d(TAG,"Broadcasting onUserSwitched " + n + " Radio Mgr receivers.");
        for (int i=0; i <n;i++) {
            try {
                mRadioMgrCallbacks.getBroadcastItem(i).onUserSwitched();
            }  catch (RemoteException e) {
                Log.e(TAG, "Unable to call Radio Mgr callback fn on callback #" + i, e);
            }
        }
    ...
    private class BluetoothServiceConnection implements ServiceConnection {
        ...
        public void setTurnOnRadio(boolean isTurnOnRadio, int radioType) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MESSAGE_ENABLE_RADIO:
                    ...
                    handleEnableRadio(msg.arg1);
                    break;

                case MESSAGE_DISABLE_RADIO:
                    handleDisableRadio(msg.arg1);
                    break;
...
*/
                ...
                case MESSAGE_BLUETOOTH_SERVICE_CONNECTED:
                {
                    ...
                    synchronized(mConnection) {
                        if (msg.arg1 == SERVICE_IBLUETOOTHGATT) {
                            ...
                            mGattBinding = false;
                            sendGattServiceStatusCallback(true);
                            break;
                        ...
                        //Do enable request
                        if (DBG)
                            Log.d(TAG, " Turning on BT modules Radio on = " +
                                   mConnection.isTurnOnRadio() + " BT On = " + mEnable);
                        if (mConnection.isTurnOnRadio()){
                            try {
                                if(!mBluetooth.enableRadio(mRadioType)) {
                    ...
                    if (mEnable && mBluetooth != null) {
                        ...
                        if (mState == BluetoothAdapter.STATE_TURNING_ON) {
                         ...
                         if (isRadioEnabled() == true) {
                            reEnableRadio = true;
                            handleDisableRadio(BLUETOOTH_RADIO_TYPE_ALL);
                            sendGattServiceStatusCallback(false);
                            sendRadioSatusCallback(false);
                         }
                        // disable
                        ...
                        // enable
                        handleEnable(mQuietEnable);
                        if (reEnableRadio == true)
                            onUserSwitched();
                    }
                    else if (isRadioEnabled() == true) {
                        //Unregister callback object
                        try {
                            mBluetooth.unregisterCallback(mBluetoothCallback);
                        } catch (RemoteException re) {
                        ...
                        bluetoothStateChangeHandler(BluetoothAdapter.STATE_OFF,
                                                    BluetoothAdapter.STATE_RADIO_OFF);
                        SystemClock.sleep(100);

                        mHandler.removeMessages(MESSAGE_BLUETOOTH_STATE_CHANGE);
                        mState = BluetoothAdapter.STATE_OFF;
                       // Send Radio state change intent to clients
    ...
    private final void initWakeLock() {
        try {
            if (mWakeLock == null) {
                mWakeLock = ParcelFileDescriptor.open(new File("/sys/power/wake_lock"),
                        ParcelFileDescriptor.MODE_READ_WRITE);
                mWakeUnlock = ParcelFileDescriptor.open(new File("/sys/power/wake_unlock"),
                        ParcelFileDescriptor.MODE_READ_WRITE);
            }
            if (DBG) Log.d(TAG, "mWakeLock: " + mWakeLock + ", mWakeUnlock: " + mWakeUnlock);
            if (mWakeLock != null && mWakeUnlock != null)
                mBluetooth.initWakeLock(mWakeLock, mWakeUnlock);
        } catch(Exception e) {
            e.printStackTrace();
        }
    }
    private void bluetoothStateChangeHandler(int prevState, int newState) {
        if (prevState != newState) {
            if ((prevState == BluetoothAdapter.STATE_OFF) && (newState == BluetoothAdapter.STATE_RADIO_OFF)) {
                ...
                // If there is no outstanding Radio On message Qued on to Bluetooth service, bring it down
                if ((mConnection.isTurnOnRadio() == false) && (mEnable == false)) {
                    if (DBG) Log.d(TAG,"Radio off...Unbinidng BT Service....");
                    sendBluetoothServiceDownCallback();
                    unbindAndFinish();
                }else {
                /* Notifiy Radio clients about Radio status */
                    sendGattServiceStatusCallback(false);
                    sendRadioSatusCallback(false);
                }
                mConnection.setTurnOnRadio(false, 0);
                return;
            }
            ...
            intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
                ...
                // Call Radio state change callbacks
                if (newState == BluetoothAdapter.STATE_RADIO_OFF)
                    sendRadioSatusCallback(false);
                else
                    sendRadioSatusCallback(true);
    ...
    private void handleEnableRadio(int radioType) {
        ...
        synchronized(mConnection) {
            if ((mBluetooth == null) &&(!mBinding)) {
                ...
                mHandler.sendMessageDelayed(timeoutMsg,TIMEOUT_BIND_MS);
                mConnection.setGetNameAddressOnly(false);
                mConnection.setTurnOnRadio(true, radioType);
                Intent i = new Intent(IBluetooth.class.getName());
                if (!mContext.bindService(i, mConnection,Context.BIND_AUTO_CREATE)) {
                    mHandler.removeMessages(MESSAGE_TIMEOUT_BIND);
                ...
                try {
                    mConnection.setTurnOnRadio(true, radioType);
                    if(!mBluetooth.enableRadio(radioType)) {
                        Log.e(TAG,"IBluetooth.enableRadio() returned false");
                    }
                } catch (RemoteException e) {
                    Log.e(TAG,"Unable to call enableRadio()",e);
    ...
    private void handleDisableRadio(int radioType) {
        synchronized(mConnection) {
            if (isRadioEnabled()) {
                try {
                    if(!mBluetooth.disableRadio(radioType)) {
