...
public final class PowerManagerService extends IPowerManager.Stub
	...
	private boolean mHoldBootFastWakeLock;
	...
	private boolean goToBootFastSleepNoUpdateLocked(long eventTime) {
		...
		/* this low battery shutdown is not standard shutdown process.
		if(mBatteryLevel<=3){
    ...
    private void updatePowerStateLocked() {
        ...
        for (int i = 0; i < count; i++) {
            final WakeLock wakeLock = mWakeLocks.get(i);
            String tag = null;
            if((wakeLock !=null) && (wakeLock.mTag != null) ) {		
                tag =wakeLock.mTag;
            } else {
                Log.d(TAG," WakeLock mTag null ");
            }

            //4.2 if (tag !=null && tag.contains("AudioOut_")) {
            if (tag !=null && tag.contains("AudioMix")) {
                success++;
            } 
        }
    }
    ...
    private void updateIsPoweredLocked(int dirty) {
        if ((dirty & DIRTY_BATTERY_STATE) != 0) {
			...
			Slog.d(TAG,"mHoldBootFastWakeLock = " + mHoldBootFastWakeLock);
    ...
    private void updateSuspendBlockerLocked() {
        ...
        //If it is fast boot shut and ac, keep system on
        if(mBootFastStats && mIsPowered){
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
				...
				case MSG_TEMP_WAKUP_RESLEEP:
					mPolicy.hideScreen(true);
					if(mBootFastStats==false){
						Slog.d(TAG,"Error here return!");
						break;
					}
					goToBootFastSleep(SystemClock.uptimeMillis());
    ...
    private final class SuspendBlockerImpl implements SuspendBlocker {
        ...
        public String toString() {
            synchronized (this) {
                return mName + ": ref count=" + mReferenceCount + " mBootFastRef = " + mBootFastRef;
            }
        }
    }
