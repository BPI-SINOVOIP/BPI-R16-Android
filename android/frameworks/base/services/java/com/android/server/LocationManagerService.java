...
public class LocationManagerService extends ILocationManager.Stub {
    ...
    private void updateProvidersLocked() {
        ...
        boolean hasProvisioned = Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.DEVICE_PROVISIONED, 0) == 1 ? true : false;
        if (hasProvisioned) {
            return;
        }
        
        int curLocationMode = Settings.Secure.getInt(mContext.getContentResolver(), Settings.Secure.LOCATION_MODE,
                    Settings.Secure.LOCATION_MODE_OFF);
        if(curLocationMode == Settings.Secure.LOCATION_MODE_OFF) {
            return;
        }
        
        int locationMode;
        boolean hasGpsLocation = mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LOCATION_GPS);
        boolean hasNetworkLocation = mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LOCATION_NETWORK);
        if (hasGpsLocation) {
        ...
        Settings.Secure.putInt(mContext.getContentResolver(), Settings.Secure.LOCATION_MODE, locationMode);
    }
