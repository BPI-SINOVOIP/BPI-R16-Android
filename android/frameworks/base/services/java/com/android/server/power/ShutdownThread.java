...
public final class ShutdownThread extends Thread {
	...
	//Allwinner for fast boot shut
    private static final int CLOSE_PROCESS_DIALOG = 2;
    // stop shutdown animation
    private static final int STOP_SHUTDOWN_ANIMATION = 3;

    private static final int MAX_SERVICES = 100;
    
    private static final int MAX_ACTIVITYS = 100;
    
    private static final int BOOTFAST_WAIT_TIME = 1500;
    ...
    //Allwinner, wether we have radio
    ...
    //Allwinner, if should do fast boot shut
    ...
    private void setAirplaneModeState(boolean enabled) {
		...
		if(Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.AIRPLANE_MODE_ON,0)==1){
	 	...
	 	Settings.System.putInt(mContext.getContentResolver(), Settings.System.AIRPLANE_MODE_SET,1);
    ...
    private static void beginShutdownSequence(Context context) {
        ...
        int shutdownduration = SystemProperties.getInt("persist.sys.shutdownduration", 0);
        if (shutdownduration <= 0) pd.show();
        	...
        	public void handleMessage(Message msg){
        		switch(msg.what) {
        			case CLOSE_PROCESS_DIALOG:
        				...
        				pd.dismiss();
        				break;
				case STOP_SHUTDOWN_ANIMATION:
					SystemProperties.set("service.bootanim.exit", "1");
					SystemProperties.set("ctl.stop", "bootanim");
					SystemProperties.set("sys.shutdown_animation", null);
					break;
        		}
        	}
        };
    ...
    public void run() {
        ...
        if(!mBootFastEnable){
            ...
            if (shutdownduration > 0) {
                long sleepTime = SystemClock.uptimeMillis() - startTime;
                sleepTime = shutdownduration - sleepTime;
                if (sleepTime > 0)
		...
		if(mWindowManager != null){
			try{
				...
				Settings.System.putInt(mContext.getContentResolver(), Settings.System.ROTATION_MODE_SET,alreadyfreeze?(0 - rotation):1);
                mWindowManager.freezeRotation(Surface.ROTATION_0);
                mWindowManager.updateRotation(true, true);
        ...
        if (shutdownduration > 0) {
            long sleepTime = SystemClock.uptimeMillis() - startTime;
            sleepTime = shutdownduration - sleepTime;
            sleepTime = sleepTime > BOOTFAST_WAIT_TIME ? sleepTime : BOOTFAST_WAIT_TIME;
            if (sleepTime > 0)
                SystemClock.sleep(sleepTime);
            sInstance.mHandler.sendEmptyMessageDelayed(STOP_SHUTDOWN_ANIMATION, 500);
        } else {
            SystemClock.sleep(BOOTFAST_WAIT_TIME);
            sInstance.mHandler.sendEmptyMessageDelayed(CLOSE_PROCESS_DIALOG, 500);
	...
	private void killRemoveService(Context context){
        ...
        for (int i=0; i<NS; i++) {
            ...
            /*Some service need very long time to stop, can ignore it*/
            if (services.get(i).process.contains("com.android.bluetooth")) {
                services.remove(i);
                i--;
                NS--;
                continue;
