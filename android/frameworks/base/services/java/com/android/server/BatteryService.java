...
public final class BatteryService extends Binder {
	...
	private native void native_setChargeState(boolean state);
    ...
    private boolean isPoweredLocked(int plugTypeSet) {
	 ...
	 /*While plugged and the ac/usb detection not finished the battery status will not be DISCHARGING,if use 
	 BATTERY_PLUGGED_ANY to get the plug status should return true.*/
	 if (plugTypeSet == BatteryManager.BATTERY_PLUGGED_ANY && mBatteryProps.batteryStatus != 
    ...
    private boolean isFastbootModeEnable() {
        return (SystemProperties.getBoolean("ro.sys.bootfast", false) &&
                (1 == Settings.System.getIntForUser(mContext.getContentResolver(),
                                                    Settings.System.BOOT_FAST_ENABLE, 0, UserHandle.USER_CURRENT)));
    }
    ...
    private void shutdownIfInBootFastModeLocked() {
        ...
        if (DEBUG) {
            Slog.d(TAG, "isSystemReady=" + ActivityManagerNative.isSystemReady());
            Slog.d(TAG, "isFastbootModeEnable=" + isFastbootModeEnable() +
                        ", batteryLevel=" + mBatteryProps.batteryLevel +
                        ", isPower=" + !isPoweredLocked(BatteryManager.BATTERY_PLUGGED_ANY));
        }

        if (ActivityManagerNative.isSystemReady()) {
            if (isFastbootModeEnable() &&
                mBatteryProps.batteryLevel < BOOT_FAST_REAL_SHUT_DOWN_LEVEL &&
    ...
    private void shutdownIfOverTempLocked() {
        ...
        Slog.d(TAG,"wanglei --> mBatteryProps.batteryTemperature = " + mBatteryProps.batteryTemperature);
        if (mBatteryProps.batteryTemperature > 600 || mBatteryProps.batteryTemperature < -100) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    if (ActivityManagerNative.isSystemReady()) {
