...
class QuickSettings {
    ...
    private void addUserTiles(ViewGroup parent, LayoutInflater inflater) {
        ...
        if (mContext.getResources().getBoolean(R.bool.quick_settings_show_userinfo)) {
    ...
    private void addSystemTiles(ViewGroup parent, LayoutInflater inflater) {
        ...
        if (mModel.deviceHasMobileData()) {
            ...
            if (mContext.getResources().getBoolean(R.bool.quick_settings_show_rssiinfo)) {
        ...
        if (mModel.deviceSupportsBluetooth()
            ...
            if (mContext.getResources().getBoolean(R.bool.quick_settings_show_bluetooth_setting)) {
    ...
    private void addTemporaryTiles(final ViewGroup parent, final LayoutInflater inflater) {
        ...
        if (mContext.getResources().getBoolean(R.bool.quick_settings_show_screenshot)) {
	        final QuickSettingsBasicTile ScreenShotTile
	                = new QuickSettingsBasicTile(mContext);
	        ScreenShotTile.setImageResource(R.drawable.ic_qs_screen_shot);
	        ...
	        mModel.addScreenShotTile(ScreenShotTile, new QuickSettingsModel.RefreshCallback() {
	            @Override
	            public void refreshView(QuickSettingsTileView unused, State state) {
	                ScreenShotTile.setText(state.label);
	            }
	        });
        ...
        // screen record
        if (mContext.getResources().getBoolean(R.bool.quick_settings_show_screenrecord)) {
	        final QuickSettingsBasicTile screenRecordTile
	                = new QuickSettingsBasicTile(mContext);
	        screenRecordTile.setImageResource(R.drawable.ic_qs_screen_record);
	        screenRecordTile.setOnClickListener(new View.OnClickListener() {
	            @Override
	            public void onClick(View v) {
	                Log.i("bright", "screen record is clicked.");
	                mBar.collapseAllPanels(true);
	                mHandler.removeCallbacks(mScreenrecordClick);
	                mHandler.postDelayed(mScreenrecordClick, 500);
	            }
	        });
	        mModel.addScreenRecordTile(screenRecordTile, new QuickSettingsModel.RefreshCallback() {
	            @Override
	            public void refreshView(QuickSettingsTileView unused, State state) {
	                screenRecordTile.setText(state.label);
	            }
	        });
    ...
    // add by chenjingjing, for screen shot in quick settings.
    ...
    // add by zengshuchuan screenrecord
    final Object mScreenrecordLock = new Object();
    boolean mScreenrecording = false;
    final String CMD_FIND_SCREENRECORD_PID = "ps | grep screenrecord";
    final String CMD_EXIT = "exit\n";
    Handler mScreenrecordHandler;
	ScreenrecordNotification myScreenrecordNotification;
    private void startScreenrecord() {
        synchronized (mScreenrecordLock) {
            if (mScreenrecording) return;
            new Thread() {
                @Override
                public void run() {
                    ...
                    try {
						...
						myScreenrecordNotification.cancelNotificaiton();
                        Log.d(TAG, "Screenrecord stop");
                    }
                }
            }.start();
			if (myScreenrecordNotification == null) {
				myScreenrecordNotification = new ScreenrecordNotification(mContext);
			}
			myScreenrecordNotification.notifyNotificaiton();
        }
    }
    ...
    private final Runnable mScreenrecordClick = new Runnable() {
        public void run() {
            if (mScreenrecordHandler == null) {
                mScreenrecordHandler = new Handler(mHandler.getLooper()) {
                    @Override
                    public void handleMessage(Message msg) {
                        switch (msg.what) {
                            case 1:
            ...
            if (mScreenrecording)
                mScreenrecordHandler.sendEmptyMessage(2);
            else
                mScreenrecordHandler.sendEmptyMessage(1);
        }
    };
