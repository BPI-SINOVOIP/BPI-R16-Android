...
public class MediaFocusControl implements OnFinished {
    ...
    // Avrcp 1.4 support
    private String mCurrentRcPlayer = null;
    ...
    private void notifyTopOfAudioFocusStack() {
        ...
        if (!mFocusStack.empty()) {
            if (canReassignAudioFocus()) {
                try {
                    if(mCurrentRcPlayer != null) {
                        String packageName = mFocusStack.peek().getCurrentPackageName();
                        if(!mCurrentRcPlayer.equals(packageName)) {
                                mCurrentRcPlayer = packageName;
                                Log.d (TAG, "NotifyRemote: Address player changed:" +
                                        mCurrentRcPlayer);
                                notifyAddressedPlayerChanged();
                        }
                    } else {
                        mCurrentRcPlayer = mFocusStack.peek().getCurrentPackageName();
                        Log.d (TAG, "NotifyRemote: Address player assigned:" + mCurrentRcPlayer);
                        notifyAddressedPlayerChanged();
                    }
                mFocusStack.peek().handleFocusGain(AudioManager.AUDIOFOCUS_GAIN);
                } catch (NullPointerException e) {
                    Log.e(TAG, "Failure to set current RC player due to "+ e);
                    e.printStackTrace();
                }
            }
        }
    ...
    // Notify address player change to Remote
    private void notifyAddressedPlayerChanged() {
            Intent i = new Intent("com.broadcom.avrcp.EVENT_ADDRESS_PLAYER_CHANGED");
            if(mCurrentRcPlayer != null) {
                i.putExtra("currAddrPlayer", mCurrentRcPlayer);
            } else {
                i.putExtra("currAddrPlayer", "none");
            }
    ...
    private void updateRemoteControlDisplay_syncAfRcs(int infoChangedFlags) {
      ...
      protected List<String> getPlayerPlayBackState() {
          synchronized(mRCStack) {
          List<String> mPlayerList = new ArrayList<String>();
          List<String> mPlayerPlayBackStateList = new ArrayList<String>();
          Boolean alreadyInList;
          String packageName;
          Iterator<RemoteControlStackEntry> stackIterator = mRCStack.iterator();
          while(stackIterator.hasNext()) {
                RemoteControlStackEntry rcse = stackIterator.next();
          packageName = rcse.mReceiverComponent.getPackageName();
          alreadyInList = false;
          ...
          if(!alreadyInList) {
              mPlayerList.add(rcse.mReceiverComponent.getPackageName());
              mPlayerPlayBackStateList.add(String.valueOf(rcse.mPlaybackState.mState));
              Log.d(TAG, "getPlayerPlayBackState" +
                  String.valueOf(rcse.mPlaybackState.mState));
          }
      }
      return mPlayerPlayBackStateList;
     }
    }
