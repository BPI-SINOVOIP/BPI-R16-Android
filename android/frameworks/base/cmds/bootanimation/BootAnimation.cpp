...
void BootAnimation::playBootMusic(const char* url)
{
    ...
    if((access(url, F_OK)) != -1 && (access("/data/sile", F_OK)) == -1) {
...
status_t BootAnimation::readyToRun() {
    ...
    char boot[PROPERTY_VALUE_MAX];
    property_get("sys.shutdown_animation", boot, "boot");
    char* animation;
    if (strcmp(boot, "shutdown")) {
        animation = SYSTEM_BOOTANIMATION_FILE;
    } else {
        animation = SYSTEM_SHUTDOWNANIMATION_FILE;
    }
    if ((encryptedAnimation &&
            ...
            ((access(animation, R_OK) == 0) &&
            (mZip.open(animation) == NO_ERROR))) {
