...
public class BluetoothStressTest extends InstrumentationTestCase {
    ...
    /*
     * Stress Test for enabling/disabling radio
     */
    public void testEnableRadio(){
    int iterations = BluetoothTestRunner.sEnableRadioIterations;
    if(iterations==0){
     return;
    }
    BluetoothAdapter adapter= BluetoothAdapter.getDefaultAdapter();
    mTestUtils.disable_radio(adapter);
    for(int i=0;i<iterations;i++){
        mTestUtils.writeOutput("radio iteration " + (i + 1) + " of " + iterations);
            mTestUtils.enable_radio(adapter);
            mTestUtils.disable_radio(adapter);
            }
        }
    ...
    /*
    *
    *Stress Test for enabling/disabling bluetooth and radio alternatively.
    */
    public void testEnableBTRadio(){
        ...
        mTestUtils.disable_radio(adapter);
        for(int i=0;i<iterations;i++){
            mTestUtils.writeOutput("BT iteration " + (i + 1) + " of " + iterations);
            mTestUtils.enable(adapter);
            mTestUtils.writeOutput("radio iteration " + (i + 1) + " of " + iterations);
            mTestUtils.enable_radio(adapter);
            mTestUtils.disable_radio(adapter);
