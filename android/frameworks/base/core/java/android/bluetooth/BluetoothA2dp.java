...
public final class BluetoothA2dp implements BluetoothProfile {
    ...
    /**
     * Intent used to broadcast the Content Protection Type for the current streaming connection
     *
     * <p>This intent will have 2 extras:
     * <ul>
     *   <li> {@link BluetoothDevice#EXTRA_DEVICE} - The remote device. </li>
     *   <li> {@link #EXTRA_CP_TYPE} - Content Protection Type. </li>
     * </ul>
     *
     * <p>Requires {@link android.Manifest.permission#BLUETOOTH} permission to
     * receive.
     ...
     */
    @SdkConstant(SdkConstantType.BROADCAST_INTENT_ACTION)
    public static final String ACTION_CP_TYPE =
        "android.bluetooth.a2dp.profile.action.CP_TYPE";


   /**
    * Extra for the CP Type for the streaming connection
    *
    * This extra represents the current Content Protection type used for streaming
    *
    * @hide
    */
   public static final String EXTRA_CP_TYPE = "android.bluetooth.a2dp.profile.extra.CP_TYPE";
    ...
    /**
     * CP_TYPE Copy Protection is not enabled for this connection
     *
     * @hide
     */
    ...
    /**
     * CP_TYPE Copy Protection type is 'COPY_NEVER'
     *
     * @hide
     */
    ...
    /**
     * CP_TYPE Copy Protection type is 'COPY_ONCE'
     *
     * @hide
     */
    ...
    /**
     * CP_TYPE Copy Protection type is 'COPY_FREE'
     *
     * @hide
     */
    ...
    /**
     * Fetch the content protection type used for the current connection
     *
     * @param device Remote Bluetooth Device
     * @return false on immediate error,
     *               true otherwise
    ...
    public boolean getContentProtectionType(BluetoothDevice device) {
        if (DBG) log("getContentProtectionType(" + device + ")");
        if (mService != null && isEnabled() &&
            isValidDevice(device)) {
            try {
                return mService.getContentProtectionType(device);
            } catch (RemoteException e) {
                Log.e(TAG, "Stack:" + Log.getStackTraceString(new Throwable()));
                return false;
            }
    ...
    /**
     * Configure the CP Type to be used for the streaming connection
     *
     * @param device Remote Bluetooth Device
     * @cp_type - Can be one of BluetoothA2dp.CP_TYPE_COPY_NEVER or
     *                   CP_TYPE_COPY_ONCE, CP_TYPE_COPY_FREE
     * @return false on immediate error,
     *               true otherwise
     * @hide
     */
    public boolean setContentProtectionType(BluetoothDevice device, int cp_type) {
        if (DBG) log("setContentProtectionType(" + device + ")");
        if (mService != null && isEnabled() &&
            isValidDevice(device)) {
            try {
                return mService.setContentProtectionType(device, cp_type);
            } catch (RemoteException e) {
                Log.e(TAG, "Stack:" + Log.getStackTraceString(new Throwable()));
                return false;
            }
