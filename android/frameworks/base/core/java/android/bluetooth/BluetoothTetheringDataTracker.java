...
public class BluetoothTetheringDataTracker extends BaseNetworkStateTracker {
    ...
    private BluetoothProfile.ServiceListener mProfileServiceListener =
        ...
        public void onServiceDisconnected(int profile) {
            AsyncChannel ac = mAsyncChannel.get();
            if(ac != null) {
                if (DBG) Log.d(TAG, "onServiceDisconnected disconnecting asyncchannel");
                try {
                    ac.disconnect();
                } catch(Exception e) {
                    if (VDBG) e.printStackTrace();
                }
            }
            mBluetoothPan = null;
        }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case AsyncChannel.CMD_CHANNEL_DISCONNECTED:
                    ...
                    AsyncChannel ac = mBtdt.mAsyncChannel.get();
                    if (VDBG) Log.d(TAG, "got CMD_CHANNEL_DISCONNECTED");
                    if(ac != null) {
                        ac.disconnected();  //This clears AsyncChannel else causes memory leaks
