...
public final class BluetoothHealth implements BluetoothProfile {
    ...
    /**
     * Get remote device's datatype and role (SDP).
     * This is an asynchronous call.
     * This will initiate a SDP on remote device
     * <p>Requires {@link android.Manifest.permission#BLUETOOTH} permission.
     *
    ...
    public boolean getDeviceDatatype(BluetoothDevice device,
        ...
        if (mService != null && isEnabled() && isValidDevice(device) &&
                config != null) {
            try {
                return mService.getDeviceDatatype(device, config);
            } catch (RemoteException e) {
                Log.e(TAG, e.toString());
            }
        } else {
    ...
    private static class BluetoothHealthCallbackWrapper extends IBluetoothHealthCallback.Stub {
        ...
        public void onHealthDeviceSourceDataTypeResult(BluetoothHealthAppConfiguration config,
            ...
            mCallback.onHealthDeviceSourceDataTypeResult(config, device, dataTypes);
        }

        @Override
        public void onHealthDeviceSinkDataTypeResult(BluetoothHealthAppConfiguration config,
                    BluetoothDevice device, int[] dataTypes){
            mCallback.onHealthDeviceSinkDataTypeResult(config, device, dataTypes);
        }
    }
