...
public final class BluetoothHeadset implements BluetoothProfile {
    ...
    // The HFP AG service can change started/stopped in runtime to support dynamic mode swithcing.
    // This can be detected by ACTION_UUID_CHANGED intent
    // and the Bluetooth Headset has to bind again to the service
    private BroadcastReceiver mLocalUuidChangedReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            Log.d(TAG, action);
            if (action.equals(BluetoothAdapter.ACTION_UUID_CHANGED)){
                ...
                if ((BluetoothUuid.isUuidPresent(uuids, BluetoothUuid.Handsfree_AG) ||
                     ...
                     && (mService == null)) {
                        // UUID present but the service is null so bind to service
                        Log.d(TAG, "UUID dynamically added so Bind to headset service ");
                  ...
                  } else if (((!BluetoothUuid.isUuidPresent(uuids, BluetoothUuid.Handsfree_AG)) &&
                        (!BluetoothUuid.isUuidPresent(uuids, BluetoothUuid.HSP_AG)))
                     && (mService != null)) {
                      // UUID present but the service is not NULL so unbind to service
                      Log.d(TAG, "UUID dynamically removed so Unbinding to headset service ");
                      synchronized (mConnection) {
                          try {
                              mService = null;
    ...
    /*package*/ BluetoothHeadset(Context context, ServiceListener l) {
        log("BluetoothHeadset Created"+context.getPackageManager());
        mContext = context;
        mServiceListener = l;
        mAdapter = BluetoothAdapter.getDefaultAdapter();

        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(BluetoothAdapter.ACTION_UUID_CHANGED);
        mContext.registerReceiver(mLocalUuidChangedReceiver, intentFilter);
    ...
    boolean doBind() {
        log("doBind"+mContext.getPackageManager());
    ...
    /*package*/ void close() {
        ...
        mContext.unregisterReceiver(mLocalUuidChangedReceiver);
    }
