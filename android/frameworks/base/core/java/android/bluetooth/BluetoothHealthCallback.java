...
public abstract class BluetoothHealthCallback {
   ...
   /**
     * Callback to inform the SDP result for a peer device
     * <p> This callback is called on the binder thread (not on the UI thread)
     *
     * @param config Bluetooth Health app configuration
     * @param device The Bluetooth Device
     * @param dataTypes  List of DataTypes the remote device supports as Source
     * @hide
     */
    public void onHealthDeviceSourceDataTypeResult(BluetoothHealthAppConfiguration config,
            BluetoothDevice device, int[] dataTypes){
        for(int i =0; i<dataTypes.length;i++){
            Log.d(TAG,"onHealthDeviceSourceDataTypeResult:Datattypes supported ="+dataTypes[i]);
        }
    }
     ...
     * @param config Bluetooth Health app configuration
     * @param device The Bluetooth Device
     * @param dataTypes  List of DataTypes the remote device supports as Sink
     * @hide
     */
    public void onHealthDeviceSinkDataTypeResult(BluetoothHealthAppConfiguration config,
            BluetoothDevice device, int[] dataTypes){
        for(int i =0; i<dataTypes.length;i++){
            Log.d(TAG,"onHealthDeviceSinkDataTypeResult:Datattypes supported ="+dataTypes[i]);
        }
    }
}
