...
public final class BluetoothAdapter {
    ...
    public boolean sspDebugConfigure(boolean enable) {
        ...
        try {
            synchronized(mManagerCallback) {
                if (mService != null) return mService.sspDebugConfigure(enable);
            }
        } catch (RemoteException e) {Log.e(TAG, "", e);}
        return false;
    }
    ...
    public boolean dutModeConfigure(boolean enable) {
        ...
        try {
            synchronized(mManagerCallback) {
                if (mService != null) return mService.dutModeConfigure(enable);
            }
        } catch (RemoteException e) {Log.e(TAG, "", e);}
        return false;
    }
    ...
    /**
    * Create vendor specific hci socket
    * @hide
    */
    public BluetoothSocket createVendorHciSocket(int hciOpCode, int hciEvent, int extraCmdId, int flags)
            throws IOException {
            return new BluetoothSocket(BluetoothSocket.TYPE_VENDOR_HCI,
                                    -1, false, false, new BluetoothDevice(getAddress()),
                                    (hciOpCode << 16) | (extraCmdId << 8) | hciEvent, null);
    }
     /**
    ...
    /**
     * Add/remove SDP record for a specific profile
     *   enable - true:add, false:remove
     *   uuid - profile (currently supported profile: PCE-0x112E)
     * @hide
     */
    public boolean setSdpRecord(boolean enable, int uuid) {
        ...
        try {
            synchronized(mManagerCallback) {
                if (mService != null) return mService.setSdpRecord(enable, uuid);
            }
        } catch (RemoteException e) {Log.e(TAG, "", e);}
        return false;
    }
    ...
    /**
     * Update profile connection state change
     *   device - remote BluetoothDevice
     *   profile - profile id (currently supported profile: PCE)
     *   state - current state
     *   prevState - previous state transitioned from
     * @hide
     */
    public void sendConnectionStateChange(BluetoothDevice
        ...
        try {
            synchronized(mManagerCallback) {
                Log.d(TAG, "sendConnectionStateChange profile:" + profile + " state:" + state);
                if (mService != null) mService.sendConnectionStateChange(device, profile, state, prevState);
            }
        } catch (RemoteException e) {Log.e(TAG, "", e);}
    }
