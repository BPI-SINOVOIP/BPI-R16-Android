...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static final boolean DEBUG_BOOTFAST = false;
    ...
    private void setAirplaneModeState(boolean enabled) {
		...
		if(Settings.System.getIntForUser(mContext.getContentResolver(),Settings.System.AIRPLANE_MODE_SET,0, UserHandle.USER_CURRENT)==1){
			Slog.d(TAG,"user setting airplane mode is on");
			Settings.System.putInt(mContext.getContentResolver(), Settings.System.AIRPLANE_MODE_SET,0);
	...
	private Runnable mBootFastPowerLongPress = new Runnable() {
	     ...
	     SystemProperties.set("sys.fasboot_shutdown", "0");
                ...
                mHandler.postDelayed(new Runnable(){
                    ...
                    public void run(){
                        ...
                        if(mWindowManager != null){
                            try{
                                int rotationSet = Settings.System.getIntForUser(mContext.getContentResolver(),
                                Settings.System.ROTATION_MODE_SET,0,UserHandle.USER_CURRENT);
                                Log.d(TAG,"rotation set = "+ rotationSet );
                                Settings.System.putIntForUser(mContext.getContentResolver(),
                                Settings.System.ROTATION_MODE_SET,0,UserHandle.USER_CURRENT);
                                if(rotationSet<1){
                                    mWindowManager.freezeRotation(0 - rotationSet);
                                }else{
    ...
    private void interceptBootFastPowerKeyDown(boolean handled) {
    	 if(DEBUG_BOOTFAST)
    		 Log.v(TAG, "interceptBootFastPowerKeyDown handled = " + handled);
    ...
    private boolean interceptBootFastPowerKeyUp(boolean canceled) {
    	 if(DEBUG_BOOTFAST)
    		Log.v(TAG,"interceptBootFastPowerKeyUp + !mPowerBootKeyHandled = " + mPowerBootKeyHandled); 
        if (!mPowerBootKeyHandled && canceled) {
    ...
    /** {@inheritDoc} */
	public void notifyLineinSwitchChanged(long whenNanos, int lineinInsert){
        Slog.i(TAG, "linein state: " + lineinInsert);
        Intent intent = new Intent(Intent.ACTION_LINEIN_PLUG);
		intent.putExtra("state", lineinInsert);
			...
			if(keyCode == KeyEvent.KEYCODE_POWER&&mPowerManager.isBootFastStatus()){
				if(!mPowerManager.isBootFastWakeFromStandby()){
					if(down){
						...
						interceptBootFastPowerKeyDown(false);
						return 0;
					}else{
	...
	public void updateBootFastSettings(){
		Log.d(TAG,"ACCELEROMETER_ROTATION = " + Settings.System.getInt(mContext.getContentResolver(),Settings.System.ACCELEROMETER_ROTATION, 0));
		Log.d(TAG,"ROTATION_MODE_SET = " + Settings.System.getIntForUser(mContext.getContentResolver(),Settings.System.ROTATION_MODE_SET,0,UserHandle.USER_CURRENT));
		
		if(Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.AIRPLANE_MODE_ON,0)==1){
			if(Settings.System.getIntForUser(mContext.getContentResolver(),Settings.System.AIRPLANE_MODE_SET,
				1, UserHandle.USER_CURRENT)==1){
				Settings.Global.putInt(mContext.getContentResolver(), Settings.System.AIRPLANE_MODE_SET,0);
