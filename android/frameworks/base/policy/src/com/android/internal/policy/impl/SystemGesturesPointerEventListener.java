...
public class SystemGesturesPointerEventListener implements PointerEventListener {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
				...
				if (event.getPointerCount() > 1) {
					final int sweep = detectSweep(event);
					mSwipeFireable = sweep == SWIPE_NONE;
					if (sweep == SWIPE_FROM_RIGHT) {
						mCallbacks.onSweepFromRight();
					}
					break;
				}
	...
	private int detectSweep(MotionEvent move) {
		...
		for (int p = 0; p < 2; p++) {
			...
			if (i != UNTRACKED_POINTER) {
				for (int h = 0; h < historySize; h++) {
					...
					if (i == 1) {
						return sweeps[0]==sweeps[1]?sweeps[0]:SWIPE_NONE;
					}
				}
				if(i < 2)
