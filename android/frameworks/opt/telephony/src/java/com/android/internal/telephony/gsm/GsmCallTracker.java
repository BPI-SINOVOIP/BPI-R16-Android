    ...
    try{
        if(SystemProperties.getInt("rw.talkingstandby.enabled", 0) == 1){
            ...
            if(currentCallState == 0){
				Rlog.d(LOG_TAG,"sceneunlock write talking_standby");
                mSceneUnlockStream.write("talking_standby".getBytes("utf-8"));
            }else{
                Rlog.d(LOG_TAG,"scenelock write talking_standby");
                mSceneLockStream.write("talking_standby".getBytes("utf-8"));
            }
       }
    }catch(IOException e){
       Rlog.d(LOG_TAG,"new FileOutputStream or write faild" );
    }
    finally{
        if(mSceneLockStream != null){
            try{
                ...
                Rlog.d(LOG_TAG,"close mSceneLockStream failed");
            }
        }
        if(mSceneUnlockStream != null){
            try{
                ...
                Rlog.d(LOG_TAG,"close mSceneUnlockStream failed");
            }
        }
   }
}
