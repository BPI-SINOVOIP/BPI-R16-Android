...
public class VCardComposer {
    ...
    public String buildVCard(final Map<String, List<ContentValues>> contentValuesListMap) {
        if (contentValuesListMap == null) {
            ...
            //Broadcom enhancement: export other fields only if this is a not a name+number only e
            if ( (mVCardType & VCardConfig.FLAG_NAME_NUMBER_ONLY_EXPORT) !=0) {
                if (DEBUG) Log.d(LOG_TAG, "FLAG_NAME_NUMBER_ONLY_EXPORT is set...Returning only name and number...");
                return builder.toString();
            }
            //Broadcom enhancement: allow refraining other field types
            if ((mVCardType & VCardConfig.FLAG_REFRAIN_EMAIL) == 0) {
                builder.appendEmails(contentValuesListMap.get(Email.CONTENT_ITEM_TYPE));
            }
            if ((mVCardType & VCardConfig.FLAG_REFRAIN_ADDRESS) == 0) {
                builder.appendPostals(contentValuesListMap.get(StructuredPostal.CONTENT_ITEM_TYPE));
            }
            if ((mVCardType & VCardConfig.FLAG_REFRAIN_ORGANIZATION) == 0) {
                builder.appendOrganizations(contentValuesListMap.get(Organization.CONTENT_ITEM_TYPE));
            }
            if ((mVCardType & VCardConfig.FLAG_REFRAIN_WEBSITES) == 0) {
