...
void ScreenRecoveryUI::draw_screen_locked()
{
    ...
    if (show_text) {
        ...
        if (show_menu) {
            ...
            char tmp[255];
            sprintf(tmp, "show=%d,max=%d", menu_show_lines, menu_max_lines);
            gr_text(x, 0, tmp, 1);
            SetColor(HEADER);

            for (; i < menu_top ; ++i) {
                if (i != 0 && menu[i][0]) gr_text(x, y, menu[i], i < menu_top);
                y += char_height+4;
            }
            ...
            for (i = menu_top + menu_show_start; i < menu_top + menu_show_start + menu_show_lines; ++i) {
                if (i == menu_top + menu_sel) {
                    ...
                    if (menu[i][0]) gr_text(x, y, menu[i], 1);
                    SetColor(MENU);
                } else {
                    if (menu[i][0]) gr_text(x, y, menu[i], i < menu_top);
                }
                y += char_height+4;
            }
            SetColor(MENU);
