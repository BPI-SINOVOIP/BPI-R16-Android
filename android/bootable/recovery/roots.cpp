...
int ensure_path_mounted(const char* path) {
    ...
    if (strcmp(v->fs_type, "yaffs2") == 0) {
        ...
        // if the device is /dev/block/mmcblk0, check /dev/block/mmcblk0p1 again, winner, 2014/1/6 18:58:32
        if(strcmp(v->blk_device,"/dev/block/mmcblk0") == 0) {
            result = mount("/dev/block/mmcblk0p1", v->mount_point, v->fs_type,
