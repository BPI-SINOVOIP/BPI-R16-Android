...
static int get_framebuffer(GGLSurface *fb)
{
    ...
    if(90 == rotation || 270 == rotation) {
          ...
          fi.line_length = vi.xres * PIXEL_SIZE;
    }
...
void* rotate_90(void *_dst,const void *_src)
{
    int pixelSize = PIXEL_SIZE,size;
    unsigned char *dst = _dst;
...
void* rotate_270(void *_dst,const void *_src)
{
    int pixelSize = PIXEL_SIZE,size;
    unsigned char *dst = _dst;
    const unsigned char *src = _src;
...
void gr_flip(void)
{
     ...
     * to make active. */
    if(90 == rotation) {
             rotate_90(gr_framebuffer[gr_active_fb].data, gr_mem_surface.data);
    } else if(180 == rotation) {
        rotate_180(gr_framebuffer[gr_active_fb].data, gr_mem_surface.data,
                   fi.line_length * vi.yres);
    } else if(270 == rotation) {
            rotate_270(gr_framebuffer[gr_active_fb].data, gr_mem_surface.data);
    } else {
