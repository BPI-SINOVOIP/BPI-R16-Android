...
#include "bmessage/bmessage_util.h"

/*
** Static constant data declarations
*/
/* Charset */
static const char * bmsg_body_charset[] =
{
...
**
** Description      Parses a tag from the stream.  Basically this any text
                                  ...
                                  tBTA_MA_STREAM * p_stream)
{
    ...
    while ( bta_ma_get_tag(p_stream, sz, BTA_MA_MAX_SIZE) )
    {
        ...
        else if ( strstr(sz, psztoken_fn) == sz  )
        {
            p_src = sz + strlen(psztoken_fn) - 1; // move to (first) semicolon, not past it
            len = strlen(p_src);
            if ( (len < BTA_MA_MAX_SIZE) && bta_ma_get_param(p_src, param))
            {
                bta_ma_get_value(p_stream, sz, BTA_MA_MAX_SIZE);
                BTA_MaBmsgAddVcardProp(p_vcard, BTA_MA_VCARD_PROP_FN, sz, param);
            }
        }
        else if ( ( strcmp(sz, psztoken_tel ) == 0 )|| ( strstr(sz, psztoken_tel) == sz )  )
        {
            bta_ma_get_value(p_stream, sz, BTA_MA_MAX_SIZE);
...
void bta_ma_stream_body(tBTA_MA_STREAM * p_stream, tBTA_MA_BMSG_BODY * p_body)
{
    ...
    if ( p_stream && p_body )
    {
        ...
        /* Body content length */
        bta_ma_stream_str(p_stream, "\r\nLENGTH:");
        //Note: please note the magic number.  This is to accomodate the BEGIN and END of the body
        //Otherwise some of the interop does not happen.
        bta_ma_stream_value(p_stream, bta_ma_get_body_length(p_body) + 22);
