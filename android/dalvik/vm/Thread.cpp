...
void getCallingProcessName(u4 pid,char *name)
{
	char proc_node[128];
...
static void threadExitUncaughtException(Thread* self, Object* group)
{
    ...
    ALOGW("1 threadid=%d: systemTid:%d thread exiting with uncaught exception (group=%p)",
        self->threadId, self->systemTid,group);
    assert(group != NULL);

    char  mCallingProcess[256]={0};
    getCallingProcessName(self->systemTid,mCallingProcess);
    ALOGW("mCallingProcess:%s",mCallingProcess);
