...
public class MmsProvider extends ContentProvider {
    ...
    static final int MMS_MAX_FILE_NAME_LENGTH = 230;
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (table.equals(TABLE_PDU)) {
            ...
            if (!plainText && !smilText) {
                ...
                int filenameLen = contentLocation.getBytes().length;

                if (!TextUtils.isEmpty(contentLocation)) {
                    File f = new File(contentLocation);
                    if(filenameLen > MMS_MAX_FILE_NAME_LENGTH){
                        contentLocation = f.getName();
                    }
                    else{
                ...
                // permission settings.
                String path = null;
                //Chinese character, too long
                if(filenameLen > MMS_MAX_FILE_NAME_LENGTH + 20){
                    int index = contentLocation.lastIndexOf(".");
                    path = getContext().getDir("parts", 0).getPath() + 
                       "/PART_" + System.currentTimeMillis() + contentLocation.substring(index,  contentLocation.length());
                }
                //Max length
                else if((contentLocation.getBytes().length) > MMS_MAX_FILE_NAME_LENGTH){
                    path = getContext().getDir("parts", 0).getPath() + "/" + contentLocation;
                }
                //normal
                else{
                    path = getContext().getDir("parts", 0).getPath()
