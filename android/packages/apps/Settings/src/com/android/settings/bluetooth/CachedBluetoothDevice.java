...
final class CachedBluetoothDevice implements Comparable<CachedBluetoothDevice> {
    ...
    boolean isConnected() {
        ...
        if(isRcConnectedDevice(mDevice)) {
            Log.d(TAG, "isConnected() Found an RC connected device.");
            return true;
        }
        return false;
    }
    ...
    boolean isRcConnectedDevice(BluetoothDevice device) {
        A2dpProfile a2dpProfile = mProfileManager.getA2dpProfile();
        if(a2dpProfile == null)  return false;
        List<BluetoothDevice> devices = a2dpProfile.getRcConnectedDevices();
        if(devices == null || devices.isEmpty())
            return false;
        return device.equals(devices.get(0));
    }
