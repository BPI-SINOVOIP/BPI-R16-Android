...
import com.broadcom.bt.app.settings.SapProfile;
import com.broadcom.bt.service.sap.BluetoothSap;
import com.broadcom.bt.app.settings.DunProfile;
...
final class LocalBluetoothProfileManager {
    ...
    private final DunProfile mDunProfile;
    private final PbapServerProfile mPbapProfile;
    private final SapProfile mSapProfile;
    private HfDeviceProfile mHfDeviceProfile;
    ...
    LocalBluetoothProfileManager(Context context,
       ...
       mDunProfile = new DunProfile(context);
       ...
       // as we do not need to monitor the profile as part of profile list
        ...
        //Add SAP profile  (Only for providing Disconnection function)
   ...
   synchronized void updateLocalProfiles(ParcelUuid[] uuids, boolean isUUIDChanged) {
    ...
    Log.d(TAG,"isUuidPresent-BluetoothUuid.AudioSink= "+
        ...
        if ((BluetoothUuid.isUuidPresent(uuids, BluetoothUuid.AudioSink)) ||
            ...
            if (mA2dpProfile == null) {
                Log.d(TAG, "Adding local A2DP profile is sink="+
                    BluetoothUuid.isUuidPresent(uuids, BluetoothUuid.AudioSink)
                    +"is source"+(BluetoothUuid.isUuidPresent(uuids, BluetoothUuid.AudioSource)));
                mA2dpProfile = new A2dpProfile(mContext, mLocalAdapter, mDeviceManager, this);
                addProfile(mA2dpProfile, A2dpProfile.NAME,
                        BluetoothA2dp.ACTION_CONNECTION_STATE_CHANGED);
            } else if (isUUIDChanged){ // The  mA2dpProfile already exists not remove and add new
                Log.d(TAG, "Remove and Add local A2DP profile is sink="+
                    BluetoothUuid.isUuidPresent(uuids, BluetoothUuid.AudioSink)
                    +"is source"+(BluetoothUuid.isUuidPresent(uuids, BluetoothUuid.AudioSource)));
                removeProfile(mA2dpProfile, A2dpProfile.NAME,
                    BluetoothA2dp.ACTION_CONNECTION_STATE_CHANGED);
                mA2dpProfile.finalize();
                mA2dpProfile =  null;

                mDeviceManager.updateRemoteProfiles();
    ...
    synchronized void updateProfiles(ParcelUuid[] uuids, ParcelUuid[] localUuids,
        ...
        if ((mMapProfile != null) &&
         ...
         /*If Local Profile role is server, remote profile may not have the
          UUID for the client role in SDP. (e.g SAP)  and hence those profiles needs to be
          added back to the list */
        if ((removedProfiles.contains(mSapProfile)) && mSapProfile != null) {
