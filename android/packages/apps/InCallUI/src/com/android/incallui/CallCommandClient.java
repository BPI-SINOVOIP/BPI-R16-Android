...
public class CallCommandClient {
    ...
    public void record(boolean onOff) {
        Log.i(this, "record: " + onOff);
        if (mCommandService == null) {
            Log.e(this, "Cannot record call; CallCommandService == null");
            return;
        }

      	int curRecordState = mVoiceRecorder.getState();
        Log.d(this, "onRecordClick: curRecordState = " + curRecordState);
	
        if(curRecordState != 1 && onOff) {
           mVoiceRecorder.startRecording(MediaRecorder.OutputFormat.THREE_GPP, ".3gp");
        } else {
            mVoiceRecorder.stop();
        }        
    }
