...
static jmethodID method_authorizeRequestCallback;
...
static void authorize_request_callback(bt_bdaddr_t *bd_addr, bt_service_id_t service_id){
    jbyteArray addr;

     ALOGV("%s: Service id:%d ", __FUNCTION__, service_id);
    ...
    callbackEnv->CallVoidMethod(sJniCallbacksObj, method_authorizeRequestCallback, addr,
		                        (jint)service_id);
...
static void classInitNative(JNIEnv* env, jclass clazz) {
    ...
    method_authorizeRequestCallback = env->GetMethodID(jniCallbackClass,
                                                    "authorizeRequestCallback", "([BI)V");
...
static jboolean authorizeServiceNative(JNIEnv *env, jobject obj, jbyteArray address,
                                           jint serviceId, jboolean accept, jboolean saveSettings){
    ALOGV("%s:",__FUNCTION__);
    ...
    int ret = sBluetoothInterface->authorize_response((bt_bdaddr_t*)addr,(bt_service_id_t)serviceId,
		                                              accept, saveSettings);
...
static void initWakeLockNative(JNIEnv *env, jobject obj, jint lock_fd, int unlock_fd) {
    ALOGV("%s: lock fd:%d, unlock fd:%d",__FUNCTION__, lock_fd, unlock_fd);
    if (!sBluetoothInterface) {
        close(lock_fd);
        close(unlock_fd);
        return;
    }
    sBluetoothInterface->init_wake_lock(lock_fd, unlock_fd);
}
...
static int sspDebugConfigureNative(JNIEnv *env, jobject object, jboolean enable)
{
    ALOGV("%s:",__FUNCTION__);
    ...
    int ret = sBluetoothInterface->ssp_debug_configure(enable);
    result = (ret == BT_STATUS_SUCCESS) ? JNI_TRUE : JNI_FALSE;
...
static int dutModeConfigureNative(JNIEnv *env, jobject object, jboolean enable)
{
    ALOGV("%s:",__FUNCTION__);
    ...
    int ret = sBluetoothInterface->dut_mode_configure(enable);
    result = (ret == BT_STATUS_SUCCESS) ? JNI_TRUE : JNI_FALSE;

    return result;
}

static jboolean updateRadioCountNative(JNIEnv *env, jobject object, jint type, jint count)
{
    ALOGV("%s:",__FUNCTION__);
    ...
    int ret = sBluetoothInterface->update_radio_count_native((bt_radio_type_t)type, count);
...
static jboolean setSdpRecordNative(JNIEnv *env, jobject obj, jboolean enable, jint uuid) {
    ALOGV("%s:",__FUNCTION__);

    jboolean result = JNI_FALSE;
    if (!sBluetoothInterface) return result;

    int ret = sBluetoothInterface->set_sdp_record(enable, uuid);
    result = (ret == BT_STATUS_SUCCESS) ? JNI_TRUE : JNI_FALSE;
    return result;
}

static void forceDisableNative(JNIEnv *env, jobject obj) {
    ALOGV("%s: ",__FUNCTION__);
    if (!sBluetoothInterface) {
        return;
    }
    sBluetoothInterface->force_disable();
}
...
static JNINativeMethod sMethods[] = {
    ...
    {"authorizeServiceNative", "([BIZZ)Z", (void*) authorizeServiceNative},
    {"getRemoteServicesNative", "([B)Z", (void*) getRemoteServicesNative},
    {"connectSocketNative", "([BI[BII)I", (void*) connectSocketNative},
    {"createSocketChannelNative", "(ILjava/lang/String;[BII)I",
     (void*) createSocketChannelNative},
    {"initWakeLockNative", "(II)V", (void*)initWakeLockNative},
    {"sspDebugConfigureNative", "(Z)Z", (void*) sspDebugConfigureNative},
    {"configHciSnoopLogNative", "(Z)Z", (void*) configHciSnoopLogNative},
    {"dutModeConfigureNative", "(Z)Z", (void*) dutModeConfigureNative},
    {"setSdpRecordNative", "(ZI)Z", (void*) setSdpRecordNative},
    {"forceDisableNative", "()V", (void*) forceDisableNative},
    {"updateRadioCountNative", "(II)Z", (void*) updateRadioCountNative},
...
jint JNI_OnLoad(JavaVM *jvm, void *reserved)
{
   ...
   if ((status = android::register_com_broadcom_bluetooth_hfdevice(e)) < 0) {
       ALOGE("jni hf-device registration failure, status: %d", status);
      return JNI_ERR;
   }
   ...
   //BRCM Enhancement: additional profiles
   ...
   if ((status = android::register_com_broadcom_bt_service_ftp_FtpService(e)) < 0) {
       ALOGE("jni FTP Server registration failure: %d", status);
      return JNI_ERR;
   }

   if ((status = android::register_com_broadcom_bt_service_sap(e)) < 0) {
       ALOGE("jni SAP registration failure: %d", status);
      return JNI_ERR;
   }

   if ((status = android::register_com_broadcom_bt_service_hid_device(e)) < 0) {
       ALOGE("jni HID Device registration failure: %d", status);
      return JNI_ERR;
   }

   if ((status = android::register_com_broadcom_bluetooth_rcc(e)) < 0) {
       ALOGE("jni AVRCP CT registration failure: %d", status);
      return JNI_ERR;
   }

   if ((status = android::register_com_broadcom_bt_service_opp(e)) < 0) {
       ALOGE("jni OPP Service registration failure: %d", status);
      return JNI_ERR;
   }

   if ((status = android::register_com_broadcom_bluetooth_mce(e)) < 0) {
       ALOGE("jni map mce registration failure: %d", status);
      return JNI_ERR;
   }
