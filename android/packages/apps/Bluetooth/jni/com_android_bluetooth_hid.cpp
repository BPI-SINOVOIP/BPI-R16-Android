...
static jint convert_report_type(jbyte report_type)
{
    switch(report_type)
    {
        case 0: /*BluetoothInputDevice.REPORT_TYPE_INPUT*/
            return BTHH_INPUT_REPORT;
        case 1: /*BluetoothInputDevice.REPORT_TYPE_OUTPUT*/
            return BTHH_OUTPUT_REPORT;
        case 2: /*BluetoothInputDevice.REPORT_TYPE_FEATURE*/
            return BTHH_FEATURE_REPORT;
        default:
            ALOGE("Fail to covert report type (%d) from JAVA", report_type);
            return -1;
    }
}
...
static void connection_state_callback(bt_bdaddr_t *bd_addr, bthh_connection_state_t state) {
    ...
    if (!addr) {
        ALOGE("Failed to get new jbyteArray bd addr for HID channel state");
...
static void get_protocol_mode_callback(bt_bdaddr_t *bd_addr, bthh_status_t hh_status,
    ...
    addr = sCallbackEnv->NewByteArray(sizeof(bt_bdaddr_t));
    if (!addr) {
        ALOGE("Failed to get new jbyteArray bd addr for get protocal mode callback");
        checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
        return;
    }
    sCallbackEnv->SetByteArrayRegion(addr, 0, sizeof(bt_bdaddr_t), (jbyte *) bd_addr);
...
static void get_report_callback(bt_bdaddr_t *bd_addr, bthh_status_t hh_status, uint8_t *rpt_data, jint rpt_size) {
    ALOGD("call to get_report_callback");
    ...
    if(rpt_data != NULL && rpt_size > 0)
    {
        rpt = sCallbackEnv->NewByteArray(rpt_size);
        if (!rpt) {
            ALOGE("Failed to get new jbyteArray rpt size for HID channel state");
            checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
            return;
        }

        sCallbackEnv->SetByteArrayRegion(rpt, 0, rpt_size, (jbyte *) rpt_data);
    }

    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onGetReport, addr, (jint) hh_status,
        rpt, rpt_size);
    checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
    sCallbackEnv->DeleteLocalRef(addr);
    ...
    if(rpt != NULL)
    {
        sCallbackEnv->DeleteLocalRef(rpt);
    }
}

static void set_report_callback(bt_bdaddr_t *bd_addr, bthh_status_t hh_status) {
    ALOGD("call to set_report_callback");
    jbyteArray addr;
    ...
    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onSetReport, addr, (jint) hh_status);
    checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
    sCallbackEnv->DeleteLocalRef(addr);
}
...
//    bt_status_t status;
    ...
    method_onGetReport = env->GetMethodID(clazz, "onGetReport", "([BI[BI)V");
    method_onSetReport = env->GetMethodID(clazz, "onSetReport", "([BI)V");
...
static jboolean getProtocolModeNative(JNIEnv *env, jobject object, jbyteArray address) {
    ...
    if ( (status = sBluetoothHidInterface->get_protocol((bt_bdaddr_t *) addr,
            (bthh_protocol_mode_t) protocolMode)) != BT_STATUS_SUCCESS) {
...
static jboolean setProtocolModeNative(JNIEnv *env, jobject object, jbyteArray address,
    ...
    if ( (status = sBluetoothHidInterface->set_protocol((bt_bdaddr_t *) addr, mode)) !=
             BT_STATUS_SUCCESS) {
        ALOGE("Failed to set protocol mode, status: %d", status);
        ret = JNI_FALSE;
    }
    env->ReleaseByteArrayElements(address, addr, 0);
...
static jboolean getReportNative(JNIEnv *env, jobject object, jbyteArray address,
        jbyte reportType, jbyte reportId, jint bufferSize) {
    ALOGD("%s: reportType = %d, reportId = %d, bufferSize = %d", __FUNCTION__,
    ...
    jint rType = convert_report_type(reportType);
    jint rId = reportId;

    if ( (status = sBluetoothHidInterface->get_report((bt_bdaddr_t *) addr,
            (bthh_report_type_t) rType, (uint8_t) rId, bufferSize)) != BT_STATUS_SUCCESS) {
        ALOGE("Failed to get report, status: %d", status);
        ret = JNI_FALSE;
    }
    env->ReleaseByteArrayElements(address, addr, 0);
...
static jboolean setReportNative(JNIEnv *env, jobject object, jbyteArray address,
    ...
    if ( (status = sBluetoothHidInterface->set_report((bt_bdaddr_t *) addr,
            (bthh_report_type_t)rType, (char*) c_report)) != BT_STATUS_SUCCESS) {
        ALOGE("Failed to set report, status: %d", status);
        ret = JNI_FALSE;
    }
...
static jboolean sendDataNative(JNIEnv *env, jobject object, jbyteArray address, jstring report) {
    ...
    const char *c_report = env->GetStringUTFChars(report, NULL);
    if ( (status = sBluetoothHidInterface->send_data((bt_bdaddr_t *) addr,
            (char*) c_report)) != BT_STATUS_SUCCESS) {
        ALOGE("Failed to send data, status: %d", status);
        ret = JNI_FALSE;
    }
    env->ReleaseStringUTFChars(report, c_report);
    env->ReleaseByteArrayElements(address, addr, 0);
