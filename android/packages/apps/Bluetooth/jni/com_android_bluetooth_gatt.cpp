...
static jmethodID method_onMonitorRssi;
static jmethodID method_onAdvFilterConfig;
...
void btgattc_monitor_rssi_cb(int client_if, bt_bdaddr_t* bda, int rssi)
{
    CHECK_CALLBACK_ENV
    ...
    snprintf(c_address, sizeof(c_address),"%02X:%02X:%02X:%02X:%02X:%02X",
             ...
             bda->address[3], bda->address[4], bda->address[5]);
    jstring address = sCallbackEnv->NewStringUTF(c_address);
    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onMonitorRssi,
                                 client_if, address, rssi);
    sCallbackEnv->DeleteLocalRef(address);
    checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
}

void btgattc_adv_filter_cb(int action, int status)
{
    CHECK_CALLBACK_ENV
    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onAdvFilterConfig,
                                 action, status);
    checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
}
...
static const btgatt_client_callbacks_t sGattClientCallbacks = {
    ...
    btgattc_monitor_rssi_cb,
    btgattc_adv_filter_cb,
    btgattc_listen_cb
...
static void classInitNative(JNIEnv* env, jclass clazz) {
    ...
    /* BRCM Extensions */
    method_onMonitorRssi = env->GetMethodID(clazz, "onMonitorRssi", "(ILjava/lang/String;I)V");
    method_onAdvFilterConfig = env->GetMethodID(clazz, "onAdvFilterConfig", "(II)V");
    /* /BRCM Extensions */
...
static void gattClientMonitorRssiNative(JNIEnv* env, jobject object, jint clientif,
                jstring address, jint rssi_low, jint rssi_range, jint rssi_high)
{
    if (!sGattIf) return;
    ...
    sGattIf->client->monitor_rssi(clientif, &bda, rssi_low, rssi_range, rssi_high);
}

static void gattClientAdvFilterEnableNative(JNIEnv* env, jobject object, jboolean enable)
{
    if (!sGattIf) return;
    sGattIf->client->adv_filter_enable(enable ? 1 : 0);
}

static void gattClientAdvFilterAddNative(JNIEnv* env, jobject object, jint type,
                jint company_id, jlong uuid_lsb, jlong uuid_msb,
                jstring name, jstring address, jbyte addr_type, jbyteArray data)
{
    ...
    switch(type)
    {
        case 0: // BTM_BLE_PF_ADDR_FILTER
            bt_bdaddr_t bda;
            jstr2bdaddr(env, &bda, address);
            sGattIf->client->adv_filter_add(type, 0, 0, 0, &bda, addr_type,0);
            break;

        case 1: // BTM_BLE_PF_SRVC_DATA
            sGattIf->client->adv_filter_add(type, 0,0,0,0,0,0);
            break;
        ...
        case 3: // BTM_BLE_PF_SRVC_SOL_UUID
        {
            ...
            sGattIf->client->adv_filter_add(type, 0,0, &uuid, 0,0,0);
            break;
        }
        ...
        case 4: // BTM_BLE_PF_LOCAL_NAME
        {
            ...
            {
                sGattIf->client->adv_filter_add(type, 0, strlen(c_name), 0, 0, 0,c_name);
        ...
        case 5: // BTM_BLE_PF_MANU_DATA
        {
            ...
            sGattIf->client->adv_filter_add(type, company_id, data_len, 0, 0, 0,(char*)data_array);
...
static void gattClientAdvFilterClearNative(JNIEnv* env, jobject object)
{
    if (!sGattIf) return;
    sGattIf->client->adv_filter_clear();
}
...
static void gattServerListenNative(JNIEnv *env, jobject object,
        jint server_if, jboolean start)
{
    if (!sGattIf) return;
    sGattIf->server->listen(server_if, start);
}
...
/* BRCM Extensions */
    {"gattClientMonitorRssiNative", "(ILjava/lang/String;III)V", (void *) gattClientMonitorRssiNative},
    {"gattClientAdvFilterEnableNative", "(Z)V", (void *) gattClientAdvFilterEnableNative},
    {"gattClientAdvFilterAddNative", "(IIJJLjava/lang/String;Ljava/lang/String;B[B)V", (void *) gattClientAdvFilterAddNative},
    {"gattClientAdvFilterClearNative", "()V", (void *) gattClientAdvFilterClearNative},

    {"gattServerListenNative", "(IZ)V", (void *) gattServerListenNative},
/* /BRCM Extensions */
