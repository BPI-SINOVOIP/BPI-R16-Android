...
namespace android {
enum   {
    FM_RDS_REG_EVT, /* RDS decoder user register event*/
    FM_RDS_PI_EVT, /* PI code */
    FM_RDS_PTY_EVT, /* program type */
    FM_RDS_TP_EVT, /* traffic program identification */
    FM_RDS_DI_EVT, /* Decoder Identification */
    FM_RDS_MS_EVT, /* music speech content */
    FM_RDS_AF_EVT, /* Alternative Frequency */
    FM_RDS_PS_EVT, /* Program service name */
    FM_RDS_PTYN_EVT, /* program type name */
    FM_RDS_RT_EVT, /* Radio Text */
    FM_RDS_PIN_EVT, /* program Item number */
    FM_RDS_CT_EVT, /* clock time */
    FM_RDS_ODA_EVT, /* open data application */
    FM_RDS_TDC_EVT, /* transparent data channel */
    FM_RDS_EWS_EVT, /* emergency warning system */
    FM_RDS_SLC_EVT, /* slow labelling code */
    FM_RDS_RPTOR_EVT, /* charcter repertoire set */
    FM_RDS_EON_EVT, /* EON information, should be evalute with sub code */
    FM_RDS_TMC_EVT, /* Traffic Message Channel */
    FM_RDS_RTP_EVT, /* RT plus event */
    FM_RDS_STATS_EVT
};

static const char *fmRdsEvtStr(int event)
{
    switch (event)
    {
    case FM_RDS_REG_EVT: return "";
    case FM_RDS_PI_EVT: return "FM_RDS_PI_EVT";
    case FM_RDS_PTY_EVT: return "FM_RDS_PTY_EVT";
    case FM_RDS_TP_EVT: return "FM_RDS_TP_EVT";
    case FM_RDS_DI_EVT: return "FM_RDS_DI_EVT";
    case FM_RDS_MS_EVT: return "FM_RDS_MS_EVT";
    case FM_RDS_AF_EVT: return "FM_RDS_AF_EVT";
    case FM_RDS_PS_EVT: return "FM_RDS_PS_EVT";
    case FM_RDS_PTYN_EVT: return "FM_RDS_PTYN_EVT";
    case FM_RDS_RT_EVT: return "FM_RDS_RT_EVT";
    case FM_RDS_PIN_EVT: return "FM_RDS_PIN_EVT";
    case FM_RDS_CT_EVT: return "FM_RDS_CT_EVT";
    case FM_RDS_ODA_EVT: return "FM_RDS_ODA_EVT";
    case FM_RDS_TDC_EVT: return "FM_RDS_TDC_EVT";
    case FM_RDS_EWS_EVT: return "FM_RDS_EWS_EVT";
    case FM_RDS_SLC_EVT: return "FM_RDS_SLC_EVT";
    case FM_RDS_RPTOR_EVT: return "FM_RDS_RPTOR_EVT";
    case FM_RDS_EON_EVT: return "FM_RDS_EON_EVT";
    case FM_RDS_TMC_EVT: return "FM_RDS_TMC_EVT";
    case FM_RDS_RTP_EVT: return "FM_RDS_RTP_EVT";
    case FM_RDS_STATS_EVT: return "FM_RDS_STATS_EVT";
    default:
        break;
    }
    return "FM_RDS Unknown";
}
...
static void af_jump_callback(int status, int rssi, int snr, int freq)
{
    ...
    ALOGI("[JNI] - TRANSMITTING EVENT BTA_FM_AF_JMP_EVT :  status = %d rssi = %d"
            " freq = %d", status, rssi, freq);
    if (processing_scan)
        processing_scan = 0;
...
static void rds_mode_callback(int status, BOOLEAN rdsOn, BOOLEAN afOn)
{
    ...
    ALOGD("%s: BTA_FM_RDS_MODE_EVT:: rdsOn: %d, afOn: %d", __FUNCTION__, rdsOn,
            afOn);
    ...
    if ((status == BT_STATUS_SUCCESS) && (rdsOn))
    {
...
static void rds_type_callback(int status, int rdsType)
{
    ...
    ALOGD("%s:BTA_FM_RDS_TYPE_EVT::status:%d, rdsType:%d", __func__, status, rdsType);
...
static void nfl_callback(int status, int noiseFloor)
    {
        ...
        ALOGI("nfl_callback(status: %d, noiseFloor %d", status, noiseFloor);
                    ...
                    char *radioText)
{
    ...
    ALOGD("rds_data_callback(status: %d (0==OK), dataType: %d (%s), index: %d, %p):"
            "BTA_FM_RDS_UPD_EVT", status , dataType, fmRdsEvtStr(dataType), index,
            radioText);
    if (NULL != radioText) {
        jstring rdsStr = sCallbackEnv->NewStringUTF(radioText);
        sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onRadioRdsUpdateEvent, (jint)status,
                (jint)dataType, (jint)index, rdsStr);
        if (rdsStr)
            sCallbackEnv->DeleteLocalRef(rdsStr);
    }
}
...
static void rtp_data_callback(btfm_rds_rtp_info_t *rtpInfo)
{
    ...
    ALOGD("%s: %s", __func__, fmRdsEvtStr(FM_RDS_RTP_EVT));
    if (NULL != rtpInfo) {
        int k = 0;

            ALOGD("%s: bRunning: %d bToggle = %d tag_num - %d",__FUNCTION__,
               rtpInfo->running,rtpInfo->tag_toggle,
               rtpInfo->tag_num);
           k = rtpInfo->tag_num;
           while(k){

                ALOGD("%s: content_type: %d start: %d len: %d",__FUNCTION__,
...
static jboolean setRdsRdbsNative(jint rdsType)
{
    ...
    ALOGI("[JNI] - setRdsRdbsNative: rdsType: %d", rdsType);
