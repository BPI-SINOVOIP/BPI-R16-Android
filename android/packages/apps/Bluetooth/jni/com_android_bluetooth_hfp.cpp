...
static jmethodID method_onBrsf;
...
static void unknown_at_callback(char *at_string,bt_bdaddr_t* bd_addr) {
    ...
    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onUnknownAt, js_at_string, addr);
    checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
...
static void brsf_callback(int local_feature, int peer_feature, bt_bdaddr_t* bd_addr) {
    jbyteArray addr;
    ...
    sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onBrsf, (jint) local_feature,
                                 (jint) peer_feature, addr);
    checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
    sCallbackEnv->DeleteLocalRef(addr);
}
...
static void classInitNative(JNIEnv* env, jclass clazz) {
    ...
    */
    ...
    method_onBrsf = env->GetMethodID(clazz, "onBrsf", "(II[B)V");
...
static jboolean copsResponseNative(JNIEnv *env, jobject object,
    ...
    if ( (status = sBluetoothHfpInterface->cops_response(operator_name,
...
static jboolean atResponseStringNative(JNIEnv *env, jobject object,
                                             jstring response_str, jbyteArray address) {
    jbyte *addr;
    bt_status_t status;
    const char *response;
    if (!sBluetoothHfpInterface) return JNI_FALSE;
...
static jboolean clccResponseNative(JNIEnv *env, jobject object, jint index, jint dir,
    ...
    if ( (status = sBluetoothHfpInterface->clcc_response(index, (bthf_call_direction_t) dir,
                     ...
                     number, (bthf_call_addrtype_t) type,
                     (bt_bdaddr_t *) addr)) != BT_STATUS_SUCCESS) {
