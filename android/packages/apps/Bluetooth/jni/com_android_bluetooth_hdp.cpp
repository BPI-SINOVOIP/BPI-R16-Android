...
static jmethodID method_onDeviceSourceDataType;
static jmethodID method_onDeviceSinkDataType;
...
static void source_datatype_callback (int app_id, bt_bdaddr_t *bd_addr,
                            int *source_datatypes, int size)
    {
        jbyteArray addr;
        jintArray datatypes;
        ...
        datatypes = (jintArray)sCallbackEnv->NewIntArray(size);
        if (datatypes == NULL) {
            ALOGE("%s: Error allocating int Array for datatypes", __FUNCTION__);
            return;
        }

        sCallbackEnv->SetIntArrayRegion(datatypes, 0, size,  (jint*)source_datatypes);

        sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onDeviceSourceDataType,app_id, addr,
                                            datatypes);
        checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
        sCallbackEnv->DeleteLocalRef(addr);
        sCallbackEnv->DeleteLocalRef(datatypes);
    }

static void sink_datatype_callback (int app_id, bt_bdaddr_t *bd_addr, int *sink_datatypes, int size)
    {
        jbyteArray addr;
        jintArray datatypes;
        ...
        sCallbackEnv->SetIntArrayRegion(datatypes, 0, size,  (jint*)sink_datatypes);

        sCallbackEnv->CallVoidMethod(mCallbacksObj, method_onDeviceSinkDataType,app_id, addr,
                                            datatypes);
        checkAndClearExceptionFromCallback(sCallbackEnv, __FUNCTION__);
        sCallbackEnv->DeleteLocalRef(addr);
        sCallbackEnv->DeleteLocalRef(datatypes);
...
static bthl_callbacks_t sBluetoothHdpCallbacks = {
    ...
    source_datatype_callback,
    sink_datatype_callback
};
...
//    bt_status_t status;
    ...
    method_onDeviceSourceDataType = env->GetMethodID(clazz, "onDeviceSourceDataType", "(I[B[I)V");
    method_onDeviceSinkDataType = env->GetMethodID(clazz, "onDeviceSinkDataType", "(I[B[I)V");
...
static jboolean getDeviceDatatypeNative(JNIEnv *env, jobject object,
                                 jbyteArray address, jint app_id) {
    bt_status_t status;
    jbyte *addr;
    jboolean state = JNI_TRUE;
    if (!sBluetoothHdpInterface) return JNI_FALSE;
    ...
    if ((status = sBluetoothHdpInterface->get_device_datatype(app_id,
                (bt_bdaddr_t *) addr)) != BT_STATUS_SUCCESS) {
        ALOGE("Failed getting device datatype, status: %d", status);
        state = JNI_FALSE;
    }
    env->ReleaseByteArrayElements(address, addr, 0);
...
static JNINativeMethod sMethods[] = {
    ...
    {"getDeviceDatatypeNative", "([BI)Z", (void *) getDeviceDatatypeNative},
};
