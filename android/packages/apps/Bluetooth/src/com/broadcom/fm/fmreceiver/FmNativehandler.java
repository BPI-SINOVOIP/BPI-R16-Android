...
public final class FmNativehandler  {
    ...
    private RadioStateChangeCallback mRadioStCallback;
    ...
    private void queueFMCommand (FMJob job) {
        ...
        msg.what = FmReceiverServiceState.OPERATION_QUEUE_FM_CMD;
        msg.obj= job;
    ...
    private void onQueueFMCommand(FMJob job) {
        if (job == null)
            return;
        Log.d(TAG, "onQueueFMCommand: add [" + job + "] to queue " + mFmQueue.toString());
                ...
                if (firstJob != null) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (existingJob.timeSent == 0) {
                            Log.d(TAG, "onQueueFMCommand: " + existingJob +
                                  " removed due to FM_OFF");
                            it.remove();
                        }
                    }
                }
    ...
    private boolean mWaitforRadioState = false;


    public FmNativehandler(Context context) {
        mContext = context;
        mRadioStCallback = new RadioStateChangeCallback();
    }
    ...
    public synchronized void stop() {
        ...
        unRegisterIntent();
        BluetoothRadioManager radioAdapter = BluetoothRadioManager.getRadioManager();
        mCleaningUp = true;

        if (radioAdapter.isRadioEnabled()) {
            ...
            if (FmReceiverServiceState.mRadioIsOn)
                radioAdapter.releaseRadioReference(BluetoothRadioManager.RADIO_TYPE_FM,
                                                   mRadioStCallback);
            initializeStateMachine();
        }
        cleanupNative();
        mRadioStCallback = null;
        mIsStarted = false;
    }
    ...
    public void finish() {
        ...
        if (operationHandler != null) {
            ...
            operationHandler.removeMessages(FmReceiverServiceState.OPERATION_QUEUE_FM_CMD);
    ...
    protected Handler operationHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            case FmReceiverServiceState.OPERATION_VOLUME_EVENT_CALLBACK:
                ...
                break;
            case FmReceiverServiceState.OPERATION_QUEUE_FM_CMD:
                Log.d(TAG, "handleMessage: OPERATION_QUEUE_FM_CMD");
                onQueueFMCommand((FMJob)msg.obj);
                break;
            default:
    ...
    private class RadioStateChangeCallback implements
                BluetoothRadioManager.BluetoothRadioStateChangeCallback {
        public void onRadioStateChange(boolean up) {
            if (V) Log.v(TAG, "Radio State Changed: up = " + up);
            if ((mWaitforRadioState) &&(up == true)) {
                mWaitforRadioState = false;
                queueFMCommand(new FMJob(FM_ON, mFunctionalityMask));
            }else if ((up == false) &&(FmReceiverServiceState.mRadioIsOn == true)) {
                if (mCleaningUp == false) mWaitforRadioState = true;
                FmReceiverServiceState.mRadioIsOn = false;
    ...
    public synchronized int process_enableChip() {
        ...
        if (radioAdapter.isRadioEnabled() == false) {
            mWaitforRadioState = true;
            radioAdapter.addRadioReference(BluetoothRadioManager.RADIO_TYPE_FM,mRadioStCallback);
        } else {
             /*BT  Radio is already enabled. Add Radio reference and & go ahead with  FM ON */
             radioAdapter.addRadioReference(BluetoothRadioManager.RADIO_TYPE_FM, mRadioStCallback);
             queueFMCommand(new FMJob(FM_ON, mFunctionalityMask));
        }
        return FmProxy.STATUS_OK;
    ...
    public synchronized int process_disableChip() {
        ...
        BluetoothRadioManager radioAdapter = BluetoothRadioManager.getRadioManager();
        mCleaningUp = true;
        radioAdapter.releaseRadioReference(BluetoothRadioManager.RADIO_TYPE_FM, mRadioStCallback);
    ...
    // Bypassed via jni rdsMode callback -> obsolete
    public void onRadioRdsTypeEvent(int status, int rdsType) {
        if (V) Log.d(TAG, "onRadioRdsTypeEvent(status:" + Integer.toString(status) +
                ", rdsType:" + Integer.toString(rdsType) + ")");
