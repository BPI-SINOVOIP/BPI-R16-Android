...
final class BondStateMachine extends StateMachine {
        ...
        public boolean processMessage(Message msg) {
            ...
            switch(msg.what) {
              ...
              case BONDING_STATE_CHANGE:
                ...
                {
                    if(isHidProfileSupported(dev) && newState == BluetoothDevice.BOND_NONE) {
                        Log.d(TAG, "Found a HID Device. Remove bond info.");
    ...
    private void setProfilePriorty (BluetoothDevice device){
        ...
        if ((avrcpService != null) &&
            (avrcpService.getPriority(device) == BluetoothProfile.PRIORITY_UNDEFINED)){
            avrcpService.setPriority(device,BluetoothProfile.PRIORITY_ON);
        }
    ...
    private void clearProfilePriorty (BluetoothDevice device){
        ...
        if (hidService != null) {
            ...
            hidService.removeDeviceFromUnpluggedList(device);
        }
        if(hidDeviceService != null)
            hidDeviceService.removeDeviceFromUnpluggedList(device);
        if(a2dpService != null)
        ...
        if(avrcpService != null)
            avrcpService.setPriority(device,BluetoothProfile.PRIORITY_UNDEFINED);
        if(hfDeviceService != null)
            hfDeviceService.setPriority(device,BluetoothProfile.PRIORITY_UNDEFINED);

    }

     private boolean isHidProfileSupported(BluetoothDevice device){
        if (device.getBondState()!= BluetoothDevice.BOND_BONDED){
            return false;
        }
        HidService hidHostService = HidService.getHidService();
        HidDeviceService hidDeviceService = HidDeviceService.getHidDeviceService();
        // if any of the profile service is  null, return
        if ((hidHostService == null) ||(hidDeviceService == null )){
            return false;
        }
        // Check if the device is present in any of the connected device list for HIDD and HIDH.
        // If so, return TRUE.
        if(hidHostService.isDeviceUnplugged(device) ||
            hidDeviceService.isDeviceUnplugged(device)) {
            Log.d(TAG, "Device ("+device+") is HID device");
            return true;
        }
        Log.d(TAG, "Device ("+device+") not a HID device");
        return false;
    }
