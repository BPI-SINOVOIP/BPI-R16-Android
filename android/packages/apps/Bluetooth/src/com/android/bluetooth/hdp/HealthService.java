...
public class HealthService extends ProfileService {
    ...
    private static final int MESSAGE_GET_DATATYPE = 5;
    private static final int MESSAGE_APP_REGISTRATION_CALLBACK = 11;
    private static final int MESSAGE_CHANNEL_STATE_CALLBACK = 12;
    private static final int MESSAGE_DEVICE_SOURCE_DATATYPE_CALLBACK = 13;
    private static final int MESSAGE_DEVICE_SINK_DATATYPE_CALLBACK = 14;
    ...
    private final class HealthServiceMessageHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MESSAGE_GET_DATATYPE:
                {
                    HealthInfo info = (HealthInfo) msg.obj;
                    byte[] devAddr = Utils.getByteAddress(info.mDevice);
                    int appId = (mApps.get(info.mConfig)).mAppId;
                    boolean status = getDeviceDatatypeNative(devAddr, appId);
                    if (!status)
                        Log.e(TAG,"GetDeviceDataType Failed");
                }
                    break;
                case MESSAGE_APP_REGISTRATION_CALLBACK:
                {
                    Log.e(TAG,"MESSAGE_APP_REGISTRATION_CALLBACK");
                    ...
                    break;
                case MESSAGE_DEVICE_SOURCE_DATATYPE_CALLBACK:
                {
                    RemoteDeviceConfig deviceConfig  = (RemoteDeviceConfig) msg.obj;
                    BluetoothHealthAppConfiguration appConfig =
                                                findAppConfigByAppId(deviceConfig.mAppId);
                    if (appConfig == null) break;

                    BluetoothDevice device = getDevice(deviceConfig.mAddr);
                    callDeviceSourceDataTypeCallback(appConfig, device, deviceConfig.mDataTypes);
                }
                    break;
                case MESSAGE_DEVICE_SINK_DATATYPE_CALLBACK:
                {
                    RemoteDeviceConfig deviceConfig  = (RemoteDeviceConfig) msg.obj;
                    BluetoothHealthAppConfiguration appConfig =
                                                findAppConfigByAppId(deviceConfig.mAppId);
                    ...
                    BluetoothDevice device = getDevice(deviceConfig.mAddr);
                    callDeviceSinkDataTypeCallback(appConfig, device, deviceConfig.mDataTypes);
                }
                    break;
...
//Handler for DeathReceipient
    ...
    private static class BluetoothHealthBinder extends IBluetoothHealth.Stub implements IProfileServiceBinder {
        ...
        public boolean getDeviceDatatype(BluetoothDevice device,
                                              BluetoothHealthAppConfiguration config) {
            HealthService service = getService();
            if (service == null) return false;
            return service.getDeviceDatatype(device, config);
        }
    ...
    boolean getDeviceDatatype(BluetoothDevice device,
                                   BluetoothHealthAppConfiguration config) {
        if (mApps.get(config) == null) {
            Log.e(TAG, "getDeviceDatatype fail to get a app id from config");
            return false;
        }

        HealthInfo info = new HealthInfo(device, config);

        Message msg = mHandler.obtainMessage(MESSAGE_GET_DATATYPE);
        msg.obj = info;
        mHandler.sendMessage(msg);
        return true;
    }
    ...
    private void onDeviceSourceDataType(int appId, byte[] addr, int[] dataTypes){
        Log.e(TAG, "onDeviceSourceDataType  appid ="+appId);
        Message msg = mHandler.obtainMessage(MESSAGE_DEVICE_SOURCE_DATATYPE_CALLBACK);
        RemoteDeviceConfig deviceConfig = new RemoteDeviceConfig(appId, addr, dataTypes);
        msg.obj = deviceConfig;
        mHandler.sendMessage(msg);

    }

    private void onDeviceSinkDataType(int appId, byte[] addr, int[] dataTypes){
        Log.e(TAG, "onDeviceSinkDataType  appid ="+appId);
        Message msg = mHandler.obtainMessage(MESSAGE_DEVICE_SINK_DATATYPE_CALLBACK);
        RemoteDeviceConfig deviceConfig = new RemoteDeviceConfig(appId, addr, dataTypes);
        msg.obj = deviceConfig;
        mHandler.sendMessage(msg);
    ...
    private void callDeviceSourceDataTypeCallback(BluetoothHealthAppConfiguration config,
                                        BluetoothDevice device, int[] dataTypes) {
        if (DBG) log ("Health Device Application: " + config);
        IBluetoothHealthCallback callback = (mApps.get(config)).mCallback;
        if (callback == null) {
            Log.e(TAG, "Callback object null");
        }
        ...
        try {
            callback.onHealthDeviceSourceDataTypeResult(config, device, dataTypes);
        } catch (RemoteException e) {
            Log.e(TAG, "Remote Exception:" + e);
        }
    }

    private void callDeviceSinkDataTypeCallback(BluetoothHealthAppConfiguration config,
                                        BluetoothDevice device, int[] dataTypes) {
        if (DBG) log ("Health Device Application: " + config);
        IBluetoothHealthCallback callback = (mApps.get(config)).mCallback;
        if (callback == null) {
        ...
        try {
            callback.onHealthDeviceSinkDataTypeResult(config, device, dataTypes);
        } catch (RemoteException e) {
            Log.e(TAG, "Remote Exception:" + e);
        }
    }
    ...
    private class HealthInfo {
        ...
        private HealthInfo(BluetoothDevice device, BluetoothHealthAppConfiguration config) {
             mDevice = device;
             mConfig = config;
        }
    ...
    // SDP result event from Hal
    private class RemoteDeviceConfig {
        int mAppId;
        byte[] mAddr;
        int[] mDataTypes;

        private RemoteDeviceConfig(int appId, byte[] addr, int[] dataTypes) {
            mAppId = appId;
            mAddr = addr;
    ...
    private native boolean getDeviceDatatypeNative(byte[] btAddress, int appId);
