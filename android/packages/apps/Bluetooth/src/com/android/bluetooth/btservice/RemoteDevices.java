...
final class RemoteDevices {
    ...
    private static final int AUTH_REQUEST_DELAY = 500;
    private static final int MESSAGE_UUID_INTENT = 1;
    private static final int MESSAGE_AUTH_REQUEST = 2;
    private static final int MAX_AUTH_DELAY_ATTEMPTS = 10;

    private HashMap<BluetoothDevice, DeviceProperties> mDevices;
    private BluetoothDevice mAuthDev = null;
    private static int mAuthPendingTout = 0;
    ...
    void authorizeRequestCallback(byte[] address, int serviceId) {
        ParcelUuid ServiceUUID = Utils.getProfileUUIDFromServiceId(serviceId);
        if (mAdapterService.isQuietModeEnabled())
        {
            /* Should this be done only for OPP..? */
            mAdapterService.authorizeService(mAdapter.getRemoteDevice(address),
                                             ServiceUUID,true,false);
        } else {
            if (BluetoothPbapService.AuthorizationPending() ==true){
                if (mAuthPendingTout < MAX_AUTH_DELAY_ATTEMPTS) {
                    mAuthPendingTout++;
                    BluetoothDevice device = getDevice(address);
                    Message message = mHandler.obtainMessage(MESSAGE_AUTH_REQUEST);
                          message.obj = device;
                          message.arg1 = serviceId;
                          mHandler.sendMessageDelayed(message, AUTH_REQUEST_DELAY);
                } else {
                    mAuthPendingTout = 0;
                    if (mAuthDev != null) {
                        mAuthDev.authorizeService(ServiceUUID, false, false);
                        mAuthDev = null;
                    }
                }
            }else {
                ...
                if(checkDeviceProfilePriority(address, serviceId)) {
                    return;
                }

                // Broadcast the  authorization request
                Intent intent = new Intent(BluetoothDevice.ACTION_AUTHORIZE_REQUEST);
                intent.putExtra(BluetoothDevice.EXTRA_DEVICE, mAdapter.getRemoteDevice(address));
                intent.putExtra(BluetoothDevice.EXTRA_UUID, ServiceUUID.toString());
     ...
     void aclStateChangeCallback(int status, byte[] address, int newState) {
        ...
        if (newState == AbstractionLayer.BT_ACL_STATE_CONNECTED) {
            ...
            //If outstanding Authorization request for this device is pending, silently ground it.
            if ((mHandler.hasMessages(MESSAGE_AUTH_REQUEST) == true) &&
                (mAuthDev != null) &&
                (mAuthDev.equals(device))){
                mHandler.removeMessages(MESSAGE_AUTH_REQUEST);
                mAuthDev = null;
            }
        }
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, device);
    ...
    private boolean checkDeviceProfilePriority(byte[] address, int serviceId) {
        ParcelUuid ServiceUUID = Utils.getProfileUUIDFromServiceId(serviceId);
        int priority = BluetoothProfile.PRIORITY_OFF;

        BluetoothDevice device = mAdapter.getRemoteDevice(address);
        if(device == null) {
            errorLog("Can't get device(" + address + ")...");
            return false;
        }

        if(BluetoothUuid.isHandsfree(ServiceUUID) || BluetoothUuid.isHeadset(ServiceUUID)) {
            debugLog("Checking HSP/HFP priority...");

            HeadsetService headsetService = HeadsetService.getHeadsetService();
            if(headsetService == null) {
                errorLog("headsetService is NULL... reject the request");
                mAdapterService.authorizeService(device, ServiceUUID, false, false);
                return true;
            }

            priority = headsetService.getPriority(device);

        } else if(BluetoothUuid.isAudioSource(ServiceUUID)
                || BluetoothUuid.isAvrcpTarget(ServiceUUID)) {
            debugLog("Checking AV priority...");

            A2dpService a2dpService = A2dpService.getA2dpService();
            if(a2dpService == null) {
                errorLog("a2dpService is NULL... reject the request");
                mAdapterService.authorizeService(device, ServiceUUID, false, false);
                return true;
            }

            priority = a2dpService.getPriority(device);

        } else if(BluetoothUuid.isInputDevice(ServiceUUID)) {
            debugLog("Checking HID priority...");

            HidService hidService = HidService.getHidService();
            if(hidService == null) {
                errorLog("hidService is NULL... reject the request");
                mAdapterService.authorizeService(device, ServiceUUID, false, false);
                return true;
            }

            priority = hidService.getPriority(device);

        } else {
            debugLog("Unhandled service id = " + serviceId);
            return false;
        }

        debugLog("profile priority = " + priority);
        if(priority == BluetoothProfile.PRIORITY_OFF) {
            mAdapterService.authorizeService(device, ServiceUUID, false, false);
        } else {
            mAdapterService.authorizeService(device, ServiceUUID, true, false);
        }
    ...
    private final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case MESSAGE_AUTH_REQUEST:
                ...
                mAuthPendingTout = 0;
                mAuthDev = dev;
                authorizeRequestCallback(Utils.getBytesFromAddress(dev.getAddress()), msg.arg1);
                break;
            }
        }
    };
