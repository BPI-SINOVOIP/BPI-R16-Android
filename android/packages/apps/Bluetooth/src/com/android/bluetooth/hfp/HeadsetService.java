...
public class HeadsetService extends ProfileService {
    ...
    //flag for maximum HFP connections that is used in AdapterService
    public static final int MAX_HFP_CONNECTIONS = HeadsetHalConstants.MAX_HFP_CONNECTIONS;
    ...
    public boolean connect(BluetoothDevice device) {
        ...
        //Headset Service is not supported in HEADSET_MODE
        if( devicemode == mService.HEADSET_MODE ) {
            Log.d(TAG,"HF connection not allowed in HEADSET_MODE");
            return false;
        }
    ...
    public boolean disconnect(BluetoothDevice device) {
        enforceCallingOrSelfPermission(BLUETOOTH_ADMIN_PERM,
                                       "Need BLUETOOTH ADMIN permission");
        Log.d(TAG, "disconnect("+device.getAddress()+")");
        int connectionState = mStateMachine.getConnectionState(device);
        if (connectionState != BluetoothProfile.STATE_CONNECTED &&
            connectionState != BluetoothProfile.STATE_CONNECTING) {
            return false;
