...
public class HidService extends ProfileService {
    ...
    /* List of recently unplugged HID devices. Need to maintain inorder to update
    Bond state correctly for incoming Virtual Unplug requests.*/
    private List<BluetoothDevice> mHidUnpluggedDevices;
    ...
    private static final int MESSAGE_ON_SET_REPORT = 13;
    ...
    protected boolean start() {
        ...
        mHidUnpluggedDevices = new ArrayList<BluetoothDevice>();
        initializeNative();
    ...
    protected boolean cleanup() {
        ...
        if(mHidUnpluggedDevices != null) {
            mHidUnpluggedDevices.clear();
        }
        clearHidService();
        return true;
    }
            ...
            switch (msg.what) {
                ...
                case MESSAGE_CONNECT_STATE_CHANGED:
                {
                    ...
                    if(halState == CONN_STATE_CONNECTED &&
                       prevState != BluetoothInputDevice.STATE_CONNECTING &&
                       (!okToConnect(device))) {
                        if (DBG) Log.d(TAG,"Incoming HID connection rejected");
                        disconnectHidNative(Utils.getByteAddress(device));
                    } else if ((halState == CONN_STATE_DISCONNECTING) &&
                              (prevState == BluetoothInputDevice.STATE_DISCONNECTED)){
                    // Ignore state CONN_STATE_DISCONNECTING received  in DISCONNECTED state
                    // This state info is received due  some race condtion in lower layers
                    //when remote device attempts to connect to host without pairing
                        Log.w(TAG,"Ignore disconnecting st for alreay disconnected dev");
                    } else {
                ...
                case MESSAGE_ON_GET_REPORT:
                {
                    ...
                    BluetoothDevice device = getDevice(b.getByteArray("a"));
                    int status = b.getInt("s", -1);
                    byte[] rpt = b.getByteArray("r");
                    int rpt_size = b.getInt("n", 0);
                    broadcastGetReportResult(device, status, rpt, rpt_size);
                }
                break;
                case MESSAGE_ON_SET_REPORT:
                {
                    BluetoothDevice device = getDevice((byte[]) msg.obj);
                    int status = msg.arg1;
                    broadcastSetReportStatus(device, status);
                }
                break;
            }
        }
    ...
    /* Function to check if the BluetoothDevice is an already un-plugged device */
    public boolean isDeviceUnplugged(BluetoothDevice device) {
        if(mHidUnpluggedDevices.size() == 0) {
            Log.w(TAG, "No unplugged devices");
            return false;
        }
        if(!mHidUnpluggedDevices.contains(device)) {
            Log.w(TAG, "Device ("+device+") not present in unplugged list");
            return false;
        }
        Log.d(TAG, "Device ("+device+") found in unplugged list");
        return true;
    }
    ...
    public void removeDeviceFromUnpluggedList(BluetoothDevice device) {
        ...
        if(mHidUnpluggedDevices.contains(device)) {
            Log.d(TAG, "Removing device ("+device+") from Virtual Unplugged list");
            mHidUnpluggedDevices.remove(device);
            return;
        }
        Log.d(TAG, "Failed to remove device ("+device+") from Virtual Unplugged list");
    }
    ...
    private void onGetReport(byte[] address, int status, byte[] rpt, int rpt_size) {
        Message msg = mHandler.obtainMessage(MESSAGE_ON_GET_REPORT);
        Bundle b = msg.getData();
        b.putByteArray("a", address);
        b.putInt("s", status);
        b.putByteArray("r", rpt);
        b.putInt("n", rpt_size);
        mHandler.sendMessage(msg);
    }

    private void onSetReport(byte[] address, int status) {
        Message msg = mHandler.obtainMessage(MESSAGE_ON_SET_REPORT);
        msg.obj = address;
        msg.arg1 = status;
        mHandler.sendMessage(msg);
    }
    ...
    private void broadcastVirtualUnplugStatus(BluetoothDevice device, int status) {
        ...
        // Also add the unplugged device to mHidUnpluggedDevices
        mHidUnpluggedDevices.add(device);
    }

    private void broadcastGetReportResult(BluetoothDevice device, int status, byte[] rpt_data,
            int rpt_size) {
        Intent intent = new Intent(BluetoothInputDevice.ACTION_GET_REPORT_RESULT);
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, device);
        intent.putExtra(BluetoothInputDevice.EXTRA_STATUS, status);
        intent.putExtra(BluetoothInputDevice.EXTRA_REPORT, rpt_data);
        intent.putExtra(BluetoothInputDevice.EXTRA_REPORT_BUFFER_SIZE, rpt_size);
        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
        sendBroadcast(intent, BLUETOOTH_PERM);
    }

    private void broadcastSetReportStatus(BluetoothDevice device, int status) {
        Intent intent = new Intent(BluetoothInputDevice.ACTION_SET_REPORT_STATUS);
        intent.putExtra(BluetoothDevice.EXTRA_DEVICE, device);
        intent.putExtra(BluetoothInputDevice.EXTRA_STATUS, status);
        intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
        sendBroadcast(intent, BLUETOOTH_PERM);
