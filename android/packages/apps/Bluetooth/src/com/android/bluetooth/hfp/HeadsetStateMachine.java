...
final class HeadsetStateMachine extends StateMachine {
    ...
    private static final int CLCC_RSP_TIMEOUT = 103;
    ...
    //                        Both mCurrentDevice and mTargetDevice are null
    private ArrayList<BluetoothDevice> mCurrentDevicesList = new ArrayList<BluetoothDevice>();
    ...
    public void cleanup() {
        ...
        int listsize = mCurrentDevicesList.size();
        if (listsize != 0) {
            Log.d(TAG,"mCurrentDevicesList.size="+listsize);
            while(0 < listsize) {
                // Since this is cleanup send device disconnected to clients
                broadcastConnectionState(mCurrentDevicesList.get(listsize-1),
    ...
    private class Disconnected extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            if (mCurrentDevicesList.size() != 0 || mTargetDevice != null || mIncomingDevice != null) {
            ...
            case HeadsetHalConstants.CONNECTION_STATE_CONNECTED:
                ...
                if (okToConnect(device)){
                    ...
                    broadcastConnectionState(device, BluetoothProfile.STATE_CONNECTED,
                                             BluetoothProfile.STATE_DISCONNECTED);
                    synchronized (HeadsetStateMachine.this) {
                        if(!mCurrentDevicesList.contains(device)) {
                            mCurrentDevicesList.add(device);
                            Log.d(TAG, "device of address" + device.getAddress() + " is adding " );
                        }
        ...
        private void startOtherProfiles(BluetoothDevice device){
    ...
    private class Pending extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case DISCONNECT:
                    BluetoothDevice device = (BluetoothDevice) message.obj;
                    if (mCurrentDevicesList.contains(device) && mTargetDevice != null &&
                ...
                case STACK_EVENT:
                    ...
                    switch (event.type) {
                        ...
                        case EVENT_TYPE_AT_CHLD:
                            ...
                            break;
                        case EVENT_TYPE_BRSF:
                            processBrsf(event.valueInt, event.valueInt2, event.device);
                            break;
                        default:
            ...
            switch (state) {
                case HeadsetHalConstants.CONNECTION_STATE_DISCONNECTED:
                    if (mCurrentDevicesList.contains(device)) {
                        ...
                        synchronized (HeadsetStateMachine.this) {
                            mCurrentDevicesList.remove(device);
                            Log.d(TAG, "device of address " + device.getAddress() + " is removed in Pending state");
                        }
                            ...
                            if (!connectHfpNative(getByteAddress(mTargetDevice))) {
                                ...
                                synchronized (HeadsetStateMachine.this) {
                                    ...
                                    if(mCurrentDevicesList.size() == 0) {
                                        ...
                                        processDualHFConnected(isInCall());
                                    }
                                }
                            }
                        } else {
            ...
            case HeadsetHalConstants.CONNECTION_STATE_CONNECTED:
                if (mCurrentDevicesList.contains(device)) {
                    ...
                    synchronized (HeadsetStateMachine.this) {
                        if(!mCurrentDevicesList.contains(device)) {
                            mCurrentDevicesList.add(device);
                            Log.d(TAG, "device of address " + device.getAddress() + " is added in Pending state");
                            mTargetDevice = null;
                            processStateTransition();
                        }
                    }
        ...
        private void processDualHFConnected(boolean inCallFlag) {
            broadcastConnectionState(mCurrentDevicesList.get(0), BluetoothProfile.STATE_CONNECTED,
                             BluetoothProfile.STATE_CONNECTING);
            transitionTo(mConnected);
            configAudioParameters(mCurrentDevicesList.get(0));
            if(inCallFlag) {
                connectAudioNative(getByteAddress(mCurrentDevicesList.get(0)));
             }
        }
    ...
    private class Connected extends State {
                ...
                case STACK_EVENT:
                    ...
                    switch (event.type) {
                        ...
                        case EVENT_TYPE_NOICE_REDUCTION:
                            Log.e(TAG, "EVENT_TYPE_NOICE_REDUCTION NREC is "+event.valueInt);
                            processNoiceReductionEvent(event.valueInt, event.device);
                            break;
                        case EVENT_TYPE_WBS:
                            Log.e(TAG, "EVENT_TYPE_WBS codec is "+event.valueInt);
        ...
        private void processConnectionEvent(int state, BluetoothDevice device) {
            switch (state) {
                case HeadsetHalConstants.CONNECTION_STATE_DISCONNECTED:
                    if (mCurrentDevicesList.contains(device)) {
                        ...
                        synchronized (HeadsetStateMachine.this) {
                            mCurrentDevicesList.remove(device);
                            Log.d(TAG, "device of address " + device.getAddress() + " is removed in Connected state");
                            if (mCurrentDevicesList.size() == 0) {
                                transitionTo(mDisconnected);
                             }
                             else {
		  ...
		  case HeadsetHalConstants.CONNECTION_STATE_CONNECTING:
                ...
                break;
       case HeadsetHalConstants.CONNECTION_STATE_CONNECTED:
                if (mCurrentDevicesList.contains(device) || mCurrentDevicesList.size() >=
                        HeadsetHalConstants.MAX_HFP_CONNECTIONS) {
                    break;
                }
                Log.w(TAG, "HFP Connected from Connected state");
                ...
                if (okToConnect(device)){
                    ...
                    synchronized (HeadsetStateMachine.this) {
                        if(!mCurrentDevicesList.contains(device)) {
                            mCurrentDevicesList.add(device);
                            Log.d(TAG, "device of address " + device.getAddress() + " is added in Connected state");
                        }
                        transitionTo(mConnected);
                    }
                    configAudioParameters(device);
    ...
    private class AudioOn extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case DISCONNECT_AUDIO:
                 ...
                 case CONNECT_AUDIO:
                    device = (BluetoothDevice) message.obj;
                    if(mCurrentDevicesList.contains(device) && !(isAudioOn() &&
                                        mActiveScoDevice.getAddress().equals(device.getAddress()))) {
                        connectAudioNative(getByteAddress(device));
                    }
                    break;
                case VOICE_RECOGNITION_START:
        ...
        private void processConnectionEvent(int state, BluetoothDevice device) {
            switch (state) {
                case HeadsetHalConstants.CONNECTION_STATE_DISCONNECTED:
                    if (mCurrentDevicesList.contains(device)) {
                        if(mActiveScoDevice.getAddress().equals(device.getAddress())) {
                        ...
                        synchronized (HeadsetStateMachine.this) {
                            mCurrentDevicesList.remove(device);
                            Log.d(TAG, "device of address " + device.getAddress() + " is removed in Audio ON state");
                            if(mCurrentDevicesList.size() == 0) {
                                transitionTo(mDisconnected);
                            }
                             else {
                                if(isInCall() && mActiveScoDevice.getAddress().equals(device.getAddress())) {
                                    broadcastConnectionState(mCurrentDevicesList.get(0), BluetoothProfile.STATE_CONNECTED,
                                                     BluetoothProfile.STATE_CONNECTING);
                                    transitionTo(mConnected);
                                    configAudioParameters(mCurrentDevicesList.get(0));
       ...
       case HeadsetHalConstants.CONNECTION_STATE_CONNECTED:
                ...
                if (okToConnect(device)){
                    ...
                    synchronized (HeadsetStateMachine.this) {
                        if(!mCurrentDevicesList.contains(device)) {
                            mCurrentDevicesList.add(device);
                            Log.d(TAG, "device of address " + device.getAddress() + " is added in Audio On state");
                        }
                    }
    ...
    private ServiceConnection mConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            if (DBG) Log.d(TAG, "Proxy object connected"+className);
            mPhoneProxy = IBluetoothHeadsetPhone.Stub.asInterface(service);
        }

        @Override
        public void onServiceDisconnected(ComponentName className) {
            if (DBG) Log.d(TAG, "Proxy object disconnected"+className);
            mPhoneProxy = null;
        }
    };
    ...
    int getConnectionState(BluetoothDevice device) {
        ...
        synchronized (this) {
            ...
            if (currentState == mPending){
                if (mTargetDevice != null){
                    // outgoing connection
                    if(mTargetDevice.equals(device))
                        return BluetoothProfile.STATE_CONNECTING;

                    // The device exists in the list and outgoing connection in progress
                    if(!mTargetDevice.equals(device) && mCurrentDevicesList.contains(device))
                ...
                if (mIncomingDevice != null){
                    ...
                    if(mIncomingDevice.equals(device))
                        return BluetoothProfile.STATE_CONNECTING;
                    // Incoming connection in progress and the device exists in the list
                    if(!mIncomingDevice.equals(device) && mCurrentDevicesList.contains(device))
                        return BluetoothProfile.STATE_CONNECTED;
                }
                // No incoming or outgoing connection
                else if (mTargetDevice == null && mIncomingDevice == null && mCurrentDevicesList.contains(device)){
    ...
    List<BluetoothDevice> getConnectedDevices() {
        ...
        synchronized(this) {
           // if (isConnected()) {
                for(int i=0;i<mCurrentDevicesList.size();i++)
                    devices.add(mCurrentDevicesList.get(i));
           // }
        }
        return devices;
    }
    ...
    //third HF connection is accepted after disconnecting first HF
    //If SCO is ON Headset which do not connected to SCO is disconnected
    private boolean connectionHandler(BluetoothDevice device){
        BluetoothDevice currentConnectedDevice = null;
        IState currentState = getCurrentState();
        if (mCurrentDevicesList.contains(device))
            return false;
        if(mCurrentDevicesList.size() == HeadsetHalConstants.MAX_HFP_CONNECTIONS){
            currentConnectedDevice = mCurrentDevicesList.get(0);
            if(currentState == mAudioOn){
                if ((HeadsetHalConstants.MAX_HFP_CONNECTIONS  > 1)){
                    if(mActiveScoDevice.equals(currentConnectedDevice)){
                        currentConnectedDevice = mCurrentDevicesList.get(1);
                     }
                }
            }
            if (!disconnectHfpNative(getByteAddress(currentConnectedDevice)))
                return false;
            else
                return true;
        }
    ...
    boolean isAudioConnected(BluetoothDevice device) {
        synchronized(this) {
            ...
            if (getCurrentState() == mAudioOn && mCurrentDevicesList.contains(device)
                && mAudioState != BluetoothHeadset.STATE_AUDIO_DISCONNECTED)
            {
                if(mActiveScoDevice != null && mActiveScoDevice.getAddress().equals(device.getAddress())) {
                    return true;
                }
            }
        }
    ...
    private void processVrEvent(int state,BluetoothDevice device) {
        ...
        if (state == HeadsetHalConstants.VR_STATE_STARTED) {
            ...
            {
                try {
                    sVoiceCommandIntent.putExtra(BluetoothDevice.EXTRA_DEVICE, device);
    ...
    {
        BluetoothDevice device = mAdapter.getRemoteDevice("FF:FF:FF:FF:FF:FF");
        if (state == HeadsetHalConstants.VR_STATE_STARTED)
        {
            ...
            if (needAudio && !isAudioOn())
            {
                ...
                /* If VR was started by HF Device, we will have device to start SCO with
                ** else we will set up SCO with the last SCO connected device (send NULL)
                */
                connectAudioNative(getByteAddress(device));
            }
        ...
        else
        {
            ...
            if (mVoiceRecognitionStarted || mWaitingForVoiceRecognition)
            {
                ...
                if (stopVoiceRecognitionNative() && !isInCall() && mActiveScoDevice != null) {
                    disconnectAudioNative(getByteAddress(mActiveScoDevice));
                    mAudioManager.setParameters("A2dpSuspended=false");
                }
            }
    ...
    /* hacked up helper function to retrieve the device that trigger the dial/vr/... */
    private BluetoothDevice getDeviceForMessage(int what)
    {
        ...
        for (BluetoothDevice device : mCurrentDevicesList)
        {
        ...
        log("getDeviceForMessage: No matching device for " + what);
        return null;
    }

    private BluetoothDevice getMatchingDevice(BluetoothDevice device)
    {
        for (BluetoothDevice matchingDevice : mCurrentDevicesList)
        {
    ...
    void setVirtualCallInProgress(boolean state) {
        ...
        /* Virtual call triggers SCO connect/disconnect in the stack. We need to make sure
        ** that the AVDTP is suspended prior to SCO setup, else it results in uni-directional
        ** audio. Similarly, after disconnect clear the A2dpSuspended flag so music can be
        ** streamed
        */
        if (state)
            mAudioManager.setParameters("A2dpSuspended=true");
        else if (!isInCall() && !mVoiceRecognitionStarted)
            mAudioManager.setParameters("A2dpSuspended=false");
    }
    ...
    synchronized boolean initiateScoUsingVirtualVoiceCall() {
        ...
        /* invoke this prior to notifying stack (audio setup), so a2dp can be suspended */
    ...
    private void processCodecSelectionEvent(int codec) {
        if (codec == 1) {
            mAudioManager.setParameters(HEADSET_NREC + "=on");
        } else {
            mAudioManager.setParameters(HEADSET_NREC + "=off");
    ...
    private void processAtClcc(BluetoothDevice device) {
        if (mPhoneProxy != null) {
            try {
                ...
                {
                    // TODO: Find a better to track the device that sent CLCC request
                    // The following will start a timer, and we will key off the device
                    // in the timer event to determine the device to which response is
                    // to be sent
    ...
    private void processAtXevent(String atString, BluetoothDevice device) {
     ...
     */
    private boolean processVendorSpecificAt(String atString,BluetoothDevice device) {
    ...
    private void processUnknownAt(String atString, BluetoothDevice device) {
        ...
        else if (!processVendorSpecificAt(atCommand,device))
            atResponseCodeNative(HeadsetHalConstants.AT_RESPONSE_ERROR, 0, getByteAddress(device));
    }
    ...
    private void processBrsf(int local_feature, int peer_feature, BluetoothDevice device) {
        log("local_feature:"+ local_feature +", peer_feature:"+ peer_feature);
        Log.e(TAG, "local_feature:"+ local_feature +", peer_feature:"+ peer_feature);
    }
    ...
    private void onVrStateChanged(int state,byte[] address) {
        ...
        event.valueInt = state;
	 Log.d(TAG,"bd addr of remote device is " + Utils.getAddressStringFromByte(address));
        event.device = getDevice(address);
        sendMessage(STACK_EVENT, event);
    }
    ...
    private void onBrsf(int local_feature, int peer_feature, byte[] address) {
        StackEvent event = new StackEvent(EVENT_TYPE_BRSF);
        event.valueInt = local_feature;
        event.valueInt2 = peer_feature;
        event.device = getDevice(address);
        sendMessage(STACK_EVENT, event);
    }
    ...
    private void processSendClccResponse(HeadsetClccResponse clcc) {
        for (BluetoothDevice device : mCurrentDevicesList)
        {
            if (getHandler().hasMessages(CLCC_RSP_TIMEOUT, device))
            {
                log("processSendClccResponse: sending to " + device);
        ...
        // CLCC_RSP_TIMEOUT should be removed only when the last
        // CLCC response is received from the Phone app (mIndex == 0)
        if (clcc.mIndex == 0)
    ...
    private void processSendVendorSpecificResultCode(HeadsetVendorSpecificResultCode resultCode) {
        ...
        atResponseStringNative(stringToSend,getByteAddress(mCurrentDevicesList.get(0)));
    }
    ...
    boolean okToConnect(BluetoothDevice device) {
        ...
        //HF connection not allowed in HEADSET_MODE
        else if( adapterService.getDeviceMode() == adapterService.HEADSET_MODE){
            log("HF connection not allowed in HEADSET_MODE");
            ret = false;
        }
    ...
    public void handleAccessPermissionResult(Intent intent) {
        ...
        if(mPhonebook != null) {
            ...
            atResponseCodeNative(HeadsetHalConstants.AT_RESPONSE_ERROR, 0,
                                 getByteAddress(mCurrentDevicesList.get(0)));
        }
    }
    ...
    final private static int EVENT_TYPE_BRSF = 18;
    ...
    /*package*/native boolean atResponseCodeNative(int responseCode, int errorCode, byte[] address);
