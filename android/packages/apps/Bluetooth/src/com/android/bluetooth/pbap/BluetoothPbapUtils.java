...
public class BluetoothPbapUtils {
    ...
    //See PBAP Spec 5.4.4.1 for filter fields
    public static final int FILTER_PHOTO = 3;
    public static final int FILTER_ADDRESS = 5;
    public static final int FILTER_LABEL  = 6;
    public static final int FILTER_TEL = 7;
    public static final int FILTER_EMAIL = 8;
    public static final int FILTER_MAILER = 9;
    public static final int FILTER_ORG = 16;
    public static final int FILTER_NOTE = 17;
    public static final int FILTER_URL = 20;
    ...
    public static VCardComposer createFilteredVCardComposer(final Context ctx,
        ...
        int vType = vcardType | BluetoothPbapConfig.DEFAULT_VCARD_CFG;
        ...
        if (isNameNumberOnly) {
            ...
            if (hasFilter(filter)) {
                if (!isFilterBitSet(filter, FILTER_ADDRESS) && !isFilterBitSet(filter, FILTER_LABEL)) {
                    if (V) Log.v(TAG, "Excluding addresses from VCardComposer...");
                    vType |= VCardConfig.FLAG_REFRAIN_ADDRESS;
                }
                if (!isFilterBitSet(filter, FILTER_EMAIL) && !isFilterBitSet(filter, FILTER_MAILER)) {
                    if (V) Log.v(TAG, "Excluding email addresses from VCardComposer...");
                    vType |= VCardConfig.FLAG_REFRAIN_EMAIL;
                }
                if (!isFilterBitSet(filter, FILTER_ORG)) {
                    if (V) Log.v(TAG, "Excluding organization from VCardComposer...");
                    vType |= VCardConfig.FLAG_REFRAIN_ORGANIZATION;
                }
                if (!isFilterBitSet(filter, FILTER_URL)) {
                    if (V) Log.v(TAG, "Excluding URLS from VCardComposer...");
                    vType |= VCardConfig.FLAG_REFRAIN_WEBSITES;
                }
                if (!isFilterBitSet(filter, FILTER_NOTE)) {
                    if (V) Log.v(TAG, "Excluding notes from VCardComposer...");
                    vType |= VCardConfig.FLAG_REFRAIN_NOTES;
                }
            }
        }
