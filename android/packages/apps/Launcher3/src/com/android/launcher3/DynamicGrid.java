...
class DeviceProfile {
    ...
    void updateFromConfiguration(Resources resources, int wPx, int hPx,
        ...
        if (isLandscape) {
            allAppsNumRows = (availableHeightPx - pageIndicatorOffset - 14 * edgeMarginPx) /
                    (iconSizePx + iconTextSizePx + 4 * edgeMarginPx);
            allAppsNumCols = (availableWidthPx - padding.left - padding.right - 12 * edgeMarginPx) /
                    (iconSizePx + 12 * edgeMarginPx);
        } else {
        	allAppsNumRows = (availableWidthPx - pageIndicatorOffset - 14 * edgeMarginPx) /
        		                (iconSizePx + iconTextSizePx + 2 * edgeMarginPx);
        	allAppsNumCols = (availableWidthPx - padding.left - padding.right - 8 * edgeMarginPx) /
                    (iconSizePx + 8 * edgeMarginPx);
        }
...
public class DynamicGrid {
    ...
    public DynamicGrid(Context context, Resources resources,
         ...
         */
        deviceProfiles.add(new DeviceProfile("allwinner",
                527, 951,  4, 5,  60, 14.4f, 5, 48));
