    ...
    private static class Sentinel {
        ...
        synchronized void awaitFinalization() throws InterruptedException {
            while (!finalized) {
                ...
                int size = callStack.length;
                if (size>7 && "libcore.java.lang.ref.FinalizeTest".equals(callStack[7].getClassName())
	                && "testSystemRunFinalizationReturnsEvenIfQueueIsNonEmpty".equals(callStack[7].getMethodName())) {
	            wait(2*60*1000);
	            System.logW("force cancle wait");
	            notifyAll();
	            finalized = true;
	        } else {
		    wait();
