...
*/
static int parse_scanres(char *scan_res, const char *target_bssid, 
			 char *ssid, char *sec,
			 char *channel, int *iswpa)
{
	char readline[MAX_LINE_LEN];
	char line_address[MAX_LINE_LEN], line_essid[MAX_LINE_LEN];
	char line_ch[MAX_LINE_LEN], line_enc[MAX_LINE_LEN], line_ie[MAX_LINE_LEN];
	char *ch, *name, *value;
	FILE* fp = NULL;
	int found = FALSE;
	...
	*iswpa = 0;
	...
	memset(line_address, 0, MAX_LINE_LEN);
	memset(line_essid, 0, MAX_LINE_LEN);
	memset(line_ch, 0, MAX_LINE_LEN);
	memset(line_enc, 0, MAX_LINE_LEN);
	memset(line_ie, 0, MAX_LINE_LEN);
	while ((!feof(fp)) && (fgets(readline, MAX_LINE_LEN, fp))) {
		if (found) {
			if ((ch = strstr(readline, PATTERN_ESSID))!=NULL)
				strcpy(line_essid, readline);
			else if ((ch = strstr(readline, PATTERN_ENCRYPTION))!=NULL)
				strcpy(line_enc, readline);
			else if ((ch = strstr(readline, PATTERN_ISWPA))!=NULL)
				strcpy(line_ie, readline);
			else if (((ch = strstr(readline, PATTERN_CELL))!=NULL) &&
				(ch = strstr(readline, PATTERN_ADDRESS)))
				break;
			continue;
		}
		if (((ch = strstr(readline, PATTERN_CELL))!=NULL) &&
			((ch = strstr(readline, PATTERN_ADDRESS))!= NULL) &&				
			((strcasestr(ch, target_bssid))!=NULL)) {
			found = TRUE;
		}
	}
	if (!found)
	...
	if (strlen(ssid)==0) {
		char *first, *sec;
		unsigned int i = 0;
		if ((ch = strstr(line_essid, PATTERN_ESSID)) != NULL) {
			for (first = ch; *first != '\0'; first++)
				if (*first == '"')
					break;
			for (sec = first+1; *sec != '\0'; sec++)
				if (*sec == '"')
					break;
			for (ch=first+1; ch != sec ; ch++) 
				ssid[i++] = *ch;
			DEBUG_PRINT("ssid = %s \n", ssid);
		}
	}
	if (strlen(ssid) == 0)
		printf("Warming!! No SSID, please consult with the vendor!!\n");	
	/* Channel
	...
	//Encryption
	ch = strstr(line_enc, PATTERN_ENCRYPTION);
	if ((name = strtok(ch, ":\r\n"))!=NULL 
		&& (value = strtok(NULL, ":\r\n"))!=NULL)
		strcpy(sec, value);
	...
	DEBUG_PRINT("%s = %s \n", name, sec);
	// WPA ?
	ch = strstr(line_ie, PATTERN_ISWPA);
	if (ch)
		*iswpa = 1;
	DEBUG_PRINT("iswpa = %d \n", *iswpa);
...
int collect_scanres()
{
	...
	printf("%s() target_bssid=[%s], ssid=[%s] \n",__func__, target_bssid_str, ssid);
	for (i = 0; i < MAX_SCAN_TIMES; i++) {
		sprintf(cmdstr, "iwlist %s scan | grep -e ESSID -e Channel -e Encryption -e WPA -e \"Address:\" > /data/misc/wifi/scan_res", ifName);
		RTK_SYSTEM(cmdstr);
		sleep(2);
		if ((parse_scanres("/data/misc/wifi/scan_res", target_bssid_str, ssid, sec, channel , &iswpa))!=TRUE)
			continue;
		if (strcmp(sec, "on") != 0)
...
//	RTK_SYSTEM(cmdstr);
	...
	RTK_SYSTEM(cmdstr);
	sprintf(cmdstr, "iwlist %s scan | grep -e ESSID -e Channel -e Encryption -e WPA > /data/misc/wifi/scan_res", ifName);
	RTK_SYSTEM(cmdstr);

	memset(ap_chlist, 0, MAX_CHNUM*sizeof(u8));
        if (get_ap_ch_list("/data/misc/wifi/scan_res", ap_chlist, &ap_chcnt)==0) {
...
void reset_driver_stage()
{
	char cmdstr[200];

	sprintf(cmdstr,"%s %s mode managed\n", priv_cmd, ifName);
	RTK_SYSTEM(cmdstr);
}
...
static void print_usage()
{
	...
	printf("rtw_simple_config -i wlan0 -c ./wpa_conf -p 14825903 -P iwpriv -n RTKSC_SAMPLE\n\n");
...
#endif
	//   Enter monitor mode.
	sprintf(cmdstr,"%s %s mode monitor\n", priv_cmd, ifName);
	RTK_SYSTEM(cmdstr);
