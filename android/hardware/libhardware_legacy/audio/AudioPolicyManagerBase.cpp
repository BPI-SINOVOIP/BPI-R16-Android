                                                  ...
                                                  const char *device_address)
{
    ...
    if (audio_is_input_device(device)) {
        ...
        if (mAvailableInputDevices & AUDIO_DEVICE_IN_BLUETOOTH_A2DP)
        {
            ...
            ALOGV("opening a2dp input router for output %d", output);

            mInputRouter = mpClientInterface->openInputRouter(AUDIO_DEVICE_IN_BLUETOOTH_A2DP, output);

            if (mInputRouter != 0) {
                ALOGD("successfully opened input router (handle %x)", mInputRouter);
            }
            else {
        ...
        else
        {
            ...
            if (mInputRouter != 0)
            {
                ALOGD("close input router");
                mpClientInterface->closeInput(mInputRouter);
                mInputRouter = 0;
            }
        }
        return NO_ERROR;
...
void AudioPolicyManagerBase::releaseOutput(audio_io_handle_t output)
{
    ...
    if (desc->mFlags & AudioSystem::OUTPUT_FLAG_DIRECT) {
        ...
        ALOGD("desc->mDirectOpenCount %d", desc->mDirectOpenCount);
        if (--desc->mDirectOpenCount == 0) {
            closeOutput(output);
                                                             ...
                                                             bool fromCache)
{
    ...
    ALOGV("getDeviceForStrategy() strategy %d, device %x",
                  strategy, mDeviceForStrategy[strategy]);
                                                       ...
                                                       uint32_t delayMs)
{
    ...
    for (size_t i = 0; i < NUM_STRATEGIES; i++) {
        ...
        if (curDevice == AUDIO_DEVICE_OUT_AUX_DIGITAL && device == AUDIO_DEVICE_OUT_SPEAKER)
            mute = true;
        bool doMute = false;
...
{
    ALOGV("getDeviceForVolume %x", device);
    if (device == AUDIO_DEVICE_NONE) {
...
const struct StringToEnum sFormatNameToEnumTable[] = {
    ...
    STRING_TO_ENUM(AUDIO_FORMAT_RAW_AC3),
    STRING_TO_ENUM(AUDIO_FORMAT_RAW_DTS),
    STRING_TO_ENUM(AUDIO_FORMAT_PCM_24BIT),
    STRING_TO_ENUM(AUDIO_FORMAT_PCM_DIR_16_BIT),
};
