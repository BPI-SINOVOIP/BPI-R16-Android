...
typedef void (*monitor_rssi_callback)(int client_if, bt_bdaddr_t* bda, int rssi);
typedef void (*adv_filter_callback)(int action, int status);
/**
...
typedef struct {
    ...
    monitor_rssi_callback               monitor_rssi_cb;
    adv_filter_callback                 adv_filter_cb;
...
typedef struct {
    ...
    /**
     * Monitor the RSSI for a connected device, triggering alerts if
     * a threshold is crossed.
     */
    bt_status_t (*monitor_rssi)( int client_if, const bt_bdaddr_t *bd_addr,
		    int rssi_low, int rssi_range, int rssi_high);

    /** Enable or disable adv. packet filtering */
    bt_status_t (*adv_filter_enable)( int enable );

    /** Configure a adv. filter condition  */
    bt_status_t (*adv_filter_add)(int type, int company_id,
                                  int len, const bt_uuid_t *p_uuid, const bt_bdaddr_t *bd_addr,
                                  char addr_type , const char* p_value);

    /** Clear all adv. filter conditions */
    bt_status_t (*adv_filter_clear)();
