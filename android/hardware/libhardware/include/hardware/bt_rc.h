...
typedef enum {
    ...
    BTRC_EVT_BATT_STATUS_CHANGED = 0x06,
    BTRC_EVT_SYS_STATUS_CHANGED = 0x07,
    BTRC_EVT_APP_SETTINGS_CHANGED = 0x08,
    BTRC_EVT_PLAY_APPLN_SETTING_CHANGED=0x09,
 ...
 *  state will have one of the values from btrc_connection_state_t
 */
typedef void (* btrc_connection_state_callback)(bt_bdaddr_t *bd_addr,
...
typedef struct {
    ...
    uint8_t   play_status;
    uint8_t   feature_bit_mask[16];
    uint16_t  char_set_id;
    uint8_t   text[BTRC_MAX_ATTR_STR_LEN];
} btrc_player_item_val_t;
...
} btrc_media_item_val_t;

typedef struct {
        uint8_t item_type;
        union
        {
           btrc_player_item_val_t player_item;
           btrc_folder_item_val_t folder_item;
           btrc_media_item_val_t  media_item;
        };
} btrc_folder_items_t;
...
typedef struct {
    ...
    /**
     * Disconnects the current AVRC connection
     */
    bt_status_t (*rc_disconnect)(const bt_bdaddr_t *bd_addr);
    ...
    */
    bt_status_t (*get_player_app_attr_text_rsp)( uint8_t num_attr, btrc_player_setting_text_t *p_attrs);

    /** Returns the application attributes text ("Shuffle"/"Repeat"/...)
    **  num_attr: Specifies the number of attribute values' text contained in the pointer p_vals
    */
    bt_status_t (*get_player_app_value_text_rsp)( uint8_t num_val, btrc_player_setting_text_t *p_vals);
   ...
   /* Get folder item list response from TG to CT */
    bt_status_t (*get_folder_items_list_rsp)(btrc_status_t rsp_status, uint16_t uid_counter,
                                             uint8_t num_items, btrc_folder_items_t *p_items);
