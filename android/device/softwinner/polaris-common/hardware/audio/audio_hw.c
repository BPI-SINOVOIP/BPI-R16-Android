...
//#define USE_AW_SOFTWARE_EQ
#define USERECORD_48000_SAMPLERATE
int call_flag = 0;
...
/*
* 3840 *8 = 30720
* 30720/(192000*2*4) = 0.02s

*/
#define HDMI_MULTI_PERIOD_SIZE  4096//3840
...
struct sunxi_audio_device {
    ...
    pthread_mutex_t lock;       /* see note below on mutex acquisition order */
    pthread_mutex_t out_lock;       /* see note below on mutex acquisition order */
    struct mixer *mixer;
...
struct sunxi_stream_out {
    ...
    struct echo_reference_itfe *echo_reference;
    struct sunxi_audio_device *dev;
#ifdef USE_AW_SOFTWARE_EQ
	char *eq_inbuffer;
	PostProcessSt TestPPInfo;
#endif
    int write_threshold;
    uint64_t written;
};
...
struct sunxi_volume_ctl {
	int offload_state;
	int pcm_state;
	bool offload_large_vol;
	int pre_headphone_vol;
	int pre_speaker_vol;
	int headphone_vol;
	int speaker_vol;
};
static struct sunxi_volume_ctl vol_ctl;
#define ANALOG_PHONE
...
static void select_output_device(struct sunxi_audio_device *adev)
{
	...
	if (adev->mode == AUDIO_MODE_NORMAL){
		...
		ALOGV("normal mode , speaker or headphone,****LINE:%d,FUNC:%s",__LINE__,__FUNCTION__);
...
static struct sunxi_stream_out *global_out = NULL;
static bool direct_thread_flag = false;
...
static int start_output_stream(struct sunxi_stream_out *out)
{
	...
	if ((adev->raw_flag ||direct_thread_flag)&&(out->format!=AUDIO_FORMAT_PCM_24BIT) && (out->format!=AUDIO_FORMAT_PCM_DIR_16_BIT))
	{
		return 0;
	}

	if ((out->format == AUDIO_FORMAT_PCM_24BIT) || (out->format == AUDIO_FORMAT_PCM_DIR_16_BIT)) {
		direct_thread_flag = true;
	}
...
ALOGD("%s,l:%d,out->config.rate:%d\n", __func__, __LINE__, out->config.rate);
	...
	if ((DEFAULT_OUT_SAMPLING_RATE != out->config.rate)&&(out->format!=AUDIO_FORMAT_PCM_24BIT) && (out->format!=AUDIO_FORMAT_PCM_DIR_16_BIT) )
	{
...
static int do_output_standby(struct sunxi_stream_out *out)
{
	...
	pthread_mutex_lock(&adev->out_lock);
    if (!out->standby) {
		if (out->pcm)
		{
			pcm_close(out->pcm);
	...
	if ((out->format == AUDIO_FORMAT_PCM_24BIT) || (out->format == AUDIO_FORMAT_PCM_DIR_16_BIT)) {
		adev->raw_flag = false;
		direct_thread_flag = false;
	}
	if (out->config.rate > 48000) {
		normal_play_volume(AUDIO_DEVICE_OUT_WIRED_HEADSET, 59);
		normal_play_volume(AUDIO_DEVICE_OUT_SPEAKER, 29);
	}
	pthread_mutex_unlock(&adev->out_lock);
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
	...
	if (ret >= 0)
	{
		...
		ALOGE("--------------USER FORCE TO SET AUDIO_PARAMETER_RAW_DATA_OUT: %d", bval);
		pthread_mutex_lock(&adev->lock);
		pthread_mutex_lock(&out->lock);
...
{
	struct sunxi_stream_out *out = (struct sunxi_stream_out *)stream;
	int I_left,I_headset;
	if (out->sample_rate > 48000) {
		I_left = (int)(left*COMPRESS_PLAYBACK_VOLUME_MAX);
		if (I_left>=6000) {
			I_left = 30;
		} else if ((I_left>=5000)&&(I_left<6000)) {
			I_left = 27;
		} else if ((I_left>=4000)&&(I_left<5000)) {
			I_left = 25;
		} else if ((I_left>=3000)&&(I_left<4000)) {
			I_left = 23;
		} else if ((I_left>=2000)&&(I_left<3000)) {
			I_left = 21;
		} else if ((I_left>=1500)&&(I_left<2000)) {
			I_left = 19;
		} else if ((I_left>=1000)&&(I_left<1500)) {
			I_left = 17;
		} else if ((I_left>=500)&&(I_left<1000)) {
			I_left = 15;
		} else if ((I_left>=400)&&(I_left<500)) {
			I_left = 13;
		} else if ((I_left>=300)&&(I_left<400)) {
			I_left = 11;
		} else if ((I_left>=150)&&(I_left<300)) {
			I_left = 9;
		} else if ((I_left>=100)&&(I_left<150)) {
			I_left = 7;
		} else if ((I_left>=50)&&(I_left<100)) {
			I_left = 5;
		} else if ((I_left>=30)&&(I_left<50)) {
			I_left = 3;
		} else if ((I_left>=10)&&(I_left<30)) {
			I_left = 1;
		} else {
			I_left = 0;
		}
		...
		I_headset = I_left*2;
		normal_play_volume(AUDIO_DEVICE_OUT_WIRED_HEADSET, I_headset);
		vol_ctl.headphone_vol = I_headset;
                         ...
                         size_t bytes)
{
    ...
    if (out->standby) {
		if (global_out!=NULL) {
			if (out->config.rate > 48000) {
				normal_play_volume(AUDIO_DEVICE_OUT_WIRED_HEADSET, vol_ctl.headphone_vol);
			}
			ALOGD("android audio underrun %s,l:%d,g_r:%d,out->config.rate:%d,h_v:%d,s_v:%d", __func__, __LINE__,global_out->config.rate, out->config.rate,vol_ctl.headphone_vol,vol_ctl.speaker_vol);
			do_output_standby(global_out);
			usleep(200000);
		} else {
			ALOGD("mixthread global_out is null:%s,l:%d,h_v:%d\n", __func__, __LINE__, vol_ctl.headphone_vol);
		}
...
#ifdef USE_AW_SOFTWARE_EQ
	memcpy(out->eq_inbuffer, buf, out_frames * frame_size);
	{
		databuf Input, Output;
		int idx = 0;
		int param[10] = {0};
		char value[PROPERTY_VALUE_MAX];
		out->TestPPInfo.InputPcmBuf  = &Input;
		out->TestPPInfo.OutputPcmBuf = &Output;
		out->TestPPInfo.channal = 2;
		out->TestPPInfo.samplerate = out->config.rate;
		out->TestPPInfo.fadeinoutflag = 0;
		out->TestPPInfo.VPS = 0;
		out->TestPPInfo.spectrumflag = 0;
		out->TestPPInfo.UserEQ[0] = 0xff;//bass 0xff
		/* Gets the current switch state. Default value is open. */
		property_get("persist.sys._31_HZ", value, "-12");
		param[0] = atoi(value);
		property_get("persist.sys._62_HZ", value, "0");
		param[1] = atoi(value);
		property_get("persist.sys._125_HZ", value, "4");
		param[2] = atoi(value);
		property_get("persist.sys._250_HZ", value, "6");
		param[3] = atoi(value);
		property_get("persist.sys._500_HZ", value, "5");
		param[4] = atoi(value);
		property_get("persist.sys._1000_HZ", value, "3");
		param[5] = atoi(value);
		property_get("persist.sys._2000_HZ", value, "3");
		param[6] = atoi(value);
		property_get("persist.sys._4000_HZ", value, "3");
		param[7] = atoi(value);
		property_get("persist.sys._8000_HZ", value, "6");
		param[8] = atoi(value);
		property_get("persist.sys._16000_HZ", value, "6");
		param[9] = atoi(value);

		for(idx = 0; idx<10 ;idx++)
		{
			out->TestPPInfo.UserEQ[idx+1] = param[idx];
		}
		out->TestPPInfo.bps = 16;
		out->TestPPInfo.InputPcmBuf->pcm16 = out->eq_inbuffer;
		out->TestPPInfo.InputPcmLen = out_frames;
		out->TestPPInfo.OutputPcmBuf->pcm16 = buf;
		out->TestPPInfo.OutputPcmBufTotLen = out_frames * frame_size;
		do_auPostProc(&out->TestPPInfo);
		out_frames = out->TestPPInfo.OutputPcmLen;
	}
#endif
...
static int start_input_stream(struct sunxi_stream_in *in)
{
	...
	in->pcm = pcm_open(1, PORT_CODEC, PCM_IN, &in->config);

    if (!pcm_is_ready(in->pcm)) {
        ALOGE("cannot open pcm_in card1 driver: %s", pcm_get_error(in->pcm));
        pcm_close(in->pcm);
        adev->active_input = NULL;
        #ifdef USERECORD_48000_SAMPLERATE
		in->config.rate = SAMPLING_RATE_48K;
		#endif
		ALOGD("%s,l:%d,in->requested_rate:%d, in->config.rate:%d\n", __func__, __LINE__, in->requested_rate, in->config.rate);

        //add by Derek
        in->pcm = pcm_open(0, PORT_CODEC, PCM_IN, &in->config);
        if (!pcm_is_ready(in->pcm))
        {
        		ALOGE("cannot open pcm_in card0 driver: %s", pcm_get_error(in->pcm));
		        pcm_close(in->pcm);
		        adev->active_input = NULL;
		        return -ENOMEM;
        }
                                   ...
                                   struct audio_stream_out **stream_out)
{
...
#ifdef USE_AW_SOFTWARE_EQ
	out->eq_inbuffer = malloc(RESAMPLER_BUFFER_SIZE);
#endif
    ...
    if (flags & AUDIO_OUTPUT_FLAG_DIRECT &&
        ...
        out->config.period_size = HDMI_MULTI_PERIOD_SIZE;
		if((out->format==AUDIO_FORMAT_PCM_24BIT) || (out->format==AUDIO_FORMAT_PCM_DIR_16_BIT))
		{
			ALOGE("CHENGKAN -----  HIFI OPEN OUTPUT!!,out->format:%x\n", out->format);
			if ((out->config.rate == 192000) && out->format==AUDIO_FORMAT_PCM_24BIT) {
				out->config.period_size = out->config.period_size*2;
			}
			ladev->raw_flag = true;
		}
    } else {
    ...
    out->standby 	= 1;
#ifdef USE_AW_SOFTWARE_EQ
	memset(&out->TestPPInfo,0,sizeof(out->TestPPInfo));
#endif
	if(!(flags & AUDIO_OUTPUT_FLAG_DIRECT)) {
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    ret = str_parms_get_str(parms, AUDIO_PARAMETER_LINEIN_EN, value, sizeof(value));
    if (ret >= 0) {
        val = atoi(value);
		fm_path_enable(val);
    }
...
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume)
{
    struct sunxi_audio_device *adev = (struct sunxi_audio_device *)dev;
	int I_volume;
	ALOGV("adev_set_voice_volume,l:%d, volume: %f", __LINE__, volume);
	if (adev->mode == AUDIO_MODE_NORMAL) {
		I_volume = (int)((volume*100/10)*3);
		I_volume = 30;
		normal_play_volume(AUDIO_DEVICE_OUT_WIRED_HEADSET, (I_volume*2-1));
		I_volume = 29;
		vol_ctl.headphone_vol = (I_volume*2-1);
	} else if (adev->mode == AUDIO_MODE_FM) {
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    *stream_in = NULL;
	...
	ALOGV("to malloc requested_rate: %d, in-buffer: period_size: %d, frame_size: %d",
		in->requested_rate, in->config.period_size, audio_stream_frame_size(&in->stream.common));
