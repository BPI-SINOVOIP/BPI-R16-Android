...
static int set_fm_path_enable(struct codec_client *client, bool enable)
{
	ALOGV("%s,l:%d,enable:%d\n", __func__, __LINE__, enable);
	if (enable) {
		mixer_ctl_set_value(client->mixer_ctls->audio_linein_in, 0, 1);
	} else {
		mixer_ctl_set_value(client->mixer_ctls->audio_linein_in, 0, 0);
	}
	return 0;
}
...
struct fm_ops pad_fm_ops = {
    ...
    .set_fm_path_enable=set_fm_path_enable,
