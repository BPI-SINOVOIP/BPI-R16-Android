...
static const int chip_id_value[] = {0x55,0x06,0x08,0x02,0xa3,0x11};
...
static int ctp_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	if(twi_id == adapter->nr){
        ...
        printk("twi_id = %d,addr:0x%x,chip_id_value:0x%x\n",twi_id,client->addr,ret);
        if(ret < 0){
			...
			chip_id = ret;
			printk("%s:%d+++chip_id = %#x =============== ok!\n",__func__,__LINE__,chip_id);		
    		return 0;
		}
        	
	}else{
//		printk("ctp_detect fail........\n");
		return -ENODEV;
	}
}
...
//			printk("***0001*** %s:goio_status = %d\n",__func__,__gpio_get_value(config_info.wakeup_gpio.gpio));
			__gpio_set_value(config_info.wakeup_gpio.gpio, 0);
			msleep(ms);
//			printk("***0002*** %s:goio_status = %d\n",__func__,__gpio_get_value(config_info.wakeup_gpio.gpio));
			__gpio_set_value(config_info.wakeup_gpio.gpio, 1);
//			printk("***0003*** %s:goio_status = %d\n",__func__,__gpio_get_value(config_info.wakeup_gpio.gpio));
		}
	}
	if (status == 1) {
		if(ms == 0) {
...
{
	printk("fts_get_upgrade_info-----------chip_id = %#x\n",chip_id);
	switch (DEVICE_IC_TYPE) {
	case IC_FT5X06:    //IC_FT5X06:
		...
		break;
	case IC_FT5306:    //IC_FT5606||IC_FT5506
...
//	case 0x0a:    //IC FT5316
		...
		break;
	case IC_FT5336:    //IC_FT5336
		upgrade_info->delay_55 = FT5336_UPGRADE_55_DELAY;
		upgrade_info->delay_aa = FT5336_UPGRADE_AA_DELAY;
		upgrade_info->upgrade_id_1 = FT5336_UPGRADE_ID_1;
		upgrade_info->upgrade_id_2 = FT5336_UPGRADE_ID_2;
		upgrade_info->delay_readid = FT5336_UPGRADE_READID_DELAY;
		break;
	default:
		break;
	}
...
E_UPGRADE_ERR_TYPE  ft5x06_ctpm_fw_upgrade(u8* pbt_buf, u16 dw_lenth)
{
	...
	printk("ft5x06_ctpm_fw_upgrade===========01\n");
	fts_get_upgrade_info(&upgradeinfo);
	printk("ft5x06_ctpm_fw_upgrade===========02\n");
        /*********Step 1:Reset  CTPM *****/
...
//		cmd_write(0x55,0xaa,0x00,0x00,2);
	...
	if (reg_val[0] == upgradeinfo.upgrade_id_1&& reg_val[1] == upgradeinfo.upgrade_id_2) {
		printk("Step 3: CTPM ID,ID1 = %#x,ID2 = %#x\n",reg_val[0],reg_val[1]);
	} 
	else {
...
#define BUFFER_LEN (2)            //len == 2 
	...
	if ( 0xa6 == a || a != b ){
		...
		if(chip_id == 0x55 || chip_id == 0x08 || chip_id == 0x00 || chip_id == 0x0a || chip_id == 0x11){
...
static void ft5x_report_multitouch(void)
{
	struct ft5x_ts_data *data = i2c_get_clientdata(this_client);
	struct ts_event *event = &data->event;
//	printk("ft5x_report_multitouch-----------into\n");
#ifdef TOUCH_KEY_SUPPORT
	if(1 == key_tp){
		return;
	}
	...
	case 1:
		...
		input_report_abs(data->input_dev, ABS_MT_WIDTH_MAJOR, 30);
		input_mt_sync(data->input_dev);
		printk("report data:===x1 = %d,y1 = %d ====\n",event->x1,event->y1);
		break;
	default:
		printk("report data:==touch_point default =\n");
		break;
	}
...
static void ft5x_ts_early_suspend(struct early_suspend *handler)
{
	...
	struct ft5x_ts_data *data = i2c_get_clientdata(this_client);
	dprintk(DEBUG_SUSPEND,"==ft5x_ts_suspend=\n");
	printk("CONFIG_HAS_EARLYSUSPEND: write FT5X0X_REG_PMODE .\n");
	ft5x_ts_release();
...
static void ft5x_ts_late_resume(struct early_suspend *handler)
{
	...
	printk("==CONFIG_HAS_EARLYSUSPEND:ft5x_ts_resume== \n");
...
//	dprintk(DEBUG_INIT,"read chip_id timers,timers=%d\n",i);
	printk("====%s read chip_id = %#x, timers_i = %d ######.  \n", __func__,chip_id,i);
	i = 0;
...
{
//	printk("====%s====\n",__func__);
	printk("[aw_ioctl]line :%d,cmd = %d,arg = %ld.\n",__LINE__,cmd,arg);
