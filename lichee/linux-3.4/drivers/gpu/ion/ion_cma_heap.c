...
#include <asm/page.h>
#include <linux/module.h> /* for export symbols */
...
{
	//struct page *page = virt_to_page(cpu_addr); /* cma use highmem, cannot use virt_to_page */
			...
			void *cpu_addr, dma_addr_t handle, size_t size)
{
	...
	for (i = 0; i < (PAGE_ALIGN(size) / PAGE_SIZE); i++) {
		page = phys_to_page((u32)handle + (i * PAGE_SIZE));
...
static struct ion_heap_ops ion_cma_ops = {
	...
	.unmap_kernel = ion_cma_unmap_kernel /* if not defined, pvr will report NULL pointer */
};
