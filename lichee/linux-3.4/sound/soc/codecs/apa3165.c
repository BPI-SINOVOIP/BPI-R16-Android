...
void apa3165_linein_play(bool on)
{
	...
	if (on) {
		...
		apa3165->jack_status = SND_JACK_LINEIN;
		/*
		* pa PDN gpio control must enable for apa3165 reg read/write.
		* so apa3165_linein_work must run after gpio_set_value(pa_item.gpio.gpio, 1);
		*/
				      ...
				      const struct i2c_device_id *i2c_id)
{
	...
	type = script_get_item("apa3165", "aux_signal", &g_aux_signal_item);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
