...
static int g_aux_plugin_count 		= 0;
static int g_aux_plugout_count  	= 0;
...
static int linein_to_hp_used		= 0;
static int linein_to_aif2_used		= 0;
static int sysaudio_to_aif2_used	= 0;
static int g_audio_lowlevel_detect  = 0;
...
static void aif2_clk_en(bool on)
{
	if (on) {
		...
		if (linein_to_aif2_used&&(!linein_plugin)&&(!play_running)) {
...
static void codec_hp_en(int on)
{
	...
	if (on) {
		...
		if ((play_running == 1) || (linein_plugin == true)) {/*used for change the path*/
			pr_debug("%s,line:%d\n",__func__,__LINE__);
			codec_wr_prcm_control(DAC_PA_SRC, 0x1, LHPPAMUTE, 0x1);
			codec_wr_prcm_control(DAC_PA_SRC, 0x1, RHPPAMUTE, 0x1);
			reg_val = read_prcm_wvalue(HP_VOLC);
...
static void linein_jack_events(unsigned long arg)
{
	...
	if (linein_to_spk_used||linein_to_aif2_used||linein_to_hp_used) {
		...
		/*
		* while linein plugin, gpio is high level(detect by hardware designed).
		...
		*/
		...
		if (!ret) {
			...
			if ((linein_plugin == false) && (g_aux_plugin_count >= AUX_PLUG_DEBOUNCRE_COUNT)) {
				linein_plugin = true;
				/*
				* for audiocodec linein jack detect.
				*/
				if (linein_to_spk_used||linein_to_hp_used||linein_to_aif2_used) {
					snd_jack_report(jack.jack, SND_JACK_LINEIN);
			...
			if ((linein_plugin == true) && (g_aux_plugout_count >= AUX_PLUG_DEBOUNCRE_COUNT)) {
				linein_plugin = false;
				/*
				* for audiocodec linein jack detect.
				*/
				if (linein_to_spk_used||linein_to_hp_used||linein_to_aif2_used) {
					snd_jack_report(jack.jack, SND_JACK_LINEIN_PLUGOUT);
	...
	struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (!(codec_analog_mainmic_en || codec_analog_headsetmic_en ||codec_digital_headsetmic_en ||
			...
			/*diable clk parts*/
			if ((1 == cap_running) || (linein_to_aif2_used && linein_plugin)){
