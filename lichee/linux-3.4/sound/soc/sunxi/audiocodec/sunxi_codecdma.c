...
static dma_addr_t codec_dma_addr = 0;
static unsigned char *codec_dma_area; /* DMA area */
#endif
...
#if defined CONFIG_ARCH_SUN8IW5
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		codec_dma_addr = substream->dma_buffer.addr;
        codec_dma_area = substream->dma_buffer.area;
        if (params_buffer_bytes(params) <= 0x4000) {
			substream->dma_buffer.addr = 0x00002400;
			substream->dma_buffer.area = (unsigned char *)0xf0002400;
...
static int sunxi_pcm_hw_free(struct snd_pcm_substream *substream)
{
	#if defined CONFIG_ARCH_SUN8IW5
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		substream->dma_buffer.addr = codec_dma_addr;
		substream->dma_buffer.area = codec_dma_area;
	}
	#endif
