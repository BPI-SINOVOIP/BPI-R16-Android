...
static int sndi2s1_soc_probe(struct snd_soc_codec *codec)
{
	...
	/* get config of cx20921 codec reset gpio pin*/
	type = script_get_item("cx_codec", "cx20921_reset", &item);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		pr_err("cx_codec cx20921_reset script_get_item return type err\n");
		cx_codec_reset = 0;
	} else {
		cx_codec_reset = item.gpio.gpio;
		/*request cx20921 codec reset control gpio*/
		ret = gpio_request(cx_codec_reset, NULL);
		if (0 != ret) {
			...
			*/
			gpio_direction_output(cx_codec_reset, 1);
			gpio_set_value(cx_codec_reset, 1);
			// reset the cx20921 codec
			gpio_set_value(cx_codec_reset, 0);
			msleep(200);
			gpio_set_value(cx_codec_reset, 1);
		}
	}
