...
int printf(const char *fmt, ...)
{
    ...
    if (current_log_level > (gd->debug_mode & 0x0000000F))
        return 0;
	...
	/* Print the string */
    if (gd->debug_mode & 0x000000F0) {
        int msecond = get_timer_masked();
	    print_len = sprintf(printbuffer_with_timestamp,"[%u.%u]%s",msecond/1000,msecond%1000, p + plen);
        p = printbuffer_with_timestamp;
        plen = 0;
    }
