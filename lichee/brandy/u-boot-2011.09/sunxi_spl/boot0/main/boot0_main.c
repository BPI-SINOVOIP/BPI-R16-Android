...
void main( void )
{
    ...
    if(fel_flag == SUNXI_RUN_EFEX_FLAG)
    {
    	...
    	printf(BOOT_NOTICE"eraly jump fel\n");
...
//  printf("BT0_head.boot_head.boot_cpu=0x%x\n", BT0_head.boot_head.boot_cpu);
	...
	{
	    //mdfs_save_value();
		printf(BOOT_INFO"dram size =%d\n", dram_size);
	}
	else
	{
		printf(BOOT_ERR"initializing SDRAM Fail.\n");
	...
	printf(BOOT_INFO"Ready to disable icache.\n");
	...
	if( status == 0 )
	{
		...
		printf(BOOT_NOTICE"Jump to secend Boot.\n");
...
static void print_version(void)
{
	printf(BOOT_NOTICE"boot0 version : %s\n", BT0_head.boot_head.platform + 2);
