...
int NAND_LogicInit(int boot_mode)
{
	...
	if((!boot_mode)&&(nand_mbr.PartCount!= 0)&&(mbr_burned_flag ==0))
	{
		...
		result = nand_info_init(nand_info, 0, (NAND_BOOT0_BLK_CNT+NAND_UBOOT_BLK_CNT), (uchar *)&nand_mbr);
		mbr_burned_flag = 1;
	}
	else
	{
		printf("not burn nand partition table!\n");
		result = nand_info_init(nand_info, 0, (NAND_BOOT0_BLK_CNT+NAND_UBOOT_BLK_CNT), NULL);
	}
...
int  NAND_EraseBootBlocks(void)
{
	...
	for( i = 0;  i < (NAND_BOOT0_BLK_CNT+NAND_UBOOT_BLK_CNT);  i++ )
	{
		para.chip = 0;
		para.block = i;
...
int  NAND_EraseChip(void)
{
	...
	for( i = 0;  i < chip_cnt;  i++ )
	{
        ...
        printf("erase chip %u \n", chip);
		if(i==0)
			start_blk = NAND_BOOT0_BLK_CNT+NAND_UBOOT_BLK_CNT;
		else
			start_blk = 0;
...
__s32 burn_uboot_in_many_blks(__u32 UBOOT_buf, __u32 length)
{
     __u32 i, k;
    __u8  oob_buf[32];
    __u32 page_size, pages_per_block, pages_per_copy, page_index,uboot_copy;
    struct boot_physical_param  para;
